...
const ORDER_SIZES: [OrderSize; 3] = [
    OrderSize { gold: 2, minutes: 144.0 },
    OrderSize { gold: 3, minutes: 210.0 },
    OrderSize { gold: 4, minutes: 276.0 },
];
...
pub fn rarity_for_level(building_data: &BuildingDataFile, level: i32) -> usize {
    ...
    phases
        .get(idx)
        .map_or(MAX_RARITY, |p| p.order_rarity.clamp(1, MAX_RARITY as i32) as usize)
}
...
fn lmd_per_minute(effects: &[OrderEffect], mix: [f64; 3]) -> f64 {
    let (lmd, minutes) = ORDER_SIZES
        .iter()
        .zip(mix)
        .fold((0.0, 0.0), |(l, m), (size, w)| {
            (l + w * order_lmd(effects, *size), m + w * size.minutes)
        });
    if minutes <= 0.0 { 0.0 } else { lmd / minutes }
}
...
mod tests {
    ...
    fn tequila_needs_high_orders() {
        assert!(value_pct(&[TEQUILA], 2).abs() < 1e-9, "no 4-gold orders below level 3");
    ...
    fn proviso_and_tequila_compose_on_disjoint_orders() {
        ...
        assert!(both > alone, "Tequila still boosts the 4-gold orders Proviso leaves alone");
    ...
    fn tailoring_shifts_the_mix_only_where_high_orders_exist() {
        ...
        assert!(v > 0.0 && v < 5.0, "tailoring throughput at L3 is marginal: {v}");
    }
