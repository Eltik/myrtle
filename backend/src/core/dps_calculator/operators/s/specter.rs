...
impl Specter {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("module", "vsBlocked", false, &[], &[1], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut atk_scale = if self.unit.module_damage && ((self.unit.module_index as f64) as f64) == 1.0 { 1.1 } else { 1.0 };
        ...
        let mut dmgbuff = if ((self.unit.module_level as f64) as f64) < 2.0 { 1.0 } else { 1.03 };
        if (self.unit.module_level as f64) == 3.0 { dmgbuff = 1.05; }
        let mut hitdmg = ((final_atk * atk_scale - defense) as f64).max((final_atk * atk_scale * 0.05) as f64)*(dmgbuff);
        let mut targets = if ((self.unit.elite as f64) as f64) == 2.0 { 3.0 } else { 2.0 };
        let mut dps = hitdmg / (self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 * (((self.unit.targets as f64)) as f64).min((targets) as f64);
        return dps;
    }
}
