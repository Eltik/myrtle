...
impl SpecterAlter {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("trait", "(doll only)", true, &[], &[], 0, 0), ("skill", "vsHigherHP", false, &[], &[], 0, 0), ("trait", "vsLowerHP", false, &[3], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atkbuff = if self.unit.trait_damage { self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) * ((((self.unit.skill_index as f64) as f64)) as f64).min((1) as f64) } else { 0.0 };
        if !self.unit.trait_damage && (self.unit.module_index as f64) == 1.0 { atkbuff += 0.15; }
        ...
        hitdmg = ((final_atk * doll_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * doll_scale * 0.05) as f64);
        return hitdmg;
        }
        ...
        final_atk = self.unit.atk * (1.0 +atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        dps = hitdmg/(self.unit.attack_interval as f64) * (self.unit.attack_speed+self.unit.skill_parameters.get(1).copied().unwrap_or(0.0))/ 100.0;
        }
        if (self.unit.skill_index as f64) == 3.0 {
        let mut dmgbonus = 1.0 + self.unit.skill_parameters.get(2).copied().unwrap_or(0.0);
        if !self.unit.skill_damage { dmgbonus = 1.0; }
        final_atk = self.unit.atk * (1.0 +atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk  - defense) as f64).max((final_atk * 0.05) as f64) * dmgbonus;
        dps = hitdmg/2.2 * self.unit.attack_speed/ 100.0 * (((self.unit.targets as f64)) as f64).min((2) as f64);
        }
        return dps;
    }
}
...
impl DpsCalculator for SpecterAlter {
