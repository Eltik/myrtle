...
impl Stainless {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("skill", "TurretOnly", true, &[0], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        dps = hitdmg/(self.unit.attack_interval as f64) * (self.unit.attack_speed + self.unit.skill_parameters.get(1).copied().unwrap_or(0.0)*(self.unit.skill_index as f64)/ 3.0)/ 100.0;
        if !self.unit.skill_damage { dps = 0.0; }
        ...
        let mut turrethitdmg = ((final_atk * turret_scale - defense) as f64).max((final_atk * turret_scale * 0.05) as f64);
        let mut turretaoedmg = ((final_atk * turret_aoe - defense) as f64).max((final_atk * turret_aoe * 0.05) as f64);
...
impl DpsCalculator for Stainless {
