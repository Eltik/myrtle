...
impl SilverAsh {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("trait", "rangedAtk", true, &[3], &[], 0, 0), ("module", "vsElite", false, &[], &[1], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        if self.unit.talent_damage && self.unit.module_damage { atk_scale *= 1.1; }
        }
        if (self.unit.module_level as f64) == 3.0 {
        if self.unit.talent_damage && self.unit.module_damage { atk_scale *= 1.15; }
        }
        }
        ...
        dps = avgphys/((self.unit.attack_interval as f64)/(self.unit.attack_speed/ 100.0)) + bonusdmg/((self.unit.attack_interval as f64)/(self.unit.attack_speed/ 100.0));
        }
        if (self.unit.skill_index as f64) == 2.0 {
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * atk_scale - defense) as f64).max((final_atk * atk_scale * 0.05) as f64);
        bonusdmg = ((final_atk * bonus *(1.0 -res/ 100.0)) as f64).max((final_atk * bonus * 0.05) as f64);
        dps = hitdmg/((self.unit.attack_interval as f64)/(self.unit.attack_speed/ 100.0)) + bonusdmg/((self.unit.attack_interval as f64)/(self.unit.attack_speed/ 100.0));
        }
        ...
        hitdmg = ((final_atk * atk_scale - defense) as f64).max((final_atk * atk_scale * 0.05) as f64);
        bonusdmg = ((final_atk * bonus *(1.0 -res/ 100.0)) as f64).max((final_atk * bonus * 0.05) as f64);
        dps = hitdmg/((self.unit.attack_interval as f64)/(self.unit.attack_speed/ 100.0)) * (((self.unit.targets as f64)) as f64).min((targets) as f64) + bonusdmg/((self.unit.attack_interval as f64)/(self.unit.attack_speed/ 100.0)) * (((self.unit.targets as f64)) as f64).min((targets) as f64);
        }
        return dps;
    }
}
...
impl DpsCalculator for SilverAsh {
