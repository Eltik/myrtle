...
impl Sideroca {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("module", "blocking", false, &[], &[2], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut dmg = if ((self.unit.module_index as f64) as f64) == 2.0 && self.unit.module_damage { 1.1 } else { 1.0 };
        let mut aspd = if self.unit.talent_damage && ((self.unit.elite as f64) as f64) == 2.0 { self.unit.talent1_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        ...
        let mut dps = hitdmg / (self.unit.attack_interval as f64) * (self.unit.attack_speed + aspd)/ 100.0 * dmg;
        return dps;
    }
}
...
impl DpsCalculator for Sideroca {
