...
impl SanktaMiksaparato {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "noStacks", true, &[], &[], 1, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        aspd = if ((self.unit.elite as f64) as f64) > 0.0 && self.unit.talent_damage { self.unit.talent1_parameters.get(1).copied().unwrap_or(0.0) * 3.0 } else { 0.0 };
        if (self.unit.skill_index as f64) < 2.0 {
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk  - defense) as f64).max((final_atk  * 0.05) as f64);
        let mut skillhitdmg = ((final_atk * self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) - defense) as f64).max((final_atk * self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) * 0.05) as f64) * 3.0;
        ...
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + self.unit.skill_parameters.get(0).copied().unwrap_or(0.0)) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64)	* (((self.unit.targets as f64)) as f64).min((3) as f64);
        dps = if self.unit.skill_damage { hitdmg/(self.unit.attack_interval as f64) * 10.0 * (self.unit.attack_speed+aspd)/ 100.0 } else { hitdmg };
        }
        return dps;
    }
}
...
impl DpsCalculator for SanktaMiksaparato {
