...
impl SanktaMiksaparato {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        aspd = if ((self.unit.elite as f64) as f64) > 0.0 && self.unit.talent_damage { self.unit.talent1_parameters[1] * 3.0 } else { 0.0 };
        if (self.unit.skill_index as f64) < 2.0 {
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk  - defense) as f64).max((final_atk  * 0.05) as f64);
        let mut skillhitdmg = ((final_atk * self.unit.skill_parameters[0] - defense) as f64).max((final_atk * self.unit.skill_parameters[0] * 0.05) as f64) * 3.0;
