...
impl Surtr {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("skill", "KillingHitsOnly", false, &[], &[], 0, 0), ("module", "NotBlocking", false, &[], &[1], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut maxtargets = if ((self.unit.skill_index as f64) as f64) == 3.0 { self.unit.skill_parameters.get(6).copied().unwrap_or(0.0) } else { 1.0 };
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmgarts = ((final_atk *(1.0 -newres/ 100.0)) as f64).max((final_atk * 0.05) as f64);
