...
impl Shalem {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        aspd = if self.unit.talent_damage { self.unit.talent1_parameters[0] } else { 0.0 };
        atkbuff = if self.unit.talent_damage { self.unit.talent1_parameters[1] } else { 0.0 };
        let mut crit_rate = if ((self.unit.elite as f64) as f64) == 2.0 { self.unit.talent2_parameters[0] } else { 0.0 };
        newres = if ((self.unit.elite as f64) as f64) == 2.0 { res * (1.0 + self.unit.talent2_parameters[1]) } else { res };
        // ###the actual skills
        ...
        atk_interval = (self.unit.attack_interval as f64) * (1.0 + self.unit.skill_parameters[0]);
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        countinghits = (( self.unit.talent2_parameters[2] /(atk_interval/((self.unit.attack_speed+aspd) as f64).trunc()/ 100.0))) + 1.0;
        ...
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        countinghits = (hits * ((self.unit.talent2_parameters[2] /((self.unit.attack_interval as f64)/((self.unit.attack_speed+aspd) as f64).trunc()/ 100.0))) + 3.0)/(self.unit.targets as f64) + 1.0;
        nocrit = ((1.0 -crit_rate) as f64).powf(countinghits as f64);
