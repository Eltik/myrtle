...
impl Shalem {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "(in IS2)", false, &[], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        aspd = if self.unit.talent_damage { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        atkbuff = if self.unit.talent_damage { self.unit.talent1_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        let mut crit_rate = if ((self.unit.elite as f64) as f64) == 2.0 { self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        newres = if ((self.unit.elite as f64) as f64) == 2.0 { res * (1.0 + self.unit.talent2_parameters.get(1).copied().unwrap_or(0.0)) } else { res };
        // ###the actual skills
        ...
        atk_interval = (self.unit.attack_interval as f64) * (1.0 + self.unit.skill_parameters.get(0).copied().unwrap_or(0.0));
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        countinghits = (( self.unit.talent2_parameters.get(2).copied().unwrap_or(0.0) /(atk_interval/((self.unit.attack_speed+aspd)/ 100.0))) as f64).trunc() + 1.0;
        nocrit = ((1.0 -crit_rate) as f64).powf(countinghits as f64);
        hitdmg = ((final_atk * (1.0 +extra_scale) * (1.0 -res/ 100.0)) as f64).max((final_atk * (1.0 +extra_scale) * 0.05) as f64);
        shreddmg = ((final_atk * (1.0 +extra_scale) * (1.0 -newres/ 100.0)) as f64).max((final_atk * (1.0 +extra_scale) * 0.05) as f64);
        avgdmg = hitdmg * nocrit + shreddmg * (1.0 -nocrit);
        dps = avgdmg/atk_interval * (self.unit.attack_speed+aspd)/ 100.0 * (((self.unit.targets as f64)) as f64).min((3) as f64);
        }
        ...
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        countinghits = (hits * ((self.unit.talent2_parameters.get(2).copied().unwrap_or(0.0) /((self.unit.attack_interval as f64)/((self.unit.attack_speed+aspd)/ 100.0))) as f64).trunc() + 3.0)/(self.unit.targets as f64) + 1.0;
        nocrit = ((1.0 -crit_rate) as f64).powf(countinghits as f64);
        hitdmg = ((final_atk * (atk_scale + extra_scale) * (1.0 -res/ 100.0)) as f64).max((final_atk * (atk_scale + extra_scale) * 0.05) as f64);
        shreddmg = ((final_atk * (atk_scale + extra_scale) * (1.0 -newres/ 100.0)) as f64).max((final_atk * (atk_scale + extra_scale) * 0.05) as f64);
        avgdmg = hitdmg * nocrit + shreddmg * (1.0 -nocrit);
        dps = 6.0 * avgdmg/(self.unit.attack_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0;
        }
        return dps;
    }
}
