...
impl SwireAlt {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "noStacks", true, &[], &[], 1, 0), ("skill", "2HitBottles", false, &[2], &[], 0, 0), ("skill", "heals>attacks", true, &[1], &[], 0, 0), ("talent", "maxModStacks", false, &[], &[2], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atkbuff = self.unit.talent1_parameters.get(3).copied().unwrap_or(0.0) * self.unit.talent1_parameters.get(2).copied().unwrap_or(0.0);
        if (self.unit.module_index as f64) == 2.0 { atkbuff += 0.2; }
        }
        ...
        if !self.unit.skill_damage { dps = dps * (3.0 /atkcycle-1.0) /(3.0 /atkcycle); }
        }
        ...
        if self.unit.skill_damage { skilldmg *= 2.0; }
        dps = hitdmg/((self.unit.attack_interval as f64)/(self.unit.attack_speed/ 100.0));
        dps = dps * (3.0 /atkcycle-1.0) /(3.0 /atkcycle) + skilldmg / 3.0;
        }
        if [0.0, 3.0].contains(&(((self.unit.skill_index as f64)) as f64)) {
        dps = hitdmg/((self.unit.attack_interval as f64)/(self.unit.attack_speed/ 100.0)) * (1.0 + (self.unit.skill_index as f64)/ 3.0);
        }
        return dps;
    }
}
