...
impl Skalter {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("module", "noModBonus", true, &[], &[1], 0, 0), ("talent", "+Seaborn", false, &[], &[], 0, 0), ("talent2", "AllyInRange(add+9%forAH)", false, &[], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        if self.unit.talent2_damage { atkbuff += self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0); }
        ...
        let mut dps = final_atk * skill_scale * ((1) as f64).max((-defense) as f64) * (self.unit.targets as f64);
        if self.unit.talent_damage { dps *= 2.0; }
        return dps;
    }
}
