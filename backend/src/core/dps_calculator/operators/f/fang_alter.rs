//! DPS calculations for FangAlter
//!
...
/// FangAlter operator implementation
pub struct FangAlter {
...
impl FangAlter {
    ...
    /// Creates a new FangAlter operator
    #[allow(unused_parens)]
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut skillhit = ((final_atk * skill_scale - defense) as f64).max((final_atk * skill_scale * 0.05) as f64) * 2.0;
        ...
        avghit = (skillhit + (atks_per_skillactivation - 1.0) * hitdmg) / atks_per_skillactivation;
        } else {
        avghit = (skillhit + ((atks_per_skillactivation ) as f64).trunc() * hitdmg) / (((atks_per_skillactivation) as f64).trunc()+1.0);
        }
        }
        ...
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        dps = hitdmg/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 * (((self.unit.targets as f64)) as f64).min(((1.0 +(self.unit.skill_index as f64)/ 2.0)) as f64);
        }
        return dps;
    }
}

impl std::ops::Deref for FangAlter {
...
impl std::ops::DerefMut for FangAlter {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.unit
    }
}

impl DpsCalculator for FangAlter {
