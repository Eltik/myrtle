...
impl Flametail {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("module", "blocking", false, &[], &[1], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atkbuff = if ((self.unit.module_index as f64) as f64) == 1.0 && self.unit.module_damage { 0.08 } else { 0.0 };
        cdmg = 1.0;
        if (self.unit.module_index as f64) == 1.0 && (self.unit.module_level as f64) > 1.0 { cdmg = if ((self.unit.module_level as f64) as f64) == 3.0 { 1.2 } else { 1.15 }; }
        let mut critrate = 0.0;
        atk_interval = if ((self.unit.skill_index as f64) as f64) == 3.0 { 1.05 * 0.7 } else { (self.unit.attack_interval as f64) };
        let mut dodge = if ((self.unit.elite as f64) as f64) == 2.0 { self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        if (self.unit.skill_index as f64) == 3.0 {
        dodge = 1.0 -((1.0 -dodge)*(1.0 -self.unit.skill_parameters.get(2).copied().unwrap_or(0.0)));
        }
        ...
        critdmg = ((final_atk * cdmg - defense) as f64).max((final_atk * cdmg * 0.05) as f64) * 2.0 * ((2) as f64).min(((self.unit.targets as f64)) as f64);
        ...
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + atkbuff) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        critdmg = ((final_atk * cdmg - defense) as f64).max((final_atk * cdmg * 0.05) as f64) * 2.0 * ((3) as f64).min(((self.unit.targets as f64)) as f64);
        avghit = critrate * critdmg + (1.0 - critrate) * hitdmg;
        dps = avghit/atk_interval * self.unit.attack_speed/ 100.0;
        }
        return dps;
...
impl DpsCalculator for Flametail {
