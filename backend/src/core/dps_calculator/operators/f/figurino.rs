...
/// Figurino operator implementation
pub struct Figurino {
...
impl Figurino {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "blocking", true, &[], &[], 1, 0)];

    /// Creates a new Figurino operator
    #[allow(unused_parens)]
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut dmg_scale = if ((self.unit.elite as f64) as f64) > 0.0 && self.unit.talent_damage { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) } else { 1.0 };
        ...
        hitdmg = ((final_atk * skill_scale - defense) as f64).max((final_atk * skill_scale * 0.05) as f64) * dmg_scale;
        dps = hitdmg * (((self.unit.targets as f64)) as f64).min((self.unit.skill_parameters.get(1).copied().unwrap_or(0.0)) as f64);
        }
        return dps;
    }
}

impl std::ops::Deref for Figurino {
...
impl std::ops::DerefMut for Figurino {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.unit
    }
}

impl DpsCalculator for Figurino {
