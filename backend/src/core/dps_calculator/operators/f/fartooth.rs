...
impl Fartooth {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "w/o talent", true, &[], &[], 0, 0), ("skill", "farAway", false, &[3], &[], 0, 0), ("module", "maxModuleBonus", false, &[], &[1], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        aspd += self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) * (self.unit.skill_index as f64)/ 2.0;
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * atk_scale - defense) as f64).max((final_atk* atk_scale * 0.05) as f64);
        dps = hitdmg/((self.unit.attack_interval as f64)/((self.unit.attack_speed+aspd)/ 100.0));
        }
        ...
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * atk_scale - defense) as f64).max((final_atk* atk_scale * 0.05) as f64)*dmgscale;
        dps = hitdmg/((self.unit.attack_interval as f64)/((self.unit.attack_speed+aspd)/ 100.0));
        }
        return dps;
    }
