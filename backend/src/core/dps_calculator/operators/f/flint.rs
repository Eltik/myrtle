...
impl Flint {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "blocking", true, &[1], &[], 0, 0), ("module", ">50%Hp", false, &[], &[1], 0, 0)];

    /// Creates a new Flint operator
    #[allow(unused_parens)]
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut dmgscale = if ((self.unit.skill_index as f64) as f64) == 1.0 && !self.unit.talent_damage { 1.0 } else { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) };
        aspd = if ((self.unit.module_index as f64) as f64) == 1.0 && self.unit.module_damage { 10.0 } else { 0.0 };
...
impl std::ops::DerefMut for Flint {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.unit
    }
}

impl DpsCalculator for Flint {
