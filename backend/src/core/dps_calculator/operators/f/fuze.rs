...
impl Fuze {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut atkbuff = if ((self.unit.skill_index as f64) as f64) == 1.0 { self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        let mut aspd = if ((self.unit.skill_index as f64) as f64) == 1.0 { self.unit.skill_parameters.get(2).copied().unwrap_or(0.0) } else { 0.0 };
        ...
        let mut max_targets = if ((self.unit.elite as f64) as f64) == 2.0 { 3.0 } else { 2.0 };
        let mut dps = hitdmg/(self.unit.attack_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0 * (((self.unit.targets as f64)) as f64).min((max_targets) as f64);
        return dps;
    }
}
