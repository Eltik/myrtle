...
impl Frostleaf {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atk_interval = if ((self.unit.elite as f64) as f64) < 2.0 { (self.unit.attack_interval as f64) } else { (self.unit.attack_interval as f64) + 0.15 };
        let mut extra_arts_scale = if ((self.unit.module_index as f64) as f64) == 1.0 { 0.1 } else { 0.0 };
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * atk_scale - defense) as f64).max((final_atk * atk_scale * 0.05) as f64);
        hitdmgarts = ((final_atk * extra_arts_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * extra_arts_scale * 0.05) as f64);
        ...
        aspd = self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) * (self.unit.skill_index as f64)/ 2.0;
