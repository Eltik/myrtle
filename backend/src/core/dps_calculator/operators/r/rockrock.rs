...
impl Rockrock {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "w/o talent", true, &[], &[], 1, 0), ("skill", "overdrive", false, &[2], &[], 0, 0), ("trait", "minDroneDmg", true, &[], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        drone_dmg = if ((self.unit.module_index as f64) as f64) == 1.0 { 0.35 } else { 0.2 };
        }
        let mut atkbuff = if self.unit.talent_damage && ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) * self.unit.talent1_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        let mut aspd = if ((self.unit.module_index as f64) as f64) == 1.0 && ((self.unit.module_level as f64) as f64) == 3.0 && self.unit.talent_damage { 5.0 } else { 0.0 };
        aspd += if ((self.unit.skill_index as f64) as f64) == 2.0 { self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) } else { self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) * ((self.unit.skill_index as f64) as f64) };
        if self.unit.skill_damage && (self.unit.skill_index as f64) == 2.0 { atkbuff += self.unit.skill_parameters.get(0).copied().unwrap_or(0.0); }
        if (self.unit.skill_index as f64) == 2.0 && self.unit.skill_damage && self.unit.trait_damage { drone_dmg *= self.unit.skill_parameters.get(3).copied().unwrap_or(0.0); }
...
impl DpsCalculator for Rockrock {
