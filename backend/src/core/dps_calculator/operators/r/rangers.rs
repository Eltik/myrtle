...
/// Rangers operator implementation
pub struct Rangers {
...
impl Rangers {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "vsAerial", false, &[], &[], 0, 0)];
    ...
    pub fn new(operator_data: OperatorData, params: OperatorParams) -> Self {
        ...
        if unit.potential > 2 { unit.attack_speed += 6.0; }
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut atk_scale = if self.unit.talent_damage { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) } else { 1.0 };
...
impl std::ops::Deref for Rangers {
...
impl std::ops::DerefMut for Rangers {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.unit
    }
}

impl DpsCalculator for Rangers {
