...
impl Rosmontis {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("skill", "withPillarDefshred", false, &[3], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut bonusart = if ((self.unit.module_index as f64) as f64) == 3.0 { 1.0 } else { 0.0 };
        defshred = if ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        newdef = ((0) as f64).max((defense - defshred) as f64);
        if (self.unit.skill_index as f64) < 2.0 {
        skill_scale = self.unit.skill_parameters.get(0).copied().unwrap_or(0.0);
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0)) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk - newdef) as f64).max((final_atk  * 0.05) as f64);
        bonushitdmg = ((final_atk * 0.5 - newdef) as f64).max((final_atk * 0.5 * 0.05) as f64) * bonushits;
        bonushitdmg += ((final_atk * (1.0 -res/ 100.0)) as f64).max((final_atk * 0.05) as f64) * bonusart;
        let mut skillhitdmg = ((final_atk * skill_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * skill_scale * 0.05) as f64);
        sp_cost = (self.unit.skill_cost as f64);
        avghit = ((sp_cost + 1.0) * (hitdmg + bonushitdmg) + skillhitdmg) / (sp_cost + 1.0);
        if (self.unit.skill_index as f64) == 0.0 { avghit = hitdmg + bonushitdmg; }
        dps = avghit/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 * (self.unit.targets as f64);
        }
        if (self.unit.skill_index as f64) == 2.0 {
        bonushits += 2.0;
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) + self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0)) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk - newdef) as f64).max((final_atk * 0.05) as f64);
        bonushitdmg = ((final_atk * 0.5 - newdef) as f64).max((final_atk * 0.5 * 0.05) as f64) * bonushits;
        bonushitdmg += ((final_atk * (1.0 -res/ 100.0)) as f64).max((final_atk * 0.05) as f64) * bonusart;
        dps = (hitdmg+ bonushitdmg)/3.15 * self.unit.attack_speed/ 100.0 * (self.unit.targets as f64);
        }
        ...
        if self.unit.shreds[0] < 1.0 && self.unit.shreds[0] > 0.0 {
        newdef *= self.unit.shreds.get(0).copied().unwrap_or(0.0);
        }
        ...
        bonushitdmg = ((final_atk * 0.5 - newdef) as f64).max((final_atk * 0.5 * 0.05) as f64) * bonushits;
        bonushitdmg += ((final_atk * (1.0 -res/ 100.0)) as f64).max((final_atk * 0.05) as f64) * bonusart;
        dps = (hitdmg+ bonushitdmg)/1.05 * self.unit.attack_speed/ 100.0 * (self.unit.targets as f64) * (((self.unit.targets as f64)) as f64).min((2) as f64);
        }
        return dps;
    }
}
