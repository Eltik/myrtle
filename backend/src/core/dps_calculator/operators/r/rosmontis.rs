...
impl Rosmontis {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        defshred = if ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent1_parameters[0] } else { 0.0 };
        newdef = ((0) as f64).max((defense - defshred) as f64);
        if (self.unit.skill_index as f64) < 2.0 {
        skill_scale = self.unit.skill_parameters[0];
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + self.unit.talent2_parameters[0]) + self.unit.buff_atk_flat;
        ...
        if (self.unit.skill_index as f64) == 2.0 {
        bonushits += 2.0;
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + self.unit.skill_parameters[1] + self.unit.talent2_parameters[0]) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk - newdef) as f64).max((final_atk * 0.05) as f64);
        bonushitdmg = ((final_atk * 0.5 - newdef) as f64).max((final_atk * 0.5 * 0.05) as f64) * bonushits;
        bonushitdmg += ((final_atk * (1.0 -res/ 100.0)) as f64).max((final_atk * 0.05) as f64) * bonusart;
