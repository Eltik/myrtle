...
impl Ray {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("trait", "outOfAmmo", true, &[], &[], 0, 0), ("talent", "with pet", false, &[], &[], 1, 0), ("talent2", "After3Hits", false, &[], &[], 2, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atk_scale = 1.2;
        let mut dmg_scale = if self.unit.talent_damage && ((self.unit.elite as f64) as f64) > 0.0 { 1.0 + self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) } else { 1.0 };
        atkbuff = if self.unit.talent2_damage && ((self.unit.elite as f64) as f64) == 2.0 { self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0) * self.unit.talent2_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        ...
        skilldmg = ((final_atk * atk_scale * skill_scale - defense) as f64).max((final_atk * atk_scale * skill_scale * 0.05) as f64) * dmg_scale;
        dps = hitdmg/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0;
        if !self.unit.trait_damage {
        dps = hitdmg/((self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 + 1.6);
        if (self.unit.module_index as f64) == 1.0 { dps = 2.0 *hitdmg/(2.0 * (self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 + 1.6); }
        }
        dps += skilldmg /((self.unit.skill_cost as f64)/(1.0 +(self.unit.sp_boost as f64))+1.2);
        }
        ...
        dps = hitdmg/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0;
        if !self.unit.trait_damage {
        dps = hitdmg/((self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 + 0.4);
        if (self.unit.module_index as f64) == 1.0 { dps = 2.0 *hitdmg/(2.0 * (self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 + 0.4); }
        }
        }
        return dps;
    }
