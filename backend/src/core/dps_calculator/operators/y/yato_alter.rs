...
impl YatoAlter {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent2", "after10s", true, &[0], &[], 2, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atkbuff = if ((self.unit.elite as f64) as f64) == 2.0 && (((self.unit.skill_index as f64) as f64) != 0.0 || self.unit.talent2_damage) { self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        ...
        hitdmgarts = ((final_atk * extra_arts * (1.0 -res/ 100.0)) as f64).max((final_atk * extra_arts * 0.05) as f64);
        dps = (hitdmg+hitdmgarts)/(self.unit.attack_interval as f64) * (self.unit.attack_speed+aspd*(self.unit.skill_index as f64))/ 100.0;
        if (self.unit.skill_index as f64) == 1.0 { dps *= 10.0 / 3.0; }
        }
        if (self.unit.skill_index as f64) == 2.0 {
        ...
        hitdmgarts = ((final_atk * atk_scale * extra_arts * (1.0 -res/ 100.0)) as f64).max((final_atk * atk_scale * extra_arts * 0.05) as f64);
        ...
        hitdmgarts = ((final_atk * skill_scale * extra_arts * (1.0 -res/ 100.0)) as f64).max((final_atk * skill_scale * extra_arts * 0.05) as f64);
