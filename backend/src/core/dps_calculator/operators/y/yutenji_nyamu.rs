...
impl YutenjiNyamu {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("module", "3inRange", false, &[], &[1], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atkbuff = if ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent2_parameters.get(2).copied().unwrap_or(0.0) } else { 0.0 };
        let mut hits = if ((self.unit.skill_index as f64) as f64) == 1.0 { 3.0 } else { 8.0 };
        if (self.unit.skill_index as f64) == 0.0 { hits = 1.0; }
        let mut prob = self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0);
        ...
        let mut counting_hits = hits * ((duration/(self.unit.attack_interval as f64)) as f64).trunc() + ((1) as f64).max((hits/ 2.0) as f64);
        ...
        let mut splashhitdmg = ((0.5 * final_atk * atk_scale - defense) as f64).max((0.5 * final_atk * atk_scale * 0.05) as f64);
        ...
        big_scale = self.unit.skill_parameters.get(0).copied().unwrap_or(0.0);
        small_scale = self.unit.skill_parameters.get(1).copied().unwrap_or(0.0);
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        bighitdmg = ((final_atk * atk_scale * big_scale - defense) as f64).max((final_atk * atk_scale * big_scale * 0.05) as f64) * ((hits/ 3.0) as f64).trunc();
        bigsplashhitdmg = ((0.5 * final_atk * atk_scale * big_scale - defense) as f64).max((0.5 * final_atk * atk_scale * big_scale * 0.05) as f64) * ((hits/ 3.0) as f64).trunc();
        smallhitdmg = ((final_atk * atk_scale * small_scale - defense) as f64).max((final_atk * atk_scale * small_scale * 0.05) as f64) * (hits - ((hits/ 3.0) as f64).trunc());
        smallsplashhitdmg = ((0.5 * final_atk * atk_scale * small_scale - defense) as f64).max((0.5 * final_atk * atk_scale * small_scale * 0.05) as f64) * (hits - ((hits/ 3.0) as f64).trunc());
        dps = (bighitdmg+smallhitdmg)/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0;
        if (self.unit.targets as f64) > 1.0 {
        dps += (bigsplashhitdmg+smallsplashhitdmg)/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 * ((self.unit.targets as f64) - 1.0);
        }
        }
...
impl DpsCalculator for YutenjiNyamu {
