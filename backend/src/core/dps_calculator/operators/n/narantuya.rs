...
impl Narantuya {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("trait", "maxRange", true, &[], &[], 0, 0), ("talent", "maxSteal", false, &[], &[], 1, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut stealbuff = if ((self.unit.elite as f64) as f64) > 0.0 && self.unit.talent_damage { self.unit.talent1_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat + stealbuff;
        ...
        interval = if self.unit.trait_damage { (self.unit.attack_interval as f64)/self.unit.attack_speed* 100.0 } else { 2.1 };
        dps = hitdmg / interval;
        if (self.unit.targets as f64) > 1.0 { dps *= 3.0; }
        }
        ...
        hitdmg = ((final_atk * skill_scale * atk_scale - defense) as f64).max((final_atk * skill_scale * atk_scale * 0.05) as f64);
        let mut returndmg = ((final_atk * return_scale * atk_scale - defense) as f64).max((final_atk * return_scale * atk_scale * 0.05) as f64) * (self.unit.targets as f64);
        interval = if self.unit.trait_damage { 1.15 } else { 2.0 };
        dps = (hitdmg+returndmg) / interval;
        }
        if [0.0, 3.0].contains(&(((self.unit.skill_index as f64)) as f64)) {
        skill_scale = if ((self.unit.skill_index as f64) as f64) == 3.0 { self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) } else { 1.0 };
        let mut aoe_scale = if ((self.unit.skill_index as f64) as f64) == 3.0 { self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat + stealbuff;
        hitdmg = ((final_atk * skill_scale * atk_scale - defense) as f64).max((final_atk * skill_scale * atk_scale * 0.05) as f64) * (((self.unit.skill_index as f64)) as f64).max((1) as f64);
        let mut aoedmg = ((final_atk * aoe_scale * atk_scale - defense) as f64).max((final_atk * aoe_scale * atk_scale * 0.05) as f64);
        if !self.unit.trait_damage { aoedmg = 0.0; }
        interval = if !self.unit.trait_damage { 20.0 /13.6 } else { ((self.unit.attack_interval as f64)/(self.unit.attack_speed/ 100.0)) };
        dps = hitdmg/interval + (((self.unit.targets as f64)) as f64).min((3) as f64) * aoedmg/interval;
        }
        return dps;
    }
}
