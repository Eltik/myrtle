...
impl Nymph {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("trait", "vsFallout(not inlcuding 800FalloutDps)", false, &[], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut talent1_scale = if self.unit.talent_damage && ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        atkbuff = if ((self.unit.elite as f64) as f64) == 2.0 && self.unit.talent2_damage { self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0) * self.unit.talent2_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        aspd = if ((self.unit.module_index as f64) as f64) == 1.0 && ((self.unit.module_level as f64) as f64) > 1.0 && self.unit.talent2_damage { self.unit.talent2_parameters.get(2).copied().unwrap_or(0.0) } else { 0.0 };
        ...
        necrosis_scale = self.unit.skill_parameters.get(1).copied().unwrap_or(0.0);
        ele_scale = self.unit.skill_parameters.get(2).copied().unwrap_or(0.0);
        ...
        dps = (hitdmg+eledmg)/(self.unit.attack_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0;
        }
        ...
        talent1_overwrite = self.unit.skill_parameters.get(3).copied().unwrap_or(0.0);
        necrosis_scale = self.unit.skill_parameters.get(1).copied().unwrap_or(0.0);
        final_atk = self.unit.atk * (1.0 +atkbuff+ self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * (1.0 -res/ 100.0)) as f64).max((final_atk * 0.05) as f64);
        skilldmg = ((final_atk * atk_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * atk_scale * 0.05) as f64) * (self.unit.targets as f64);
        dps = hitdmg/(self.unit.attack_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0 + skilldmg/sp_cost;
        }
        ...
        aspd += if ((self.unit.skill_index as f64) as f64) == 3.0 { self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        final_atk = self.unit.atk * (1.0 +atkbuff+ self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * (1.0 -res/ 100.0)) as f64).max((final_atk * 0.05) as f64);
        if self.unit.trait_damage && self.unit.talent_damage && (self.unit.skill_index as f64) == 3.0 {
        hitdmg = final_atk * ((1) as f64).max((-res) as f64) /(1.0 +self.unit.buff_fragile) * burst_scale;
        }
        dps = hitdmg/(self.unit.attack_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0 * (((self.unit.targets as f64)) as f64).min((1.0 +(self.unit.skill_index as f64)/ 3.0) as f64);
        }
        extra_dmg = 0.0;
