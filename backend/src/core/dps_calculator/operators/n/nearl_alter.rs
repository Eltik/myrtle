...
impl NearlAlter {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut aspd = if ((self.unit.module_index as f64) as f64) == 2.0 && self.unit.module_damage { 30.0 } else { 0.0 };
        let mut def_shred = if ((self.unit.elite as f64) as f64) == 2.0 { self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        let mut final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) * (((self.unit.skill_index as f64)) as f64).min((1) as f64)) + self.unit.buff_atk_flat;
        if (self.unit.skill_index as f64) == 1.0 { aspd += self.unit.skill_parameters.get(1).copied().unwrap_or(0.0); }
        let mut hitdmg = ((final_atk * atk_scale - defense * (1.0 - def_shred)) as f64).max((final_atk * atk_scale * 0.05) as f64);
...
impl DpsCalculator for NearlAlter {
