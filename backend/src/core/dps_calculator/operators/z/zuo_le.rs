...
impl ZuoLe {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        aspd = if self.unit.talent_damage && self.unit.talent2_damage { self.unit.talent1_parameters.iter().cloned().fold(f64::NEG_INFINITY, f64::max) } else { 0.0 };
        if self.unit.talent_damage && self.unit.talent2_damage { sp_recovery += self.unit.talent1_parameters.get(2).copied().unwrap_or(0.0); }
        if (self.unit.elite as f64) == 2.0 {
        sp_recovery += if self.unit.talent_damage && self.unit.talent2_damage { self.unit.talent2_parameters.get(2).copied().unwrap_or(0.0) / (self.unit.attack_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0 } else { self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0) / (self.unit.attack_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0 };
        }
        let mut tal_scale = if ((self.unit.module_index as f64) as f64) == 2.0 && self.unit.talent2_damage && self.unit.talent_damage { 0.9 + 0.1 * ((self.unit.module_level as f64) as f64) } else { 1.0 };
        let mut apply_rate = if self.unit.talent_damage && self.unit.talent2_damage && ((self.unit.elite as f64) as f64) == 2.0 { self.unit.talent2_parameters.get(2).copied().unwrap_or(0.0) } else { 0.2 };
        if (self.unit.skill_index as f64) == 0.0 {
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        hitdmg2 = ((final_atk * tal_scale - defense) as f64).max((final_atk * tal_scale * 0.05) as f64);
        ...
        let mut hits = if self.unit.talent_damage && self.unit.talent2_damage { 3.0 } else { 1.0 };
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        skilldmg = ((final_atk * atk_scale - defense) as f64).max((final_atk* atk_scale * 0.05) as f64);
        hitdmg2 = ((final_atk * tal_scale - defense) as f64).max((final_atk * tal_scale * 0.05) as f64);
        hitdmg = hitdmg * (1.0 -apply_rate) + hitdmg2 * apply_rate;
        sp_cost = (self.unit.skill_cost as f64) / (sp_recovery + (self.unit.sp_boost as f64)) + 1.2;
        ...
        if atk_scale > 1.41 {
        avghit = (skilldmg * hits  + (atks_per_skillactivation - 1.0) * hitdmg) / atks_per_skillactivation;
        } else {
        avghit = (skilldmg * hits  + ((atks_per_skillactivation) as f64).trunc() * hitdmg) / (((atks_per_skillactivation) as f64).trunc()+1.0);
        }
        }
        ...
        hitdmg = hitdmg * (1.0 -apply_rate) + hitdmg2 * apply_rate;
        skilldmg = ((final_atk * atk_scale - defense) as f64).max((final_atk* atk_scale * 0.05) as f64);
        let mut skilldmg2 = ((2.0 *final_atk * atk_scale - defense) as f64).max((2.0 *final_atk* atk_scale * 0.05) as f64);
        sp_cost = (self.unit.skill_cost as f64) / (sp_recovery + (self.unit.sp_boost as f64)) + 1.2;
        dps = hitdmg/(self.unit.attack_interval as f64) * (self.unit.attack_speed + aspd)/ 100.0;
        dps += (6.0 * skilldmg + skilldmg2) / sp_cost * (((self.unit.targets as f64)) as f64).min((3) as f64);
        }
        return dps;
    }
}
