...
impl Ascalon {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "1Stack", true, &[], &[], 0, 0), ("talent2", "NoRangedTiles", true, &[], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        if (self.unit.elite as f64) == 2.0 && self.unit.talent2_damage { aspd += self.unit.talent2_parameters.get(1).copied().unwrap_or(0.0); }
        ...
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + self.unit.skill_parameters.get(1).copied().unwrap_or(0.0)) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        dps = hitdmg/atk_interval * (self.unit.attack_speed+aspd)/ 100.0 * (self.unit.targets as f64);
        }
        dps += (self.unit.targets as f64) * final_atk * talentstacks * talentscale * ((1.0 -res/ 100.0) as f64).max((0.05) as f64);
        return dps;
    }
}
