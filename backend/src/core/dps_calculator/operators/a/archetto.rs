...
impl Archetto {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "+2ndSniper", false, &[], &[1], 0, 2), ("module", "aerialTarget", false, &[], &[1], 0, 0), ("module", "GroundEnemy", false, &[], &[2], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut recovery_interval = if ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent1_parameters.iter().cloned().fold(f64::NEG_INFINITY, f64::max) } else { 10000000.0 };
        if (self.unit.module_index as f64) == 1.0 && self.unit.talent_damage && (self.unit.module_level as f64) > 1.0 {
        recovery_interval -= if ((self.unit.module_level as f64) as f64) == 2.0 { 0.3 } else { 0.4 };
        }
        ...
        skilldmg = ((final_atk * skill_scale * atk_scale - defense) as f64).max((final_atk * skill_scale * atk_scale * 0.05) as f64);
        let mut aoedmg = ((final_atk * skill_scale2 * atk_scale - defense) as f64).max((final_atk * skill_scale2 * atk_scale * 0.05) as f64);
        // figuring out the chance of the talent to activate during downtime
        ...
        let mut hitdps = hitdmg/((self.unit.attack_interval as f64)/((self.unit.attack_speed+aspd)/ 100.0)) * (new_spcost-1.0)/new_spcost;
        let mut skilldps = skilldmg/((self.unit.attack_interval as f64)/((self.unit.attack_speed+aspd)/ 100.0)) /new_spcost;
        let mut aoedps = aoedmg/((self.unit.attack_interval as f64)/((self.unit.attack_speed+aspd)/ 100.0)) /new_spcost *((((self.unit.targets as f64)) as f64).min((4) as f64)-1.0);
        dps = hitdps + skilldps + aoedps;
        }
        ...
        dps = hitdmg/((self.unit.attack_interval as f64)/((self.unit.attack_speed+aspd)/ 100.0)) + sprecovery/(self.unit.skill_cost as f64) * skilldmg * totalhits[((targets) as usize).saturating_sub(1)];
        }
        if [0.0, 3.0].contains(&(((self.unit.skill_index as f64)) as f64)) {
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + self.unit.skill_parameters.get(0).copied().unwrap_or(0.0)*(self.unit.skill_index as f64)/ 3.0) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * atk_scale - defense) as f64).max((final_atk * atk_scale * 0.05) as f64) * (1.0 + (self.unit.skill_index as f64) * 2.0 / 3.0);
        dps = hitdmg/(self.unit.attack_interval as f64) * (self.unit.attack_speed + aspd)/ 100.0;
        if (self.unit.skill_index as f64) == 3.0 { dps *= (((self.unit.targets as f64)) as f64).min((2) as f64); }
        }
        return dps;
    }
}
