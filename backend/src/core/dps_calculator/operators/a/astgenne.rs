...
impl Astgenne {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "maxStacks", false, &[], &[], 1, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        aspd = if self.unit.talent_damage && ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) * self.unit.talent1_parameters.get(2).copied().unwrap_or(0.0) } else { 0.0 };
