...
impl AmiyaMedic {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("skill", "maxStacks", false, &[2], &[], 0, 0), ("skill", "noStacks", true, &[2], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        if (self.unit.skill_index as f64) == 2.0 {
        atkbuff = if self.unit.skill_damage { 5.0 * self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
