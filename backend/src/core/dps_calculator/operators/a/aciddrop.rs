...
impl Aciddrop {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        else if self.unit.talent_damage { mindmg = self.unit.talent1_parameters[1]; }
        else { mindmg = self.unit.talent1_parameters[0]; }
        let mut aspd = if ((self.unit.skill_index as f64) as f64) == 1.0 { self.unit.skill_parameters[0] } else { 0.0 };
