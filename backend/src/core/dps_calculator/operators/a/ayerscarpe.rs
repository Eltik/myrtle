...
impl Ayerscarpe {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        aspd = if ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent1_parameters[0] } else { 0.0 };
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat;
        if (self.unit.skill_index as f64) < 2.0 {
        skill_scale = self.unit.skill_parameters[0];
        let mut targets = if ((self.unit.skill_index as f64) as f64) == 1.0 { self.unit.skill_parameters[2] } else { 1.0 };
