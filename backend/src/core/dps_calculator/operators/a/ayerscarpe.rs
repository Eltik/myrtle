...
impl Ayerscarpe {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        aspd = if ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat;
        if (self.unit.skill_index as f64) < 2.0 {
        skill_scale = self.unit.skill_parameters.get(0).copied().unwrap_or(0.0);
        let mut targets = if ((self.unit.skill_index as f64) as f64) == 1.0 { self.unit.skill_parameters.get(2).copied().unwrap_or(0.0) } else { 1.0 };
        ...
        skilldmg = ((final_atk * skill_scale *(1.0 -res/ 100.0)) as f64).max((final_atk * skill_scale * 0.05) as f64) * (((self.unit.targets as f64)) as f64).min((targets) as f64);
        avgdmg = ((self.unit.skill_cost as f64) * hitdmg + skilldmg)/((self.unit.skill_cost as f64) + 1.0);
        if (self.unit.skill_index as f64) == 0.0 { avgdmg = hitdmg; }
        dps = (avgdmg + bonusdmg) / (self.unit.attack_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0;
        }
        if (self.unit.skill_index as f64) == 2.0 {
        ...
        skilldmg *= if self.unit.trait_damage { (self.unit.targets as f64) } else { (self.unit.targets as f64) -1.0 };
        dps = (hitdmg + bonusdmg + skilldmg) / (self.unit.attack_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0;
        }
        return dps;
    }
}
