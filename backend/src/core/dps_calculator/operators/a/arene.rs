...
impl Arene {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("trait", "rangedAtk", true, &[1], &[], 0, 0), ("talent", "vsDrones", false, &[], &[], 1, 0), ("module", "+12aspd(mod)", false, &[], &[2], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atk_scale = if self.unit.talent_damage { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) } else { 1.0 };
        aspd = if ((self.unit.module_index as f64) as f64) == 2.0 && ((self.unit.targets as f64) > 1.0 || self.unit.module_damage) { 12.0 } else { 0.0 };
        if !trait_damage && (self.unit.skill_index as f64) != 2.0 { atk_scale *= 0.8; }
