...
impl Aosta {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("trait", "distant", true, &[], &[], 0, 0), ("talent", "blockedTarget", true, &[], &[], 1, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        if self.unit.trait_damage && (self.unit.module_index as f64) == 1.0 { atk_scale = 1.6; }
        let mut talent_scale = if ((self.unit.elite as f64) as f64) > 0.0 && self.unit.talent_damage { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        let mut talent_duration = self.unit.talent1_parameters.get(1).copied().unwrap_or(0.0);
        aspd = if ((self.unit.skill_index as f64) as f64) == 1.0 { self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        if (self.unit.skill_index as f64) < 2.0 {
        final_atk = self.unit.atk * (1.0 + self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) * (self.unit.skill_index as f64) + self.unit.buff_atk) + self.unit.buff_atk_flat;
        ...
        if (self.unit.skill_index as f64) == 2.0 {
        final_atk = self.unit.atk * (1.0 + self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * atk_scale - defense) as f64).max((final_atk * atk_scale * 0.05) as f64);
        dps = hitdmg / 3.45 * self.unit.attack_speed / 100.0 * (self.unit.targets as f64);
        talent_scale *= 2.0;
        }
        let mut active_ratio = ((1) as f64).min((talent_duration/ ((self.unit.attack_interval as f64) / (self.unit.attack_speed+aspd) * 100.0)) as f64);
        let mut arts_dps = ((final_atk * talent_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * talent_scale * 0.05) as f64) * active_ratio * (self.unit.targets as f64);
        dps += arts_dps;
        return dps;
    }
}
