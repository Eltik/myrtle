...
impl Aak {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut cdmg = self.unit.talent1_parameters.iter().cloned().fold(f64::NEG_INFINITY, f64::max);
        let mut crit_rate = 0.25;
        if (self.unit.module_index as f64) == 1.0 && (self.unit.module_level as f64) > 1.0 { crit_rate = if ((self.unit.module_level as f64) as f64) == 2.0 { 0.25 + 0.75 * 0.2 } else { 0.25 + 0.75 * 0.3 }; }
        let mut final_atk = if ((self.unit.skill_index as f64) as f64) == 3.0 { self.unit.atk * (1.0 + self.unit.buff_atk + self.unit.skill_parameters.get(0).copied().unwrap_or(0.0)) + self.unit.buff_atk_flat } else { self.unit.atk * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat };
        let mut aspd = if ((self.unit.skill_index as f64) as f64) == 1.0 { self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) } else { self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) * ((self.unit.skill_index as f64) as f64) / 3.0 };
