...
impl Astesia {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "maxStacks", false, &[], &[], 0, 0), ("module", "blocking", false, &[], &[2], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut aspd = if self.unit.talent_damage { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) * self.unit.talent1_parameters.get(2).copied().unwrap_or(0.0) } else { 0.0 };
        ...
        if (self.unit.skill_index as f64) == 2.0 { dps *= (((self.unit.targets as f64)) as f64).min((2) as f64); }
        return dps;
    }
}
