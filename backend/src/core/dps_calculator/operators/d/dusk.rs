...
impl Dusk {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent2", "+Freeling", false, &[], &[], 2, 0), ("skill", "vsLowHp", false, &[2], &[], 0, 0)];
    ...
    pub fn new(operator_data: OperatorData, params: OperatorParams) -> Self {
        ...
        if unit.module_index == 2 && unit.module_level == 2 { unit.drone_atk += 15.0; }
        if unit.module_index == 2 && unit.module_level == 3 { unit.drone_atk += 25.0; }
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atkbuff = if self.unit.talent_damage && ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) * self.unit.talent1_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        ...
        atk_scale = if self.unit.skill_damage { self.unit.skill_parameters.get(3).copied().unwrap_or(0.0) } else { 1.0 };
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * atk_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * atk_scale * 0.05) as f64);
        dps = hitdmg/(self.unit.attack_interval as f64) * (self.unit.attack_speed + self.unit.skill_parameters.get(1).copied().unwrap_or(0.0))/ 100.0 * (self.unit.targets as f64);
        }
