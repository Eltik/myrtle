...
impl Diamante {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atkbuff = if self.unit.talent_damage && self.unit.skill_damage && ((self.unit.skill_index as f64) as f64) == 2.0 { self.unit.talent1_parameters[0] } else { 0.0 };
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        skill_scale = if self.unit.talent_damage && self.unit.skill_damage && ((self.unit.skill_index as f64) as f64) == 2.0 { self.unit.skill_parameters[1] } else { 0.0 };
        ...
        dps = (hitdmg+eledmg) / (self.unit.attack_interval as f64) * (self.unit.attack_speed + self.unit.skill_parameters[0]) / 100.0 * (((self.unit.targets as f64)) as f64).min((2) as f64);
        }
        if (self.unit.skill_index as f64) == 1.0 {
        atkbuff = self.unit.skill_parameters[0];
        let mut ele_application = self.unit.skill_parameters[1];
