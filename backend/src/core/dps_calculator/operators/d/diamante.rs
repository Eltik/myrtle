...
impl Diamante {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "vsFallout(800dpsNotIncluded)", false, &[], &[], 0, 0), ("trait", "noNecrosis", true, &[], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        burst_scale = if ((self.unit.module_index as f64) as f64) == 1.0 { 1.1 } else { 1.0 };
        if [0.0, 2.0].contains(&(((self.unit.skill_index as f64)) as f64)) {
        atkbuff = if self.unit.talent_damage && self.unit.skill_damage && ((self.unit.skill_index as f64) as f64) == 2.0 { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        skill_scale = if self.unit.talent_damage && self.unit.skill_damage && ((self.unit.skill_index as f64) as f64) == 2.0 { self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        hitdmg = ((final_atk * (1.0 -res/ 100.0)) as f64).max((final_atk * 0.05) as f64);
        eledmg = ((final_atk * 0.0 * (1.0 -res/ 100.0)) as f64).max((final_atk * skill_scale) as f64) /(1.0 +self.unit.buff_fragile) * burst_scale;
        dps = (hitdmg+eledmg) / (self.unit.attack_interval as f64) * (self.unit.attack_speed + self.unit.skill_parameters.get(0).copied().unwrap_or(0.0)) / 100.0 * (((self.unit.targets as f64)) as f64).min((2) as f64);
        }
        ...
        let mut final_atk_necro = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        let mut elemental_health = if !self.unit.talent_damage && !self.unit.skill_damage { 2000.0 } else { 1000.0 };
        let mut time_to_apply_necrosis = elemental_health / (final_atk * ele_application / (self.unit.attack_interval as f64) * (self.unit.attack_speed) / 100.0);
        let mut fallout_dps = 12000.0 / (time_to_apply_necrosis + 15.0) /(1.0 +self.unit.buff_fragile);
        hitdmg = ((final_atk * (1.0 -res/ 100.0)) as f64).max((final_atk * 0.05) as f64);
        let mut hitdmg_necro = ((final_atk_necro * (1.0 -res/ 100.0)) as f64).max((final_atk_necro * 0.05) as f64);
        let mut eledmg_necro = ((final_atk_necro * 0.0 * (1.0 -res/ 100.0)) as f64).max((final_atk_necro * skill_scale) as f64) /(1.0 +self.unit.buff_fragile);
        let mut avg_hitdmg = hitdmg * time_to_apply_necrosis / (time_to_apply_necrosis + 15.0) + hitdmg_necro * 15.0 / (time_to_apply_necrosis + 15.0);
        let mut avg_eledmg = eledmg_necro * 15.0 / (time_to_apply_necrosis + 15.0);
        if !self.unit.trait_damage { dps = (hitdmg) / (self.unit.attack_interval as f64) * (self.unit.attack_speed) / 100.0; }
        else if !self.unit.talent_damage && !self.unit.skill_damage { dps = fallout_dps + (avg_hitdmg + avg_eledmg * burst_scale) / (self.unit.attack_interval as f64) * (self.unit.attack_speed) / 100.0; }
        else if self.unit.talent_damage ^ self.unit.skill_damage { dps = fallout_dps + (avg_hitdmg + avg_eledmg * burst_scale) / (self.unit.attack_interval as f64) * (self.unit.attack_speed) / 100.0; }
        else { dps = (hitdmg_necro + eledmg_necro) * burst_scale / (self.unit.attack_interval as f64) * (self.unit.attack_speed) / 100.0; }
        }
        return dps;
    }
}
