...
impl Durnar {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut final_atk = self.unit.atk * (1.0 + self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) * (((self.unit.skill_index as f64)) as f64).min((1) as f64) + self.unit.buff_atk + self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0)) + self.unit.buff_atk_flat;
        ...
        if (self.unit.skill_index as f64) == 2.0 { dps *= (((self.unit.targets as f64)) as f64).min((3) as f64); }
        return dps;
    }
}
