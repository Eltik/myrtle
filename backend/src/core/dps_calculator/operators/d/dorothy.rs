...
impl Dorothy {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("trait", "noMines", true, &[0], &[], 0, 0), ("talent", "1MinePerSPcost", true, &[], &[], 0, 0), ("talent2", "maxTalent2", false, &[], &[], 0, 0), ("module", "+ExtraMines", false, &[], &[1], 0, 3)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atkbuff = if self.unit.talent2_damage { self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0) * self.unit.talent2_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        cdmg = if ((self.unit.module_index as f64) as f64) == 2.0 { 1.2 } else { 1.0 };
        if (self.unit.module_index as f64) == 1.0 && (self.unit.module_level as f64) == 3.0 && self.unit.module_damage { cdmg = 1.5; }
        sp_cost = (((self.unit.skill_cost as f64) / (1.0 + (self.unit.sp_boost as f64))) as f64).max((5) as f64);
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        let mut mine_scale = if self.unit.trait_damage && ((self.unit.skill_index as f64) as f64) > 0.0 { self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        if (self.unit.skill_index as f64) == 1.0 {
        defshred = 1.0 + self.unit.skill_parameters.get(2).copied().unwrap_or(0.0);
        hitdmgmine = ((final_atk * mine_scale - defense * defshred) as f64).max((final_atk * mine_scale * 0.05) as f64) * cdmg;
        if !self.unit.trait_damage || !self.unit.skill_damage {
        defshred = 1.0;
        } else if !self.unit.talent_damage {
        defshred = 1.0 + 5.0 / sp_cost * self.unit.skill_parameters.get(2).copied().unwrap_or(0.0);
        }
        ...
        hitdmgmine = ((final_atk * mine_scale - defense) as f64).max((final_atk * mine_scale * 0.05) as f64) * cdmg;
        }
        if (self.unit.skill_index as f64) == 3.0 {
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        hitdmgmine = ((final_atk * mine_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * mine_scale * 0.05) as f64) * cdmg;
        }
        let mut minedps = if self.unit.talent_damage { hitdmgmine/ 5.0 } else { hitdmgmine/sp_cost };
        dps = hitdmg/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 + minedps * (self.unit.targets as f64);
        return dps;
    }
}
