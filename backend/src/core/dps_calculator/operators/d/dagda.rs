...
impl Dagda {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "maxStacks", false, &[], &[], 1, 0), ("module", ">50%hp", false, &[], &[1], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut aspd = if ((self.unit.module_index as f64) as f64) == 1.0 && self.unit.module_damage { 10.0 } else { 0.0 };
        let mut crit_rate = 0.3;
        let mut cdmg = if self.unit.talent_damage { self.unit.talent1_parameters.get(2).copied().unwrap_or(0.0) } else { self.unit.talent1_parameters.get(1).copied().unwrap_or(0.0) };
        if (self.unit.elite as f64) == 0.0 { cdmg = 1.0; }
        if (self.unit.skill_index as f64) == 2.0 { crit_rate = self.unit.skill_parameters.get(1).copied().unwrap_or(0.0); }
        let mut hits = if ((self.unit.skill_index as f64) as f64) == 2.0 { 2.0 } else { 1.0 };
        let mut atkbuff = if ((self.unit.skill_index as f64) as f64) == 2.0 { self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        ...
        let mut critdmg = ((final_atk * cdmg - defense) as f64).max((final_atk * cdmg * 0.05) as f64);
        let mut avgdmg = crit_rate * critdmg + (1.0 -crit_rate) * hitdmg;
        let mut dps = hits * avgdmg/(self.unit.attack_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0;
        return dps;
    }
}
