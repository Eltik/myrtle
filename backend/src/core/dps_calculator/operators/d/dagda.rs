...
impl Dagda {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut cdmg = if self.unit.talent_damage { self.unit.talent1_parameters[2] } else { self.unit.talent1_parameters[1] };
        if (self.unit.elite as f64) == 0.0 { cdmg = 1.0; }
        if (self.unit.skill_index as f64) == 2.0 { crit_rate = self.unit.skill_parameters[1]; }
        let mut hits = if ((self.unit.skill_index as f64) as f64) == 2.0 { 2.0 } else { 1.0 };
        let mut atkbuff = if ((self.unit.skill_index as f64) as f64) == 2.0 { self.unit.skill_parameters[0] } else { 0.0 };
