...
impl LapplandAlter {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "noTalentBuffs", true, &[], &[], 1, 0), ("trait", "minDroneDmg", true, &[], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        if (self.unit.skill_index as f64) == 1.0 { drones += 1.0; }
        if (self.unit.skill_index as f64) == 2.0 { drones += 3.0; }
        if (self.unit.skill_index as f64) == 3.0 { drones += 2.0; }
        ...
        dps += (self.unit.targets as f64) * final_atk * self.unit.skill_parameters.get(4).copied().unwrap_or(0.0) * (((1.0 -res/ 100.0)) as f64).max((0.05) as f64);
        }
        return dps;
    }
}
