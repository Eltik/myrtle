...
impl Laios {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "TalentUp", false, &[], &[], 1, 0), ("module", "ModRevive", false, &[], &[2], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut new_defense = if self.unit.talent_damage && ((self.unit.elite as f64) as f64) > 0.0 { defense * (1.0 -self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0)) } else { defense };
        if (self.unit.skill_index as f64) < 2.0 {
        atkbuff = if self.unit.skill_damage && ((self.unit.skill_index as f64) as f64) == 1.0 { self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        ...
        dps = (dps * sp_cost + skilldmg) / (sp_cost + self.unit.skill_parameters.get(1).copied().unwrap_or(0.0));
        }
        return dps;
    }
}
