...
impl Logos {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut bonuschance = if ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent1_parameters[0] } else { 0.0 };
        ...
        let mut shreddmg = if ((self.unit.elite as f64) as f64) == 2.0 { self.unit.talent2_parameters[2] } else { 0.0 };
        ...
        hitdmg = ((final_atk * (1.0 -newres/ 100.0)) as f64).max((final_atk * 0.05) as f64) + ((shreddmg * (1.0 -newres/ 100.0)) as f64).max((shreddmg * 0.05) as f64);
        bonusdmg = (((final_atk * bonusdmg * (1.0 -newres/ 100.0)) as f64).max((final_atk * bonusdmg * 0.05) as f64) + ((shreddmg * (1.0 -newres/ 100.0)) as f64).max((shreddmg * 0.05) as f64)) * bonuschance * bonus_hitcount;
        dps = (hitdmg+bonusdmg)/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 * (((self.unit.targets as f64)) as f64).min((self.unit.skill_parameters[1]) as f64);
        if (self.unit.module_index as f64) == 3.0 && self.unit.talent_damage {
        ele_gauge = if self.unit.module_damage { 1000.0 } else { 2000.0 };
        eledps = dps * 0.08 / (((self.unit.targets as f64)) as f64).min((self.unit.skill_parameters[1]) as f64);
        fallouttime = ele_gauge / eledps;
        dps += 12000.0 /(fallouttime + 15.0) * (((self.unit.targets as f64)) as f64).min((self.unit.skill_parameters[1]) as f64) /(1.0 +self.unit.buff_fragile);
        if (self.unit.module_level as f64) > 1.0 {
        dps += final_atk * falloutdmg/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 * (((self.unit.targets as f64)) as f64).min((self.unit.skill_parameters[1]) as f64) * bonuschance * 15.0 / (fallouttime + 15.0);
        }
        }
        }
        return dps;
