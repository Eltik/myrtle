...
impl Logos {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("skill", "after5sec", false, &[2], &[], 0, 0), ("talent", "withAvgNecrosis", false, &[], &[3], 0, 0), ("talent", "vsBoss", false, &[], &[3], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut bonuschance = if ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        if (self.unit.module_index as f64) == 3.0 { bonuschance += 0.1 * ((self.unit.module_level as f64) - 1.0); }
        bonusdmg = self.unit.talent1_parameters.get(1).copied().unwrap_or(0.0);
        let mut bonus_hitcount = if ((self.unit.module_index as f64) as f64) == 2.0 && ((self.unit.module_level as f64) as f64) > 1.0 { 2.0 } else { 1.0 };
        let mut falloutdmg = if ((self.unit.module_index as f64) as f64) == 3.0 && ((self.unit.module_level as f64) as f64) > 1.0 { 0.2 * ((self.unit.module_level as f64) as f64) /(1.0 +self.unit.buff_fragile) } else { 0.0 };
        newres = if ((self.unit.elite as f64) as f64) == 2.0 { ((0) as f64).max((res-10.0) as f64) } else { res };
        ...
        let mut shreddmg = if ((self.unit.elite as f64) as f64) == 2.0 { self.unit.talent2_parameters.get(2).copied().unwrap_or(0.0) } else { 0.0 };
        ...
        hitdmg = ((final_atk * (1.0 -newres/ 100.0)) as f64).max((final_atk * 0.05) as f64) + ((shreddmg * (1.0 -newres/ 100.0)) as f64).max((shreddmg * 0.05) as f64);
        bonusdmg = (((final_atk * bonusdmg * (1.0 -newres/ 100.0)) as f64).max((final_atk * bonusdmg * 0.05) as f64) + ((shreddmg * (1.0 -newres/ 100.0)) as f64).max((shreddmg * 0.05) as f64)) * bonuschance * bonus_hitcount;
        dps = (hitdmg+bonusdmg)/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0;
        if (self.unit.module_index as f64) == 3.0 && self.unit.talent_damage {
        ele_gauge = if self.unit.module_damage { 1000.0 } else { 2000.0 };
        eledps = dps * 0.08;
        fallouttime = ele_gauge / eledps;
        dps += 12000.0 /(fallouttime + 15.0)/(1.0 +self.unit.buff_fragile);
        if (self.unit.module_level as f64) > 1.0 {
        dps += final_atk * falloutdmg /(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 * bonuschance * 15.0 / (fallouttime + 15.0);
        }
        }
        }
        if (self.unit.skill_index as f64) == 2.0 {
        let mut scaling = self.unit.skill_parameters.get(2).copied().unwrap_or(0.0);
        if self.unit.skill_damage { scaling *= 3.0; }
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * scaling * (1.0 -newres/ 100.0)) as f64).max((final_atk * scaling * 0.05) as f64) + ((shreddmg * (1.0 -newres/ 100.0)) as f64).max((shreddmg * 0.05) as f64);
        bonusdmg = (((final_atk * bonusdmg * (1.0 -newres/ 100.0)) as f64).max((final_atk * bonusdmg * 0.05) as f64) + ((shreddmg * (1.0 -newres/ 100.0)) as f64).max((shreddmg * 0.05) as f64)) * bonuschance * bonus_hitcount;
        ...
        hitdmg = ((final_atk * (1.0 -newres/ 100.0)) as f64).max((final_atk * 0.05) as f64) + ((shreddmg * (1.0 -newres/ 100.0)) as f64).max((shreddmg * 0.05) as f64);
        bonusdmg = (((final_atk * bonusdmg * (1.0 -newres/ 100.0)) as f64).max((final_atk * bonusdmg * 0.05) as f64) + ((shreddmg * (1.0 -newres/ 100.0)) as f64).max((shreddmg * 0.05) as f64)) * bonuschance * bonus_hitcount;
        dps = (hitdmg+bonusdmg)/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 * (((self.unit.targets as f64)) as f64).min((self.unit.skill_parameters.get(1).copied().unwrap_or(0.0)) as f64);
        if (self.unit.module_index as f64) == 3.0 && self.unit.talent_damage {
        ele_gauge = if self.unit.module_damage { 1000.0 } else { 2000.0 };
        eledps = dps * 0.08 / (((self.unit.targets as f64)) as f64).min((self.unit.skill_parameters.get(1).copied().unwrap_or(0.0)) as f64);
        fallouttime = ele_gauge / eledps;
        dps += 12000.0 /(fallouttime + 15.0) * (((self.unit.targets as f64)) as f64).min((self.unit.skill_parameters.get(1).copied().unwrap_or(0.0)) as f64) /(1.0 +self.unit.buff_fragile);
        if (self.unit.module_level as f64) > 1.0 {
        dps += final_atk * falloutdmg/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 * (((self.unit.targets as f64)) as f64).min((self.unit.skill_parameters.get(1).copied().unwrap_or(0.0)) as f64) * bonuschance * 15.0 / (fallouttime + 15.0);
        }
        }
        }
        return dps;
