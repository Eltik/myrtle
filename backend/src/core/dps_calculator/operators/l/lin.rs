...
impl Lin {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("module", "manyTargets", false, &[], &[2], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        if (self.unit.skill_index as f64) == 0.0 { return res * 0.0; }
        ...
        dps = hitdmgarts/atk_interval * (self.unit.attack_speed+aspd)/ 100.0 * (self.unit.targets as f64);
        } else {
        if (self.unit.skill_index as f64) == 1.0 { atk_interval = 3.0; }
        ...
        hitdmgarts = ((final_atk * (1.0 -res/ 100.0)) as f64).max((final_atk * 0.05) as f64);
        dps = hitdmgarts/atk_interval * self.unit.attack_speed/ 100.0 * (self.unit.targets as f64);
        }
        if (self.unit.module_index as f64) == 2.0 && self.unit.module_damage { dps *= 1.15; }
        return dps;
    }
}
