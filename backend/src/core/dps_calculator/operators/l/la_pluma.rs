...
impl LaPluma {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "maxStacks", false, &[], &[], 0, 0), ("skill", "lowHpTarget", false, &[2], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        if self.unit.talent_damage && (self.unit.module_index as f64) == 1.0 && (self.unit.module_level as f64) > 1.0 { atkbuff = self.unit.talent1_parameters.get(2).copied().unwrap_or(0.0); }
        ...
        dps = avgphys/(self.unit.attack_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0 * (self.unit.targets as f64);
        }
        if (self.unit.skill_index as f64) == 2.0 {
        atk_interval = (self.unit.attack_interval as f64) * (1.0 + self.unit.skill_parameters.get(3).copied().unwrap_or(0.0));
