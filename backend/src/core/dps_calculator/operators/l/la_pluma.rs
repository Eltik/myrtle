...
impl LaPluma {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atkbuff = 0.0;
        aspd = if self.unit.talent_damage { self.unit.talent1_parameters[0] * self.unit.talent1_parameters[1] } else { 0.0 };
        if self.unit.talent_damage && (self.unit.module_index as f64) == 1.0 && (self.unit.module_level as f64) > 1.0 { atkbuff = self.unit.talent1_parameters[2]; }
        ...
        if (self.unit.skill_index as f64) == 2.0 {
        atk_interval = (self.unit.attack_interval as f64) * (1.0 + self.unit.skill_parameters[3]);
