...
impl Leto {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("trait", "rangedAtk", true, &[2], &[], 0, 0), ("module", "+12aspd(mod)", false, &[], &[2], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut atk_scale = if ((self.unit.skill_index as f64) as f64) < 2.0 && !self.unit.trait_damage { 0.8 } else { 1.0 };
        let mut aspd = if ((self.unit.module_index as f64) as f64) == 2.0 && ((self.unit.targets as f64) > 1.0 || self.unit.module_damage) { 12.0 } else { 0.0 };
        aspd += self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0);
        let mut final_atk = self.unit.atk * (1.0 + self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) * (((self.unit.skill_index as f64)) as f64).min((1) as f64) + self.unit.buff_atk) + self.unit.buff_atk_flat;
        if (self.unit.skill_index as f64) == 1.0 { aspd += self.unit.skill_parameters.get(1).copied().unwrap_or(0.0); }
        ...
        if (self.unit.skill_index as f64) == 2.0 && (self.unit.targets as f64) > 1.0 { dps *= 2.0; }
        return dps;
    }
}
