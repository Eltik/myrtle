...
impl Lemuen {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "vsMarked", false, &[], &[], 1, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atkbuff = if self.unit.talent2_damage && ((self.unit.elite as f64) as f64) > 1.0 { self.unit.talent2_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        let mut dmg = if self.unit.talent_damage && ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent1_parameters.iter().cloned().fold(f64::INFINITY, f64::min) } else { 1.0 };
        if (self.unit.skill_index as f64) < 2.0 {
        atk_scale = if ((self.unit.skill_index as f64) as f64) == 1.0 { self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) } else { 1.0 };
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + atkbuff) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * atk_scale - defense) as f64).max((final_atk * atk_scale * 0.05) as f64) * (((self.unit.targets as f64)) as f64).min((1.0 + (self.unit.skill_index as f64)) as f64) * dmg;
        ...
        if self.unit.talent_damage { hitdmg = ((final_atk * atk_scale - defense) as f64).max((final_atk * atk_scale * 0.05) as f64) * dmg; }
        ...
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + atkbuff) + self.unit.buff_atk_flat;
        let mut ammo = if self.unit.talent2_damage { 5.0 + self.unit.talent2_parameters.get(2).copied().unwrap_or(0.0) } else { 5.0 };
        let mut centralhit_dmg = ((final_atk * self.unit.skill_parameters.get(4).copied().unwrap_or(0.0) - defense) as f64).max((final_atk * self.unit.skill_parameters.get(4).copied().unwrap_or(0.0) * 0.05) as f64) * dmg;
        let mut outerhit_dmg = ((final_atk * self.unit.skill_parameters.get(6).copied().unwrap_or(0.0) - defense) as f64).max((final_atk * self.unit.skill_parameters.get(6).copied().unwrap_or(0.0) * 0.05) as f64) * dmg;
    ...
    pub fn total_dmg(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut extra_ammo = if ((self.unit.elite as f64) as f64) > 1.0 && self.unit.talent2_damage { self.unit.talent2_parameters.get(2).copied().unwrap_or(0.0) } else { 0.0 };
