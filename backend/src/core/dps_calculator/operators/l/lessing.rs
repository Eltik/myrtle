...
impl Lessing {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent2", "w/o talent2", true, &[], &[], 2, 0), ("talent", "vsBlocked", true, &[3], &[2], 0, 2), ("module", "vsBlocked", false, &[3], &[1], 0, 0), ("module", "afterRevive", false, &[], &[2], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atkbuff = if self.unit.talent2_damage { self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        aspd = if ((self.unit.module_index as f64) as f64) == 2.0 && self.unit.module_damage { 30.0 } else { 0.0 };
        let mut newdef = if ((self.unit.module_index as f64) as f64) == 2.0 && ((self.unit.module_level as f64) as f64) > 1.0 && self.unit.talent_damage { defense * (1.0 - 0.04 * ((self.unit.module_level as f64) as f64)) } else { defense };
        ...
        if (self.unit.skill_index as f64) == 2.0 {
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * atk_scale - newdef) as f64).max((final_atk * atk_scale * 0.05) as f64);
        dps = 2.0 * hitdmg/(self.unit.attack_interval as f64) * (self.unit.attack_speed + aspd)/ 100.0;
        }
        if (self.unit.skill_index as f64) == 3.0 {
        if self.unit.skill_damage { atk_scale *= self.unit.skill_parameters.get(1).copied().unwrap_or(0.0); }
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * atk_scale - newdef) as f64).max((final_atk * atk_scale * 0.05) as f64);
