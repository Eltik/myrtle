...
impl Lee {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "blocking", false, &[], &[], 1, 0), ("module", "5modStacks", false, &[], &[2], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut aspd = if self.unit.talent_damage && ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent1_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        if (self.unit.targets as f64) == 1.0 && self.unit.talent_damage { aspd *= 2.0; }
        let mut atkbuff = if ((self.unit.module_index as f64) as f64) == 2.0 && self.unit.module_damage { 0.2 } else { 0.0 };
        if (self.unit.skill_index as f64) == 2.0 { aspd += self.unit.skill_parameters.get(5).copied().unwrap_or(0.0); }
        else { atkbuff += self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) * (((self.unit.skill_index as f64)) as f64).min((1) as f64); }
