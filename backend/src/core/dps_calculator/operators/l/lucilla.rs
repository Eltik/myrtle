...
/// Lucilla operator implementation
pub struct Lucilla {
...
impl Lucilla {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "vsTrash", false, &[], &[], 1, 0), ("skill", "(maxMultiplier)", false, &[2], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut fragile = if ((self.unit.elite as f64) as f64) > 0.0 && self.unit.talent_damage { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) - 1.0 } else { 0.0 };
        if (self.unit.skill_index as f64) == 2.0 && self.unit.skill_damage { fragile *= self.unit.skill_parameters.get(3).copied().unwrap_or(0.0); }
        ...
        skilldmg = ((final_atk * skill_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * skill_scale * 0.05) as f64) * (((self.unit.targets as f64)) as f64).min((self.unit.skill_parameters.get(1).copied().unwrap_or(0.0)) as f64);
        if (self.unit.skill_index as f64) == 0.0 { skilldmg = hitdmg; }
        avghit = (hitdmg * (self.unit.skill_cost as f64) + skilldmg)/((self.unit.skill_cost as f64) + 1.0);
        ...
        hitdmg = ((final_atk * (1.0 -res/ 100.0)) as f64).max((final_atk * 0.05) as f64);
        dps = hitdmg / (self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 * (((self.unit.targets as f64)) as f64).min((2) as f64);
        }
        return dps*(1.0 +fragile)/(1.0 +self.unit.buff_fragile);
    }
}

impl std::ops::Deref for Lucilla {
...
impl std::ops::DerefMut for Lucilla {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.unit
    }
}

impl DpsCalculator for Lucilla {
