...
impl LuoXiaohei {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("trait", "rangedAtk", true, &[0], &[], 0, 0), ("module", "+12aspd(mod)", false, &[], &[2], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        below50 = false;
        if self.unit.skill_index == 2 && self.unit.skill_damage { below50 = true; }
        if self.unit.module_index == 2 && self.unit.module_level > 1 && self.unit.talent_damage { below50 = true; }
        ...
        let mut dmg_scale = if below50 { 1.0 + 0.04 * ((self.unit.module_level as f64) as f64) } else { 1.0 };
        ...
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64) * dmg_scale;
        let mut newdef = ((defense - self.unit.skill_parameters.get(2).copied().unwrap_or(0.0)) as f64).max((0) as f64);
