...
impl Goldenglow {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        newres = ((res-self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0)) as f64).max((0) as f64);
        let mut drone_dmg = if ((self.unit.module_index as f64) as f64) == 2.0 { 1.2 } else { 1.1 };
        let mut drone_explosion = if ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent1_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        let mut explosion_prob = if ((self.unit.elite as f64) as f64) > 0.0 { 0.1 } else { 0.0 };
        aspd = 0.0;
        let mut drones = 2.0;
        ...
        let mut dmgperinterval = final_atk*(3.0 -drones) + drones * drone_atk * (1.0 -explosion_prob) + drones * drone_explosion * explosion_prob;
        if (self.unit.skill_index as f64) == 0.0 { dmgperinterval = final_atk + drone_atk; }
        hitdmgarts = ((dmgperinterval *(1.0 -newres/ 100.0)) as f64).max((dmgperinterval * 0.05) as f64);
...
impl DpsCalculator for Goldenglow {
