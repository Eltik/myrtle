...
impl Gracebearer {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "+talent", false, &[], &[], 1, 0), ("skill", "avgNervous", false, &[1], &[], 0, 0), ("trait", "(vsBoss)", true, &[], &[], 0, 0), ("skill", "vsFallout", false, &[2], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atkbuff = if self.unit.talent_damage { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) + self.unit.talent1_parameters.get(1).copied().unwrap_or(0.0) } else { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) };
        }
        ...
        ele_gauge = if self.unit.trait_damage { 1000.0 } else { 2000.0 };
        time_to_fallout = ele_gauge/(0.1 * dps);
        ...
        skilldmg = if !self.unit.skill_damage { ((final_atk * skill_scale - defense) as f64).max((final_atk * skill_scale * 0.05) as f64) } else { final_atk * skill_scale };
        dps += 3.0 * skilldmg/(((self.unit.skill_cost as f64) + 1.2)/(1.0 +(self.unit.sp_boost as f64))) * (((self.unit.targets as f64)) as f64).min((self.unit.skill_parameters.get(1).copied().unwrap_or(0.0)) as f64);
        }
        return dps;
    }
}
...
impl DpsCalculator for Gracebearer {
