...
impl Gnosis {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("skill", "vsFrozen", false, &[], &[], 0, 0), ("module", "vsElite", false, &[], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut coldfragile = if ((self.unit.elite as f64) as f64) > 0.0 { 0.5 * (self.unit.talent1_parameters.iter().cloned().fold(f64::NEG_INFINITY, f64::max) - 1.0) } else { 0.0 };
        ...
        atkbuff = if ((self.unit.module_index as f64) as f64) == 2.0 && ((self.unit.module_level as f64) as f64) > 1.0 { 0.05 * ((self.unit.module_level as f64) as f64) } else { 0.0 };
        let mut extra_sp = if ((self.unit.module_index as f64) as f64) == 2.0 && ((self.unit.skill_index as f64) as f64) == 1.0 && self.unit.module_damage { 0.25 } else { 0.0 };
        // ###the actual skills
        if (self.unit.skill_index as f64) < 2.0 {
        skill_scale = self.unit.skill_parameters.get(0).copied().unwrap_or(0.0);
        sp_cost = (self.unit.skill_cost as f64)/(1.0 + (self.unit.sp_boost as f64) + extra_sp) + 1.2;
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + atkbuff) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * (1.0 -res/ 100.0)) as f64).max((final_atk * 0.05) as f64)*(1.0 +coldfragile)/(1.0 +self.unit.buff_fragile);
        let mut skilldmg1 = ((final_atk * skill_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * skill_scale * 0.05) as f64)*(1.0 +coldfragile)/(1.0 +self.unit.buff_fragile);
        let mut skilldmg2 = ((final_atk * skill_scale * (1.0 -frozenres/ 100.0)) as f64).max((final_atk * skill_scale * 0.05) as f64)*(1.0 +frozenfragile)/(1.0 +self.unit.buff_fragile);
        skilldmg = skilldmg1 + skilldmg2;
        ...
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + atkbuff) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * (1.0 -res/ 100.0)) as f64).max((final_atk * 0.05) as f64)*(1.0 +coldfragile)/(1.0 +self.unit.buff_fragile);
        if self.unit.skill_damage { hitdmg = ((final_atk * (1.0 -frozenres/ 100.0)) as f64).max((final_atk * 0.05) as f64)*(1.0 +frozenfragile)/(1.0 +self.unit.buff_fragile); }
        dps = hitdmg/((self.unit.attack_interval as f64)/((self.unit.attack_speed + aspd)/ 100.0)) * ((2) as f64).min(((self.unit.targets as f64)) as f64);
        }
        return dps;
    }
}
