...
impl Hoolheyak {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "vsAerial", false, &[], &[], 0, 0), ("skill", "maxRange", false, &[], &[], 0, 0), ("talent2", "vsLowHp", false, &[], &[2], 0, 2), ("module", "vsElite", false, &[1], &[2], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        newres = if [1.0, 3.0].contains(&((((self.unit.module_index as f64) as f64)) as f64)) { ((res-10.0) as f64).max((0) as f64) } else { res };
        ...
        if (self.unit.module_index as f64) == 2.0 && self.unit.module_damage { sp_cost = (self.unit.skill_cost as f64)/(1.0 + (self.unit.sp_boost as f64) + 1.0/(self.unit.attack_interval as f64)*self.unit.attack_speed/ 100.0) + 1.2; }
        hitdmgarts = ((final_atk * atk_scale * (1.0 -newres/ 100.0)) as f64).max((final_atk * atk_scale * 0.05) as f64) * dmg_scale;
        skilldmg = ((final_atk * atk_scale * skill_scale * (1.0 -newres/ 100.0)) as f64).max((final_atk * atk_scale * skill_scale * 0.05) as f64) * dmg_scale;
        ...
        avghit = (skilldmg * ((2) as f64).min(((self.unit.targets as f64)) as f64) + (atks_per_skillactivation - 1.0) * hitdmgarts) / atks_per_skillactivation;
        } else {
        avghit = (skilldmg * ((2) as f64).min(((self.unit.targets as f64)) as f64) + ((atks_per_skillactivation) as f64).trunc() * hitdmgarts) / (((atks_per_skillactivation) as f64).trunc()+1.0);
        }
        }
        ...
        hitdmgarts = ((final_atk * atk_scale * skill_scale * (1.0 -newres/ 100.0)) as f64).max((final_atk * atk_scale * skill_scale * 0.05) as f64) * dmg_scale;
        dps = 9.0 * hitdmgarts/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0;
        }
        if (self.unit.skill_index as f64) == 3.0 {
        skill_scale = if self.unit.skill_damage { self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) } else { self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) };
        hitdmgarts = ((final_atk * atk_scale * skill_scale * (1.0 -newres/ 100.0)) as f64).max((final_atk * atk_scale * skill_scale * 0.05) as f64) * dmg_scale;
        dps = hitdmgarts/ 3.0 * self.unit.attack_speed/ 100.0 * (((self.unit.targets as f64)) as f64).min((3) as f64);
        }
        return dps;
    }
}
