...
impl Horn {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent2", "afterRevive", false, &[], &[], 2, 0), ("skill", "overdrive", false, &[1], &[], 0, 0), ("module", "blockedTarget", false, &[], &[1], 0, 0), ("module", "rangedAtk", false, &[], &[2], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        aspd = if self.unit.talent2_damage { self.unit.talent2_parameters.get(2).copied().unwrap_or(0.0) } else { 0.0 };
        if (self.unit.module_index as f64) == 2.0 && self.unit.module_damage { aspd += 10.0; }
        if (self.unit.module_index as f64) == 2.0 && (self.unit.module_level as f64) > 1.0 {
        if (self.unit.module_level as f64) == 2.0 { aspd += 5.0; }
        if (self.unit.module_level as f64) == 3.0 { aspd += 8.0; }
        }
        ...
        dps = avghit/(self.unit.attack_interval as f64)*(self.unit.attack_speed+aspd)/ 100.0 * (self.unit.targets as f64);
        }
        if (self.unit.skill_index as f64) == 2.0 {
        ...
        artsdmg = ((final_atk * atk_scale * arts_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * atk_scale * arts_scale * 0.05) as f64);
        dps = (hitdmg+artsdmg)/(self.unit.attack_interval as f64)*(self.unit.attack_speed+aspd)/ 100.0 * (self.unit.targets as f64);
        }
        if (self.unit.skill_index as f64) == 3.0 {
        atk_interval = (self.unit.attack_interval as f64) + self.unit.skill_parameters.get(1).copied().unwrap_or(0.0);
        atkbuff += self.unit.skill_parameters.get(0).copied().unwrap_or(0.0);
        if self.unit.skill_damage { atkbuff += self.unit.skill_parameters.get(0).copied().unwrap_or(0.0); }
        final_atk = self.unit.atk * (1.0 + atkbuff+ self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * atk_scale - defense) as f64).max((final_atk * atk_scale * 0.05) as f64);
        dps = hitdmg/atk_interval*(self.unit.attack_speed+aspd)/ 100.0 * (self.unit.targets as f64);
        }
        return dps;
    }
}
