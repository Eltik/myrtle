...
impl Hibiscus {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        hitdmg = ((final_atk * (1.0 -res/ 100.0)) as f64).max((final_atk * 0.05) as f64) * dmg;
        dps = hitdmg/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0;
        }
        if (self.unit.skill_index as f64) == 2.0 {
        let mut scale = self.unit.skill_parameters.get(0).copied().unwrap_or(0.0);
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * scale * (1.0 -res/ 100.0)) as f64).max((final_atk * scale * 0.05) as f64) * dmg;
...
impl DpsCalculator for Hibiscus {
