...
impl HoshigumaAlter {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atkbuff = if self.unit.talent2_damage && self.unit.talent_damage && ((self.unit.elite as f64) as f64) == 2.0 { self.unit.talent2_parameters[2] } else { 0.0 };
        // UNTRANSLATED: if self.module == 1 and self.module_lvl > 1 and self.talent2_dmg and self.talent_dmg: atkbuff += 0.05 * self.module_lvl
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        dps = hitdmg/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0;
        if (self.unit.skill_index as f64) == 1.0 {
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk + self.unit.skill_parameters[0]) + self.unit.buff_atk_flat;
        ...
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk + self.unit.skill_parameters[1]) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * (1.0 + extra_scale) * (1.0 -res/ 100.0)) as f64).max((final_atk * (1.0 + extra_scale) * 0.05) as f64);
        let mut hits = if self.unit.skill_damage { 4.0 } else { 3.0 };
        dps = hits * hitdmg/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 * ((self.unit.skill_parameters[2]) as f64).min(((self.unit.targets as f64)) as f64);
        }
        return dps;
    }
