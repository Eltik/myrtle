...
impl HoshigumaAlter {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent2", "maxTenacity", false, &[], &[], 2, 0), ("skill", "lastStand", false, &[3], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atkbuff = if self.unit.talent2_damage && self.unit.talent_damage && ((self.unit.elite as f64) as f64) == 2.0 { self.unit.talent2_parameters.get(2).copied().unwrap_or(0.0) } else { 0.0 };
        if (self.unit.module_index as f64) == 1.0 && (self.unit.module_level as f64) > 1.0 && self.unit.talent2_damage && self.unit.talent_damage { atkbuff += 0.05 * (self.unit.module_level as f64); }
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        dps = hitdmg/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0;
        if (self.unit.skill_index as f64) == 1.0 {
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk + self.unit.skill_parameters.get(0).copied().unwrap_or(0.0)) + self.unit.buff_atk_flat;
        ...
        let mut reflectdmg = ((final_atk * skill_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * skill_scale * 0.05) as f64);
        dps += reflectdmg * 0.0 /* self.hits - defaults to 0 */;
        }
        if (self.unit.skill_index as f64) == 3.0 {
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk + self.unit.skill_parameters.get(1).copied().unwrap_or(0.0)) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * (1.0 + extra_scale) * (1.0 -res/ 100.0)) as f64).max((final_atk * (1.0 + extra_scale) * 0.05) as f64);
        let mut hits = if self.unit.skill_damage { 4.0 } else { 3.0 };
        dps = hits * hitdmg/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 * ((self.unit.skill_parameters.get(2).copied().unwrap_or(0.0)) as f64).min(((self.unit.targets as f64)) as f64);
        }
        return dps;
    }
    ...
    pub fn total_dmg(&self, enemy: &EnemyStats) -> f64 {
        ...
        if (self.unit.skill_index as f64) == 3.0 && self.unit.skill_damage { skill_duration = 11.0; }
