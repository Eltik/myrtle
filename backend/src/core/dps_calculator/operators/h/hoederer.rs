...
impl Hoederer {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "vsStun/Bind", false, &[], &[], 1, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        dps = avgphys/atk_interval * self.unit.attack_speed/ 100.0 * (((self.unit.targets as f64)) as f64).min((2) as f64) * dmg_bonus;
        }
        if (self.unit.skill_index as f64) == 2.0 {
        let mut maxtargets = if self.unit.skill_damage { 3.0 } else { 2.0 };
        if self.unit.skill_damage { atk_interval = 3.0; }
        ...
        hitdmg = ((final_atk * atk_scale - defense) as f64).max((final_atk * atk_scale * 0.05) as f64);
        dps = hitdmg/atk_interval * self.unit.attack_speed/ 100.0 * (((self.unit.targets as f64)) as f64).min((maxtargets) as f64) * dmg_bonus;
        }
        ...
        atk_cycle = atk_interval / self.unit.attack_speed * 100.0;
        ...
        dps = hitdmg/atk_interval * self.unit.attack_speed/ 100.0 * dmg_bonus  + 200.0;
...
impl DpsCalculator for Hoederer {
