...
impl Hoshiguma {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("module", "afterDodge", false, &[], &[2], 0, 2)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atkbuff = if ((self.unit.module_index as f64) as f64) == 2.0 && ((self.unit.module_level as f64) as f64) > 1.0 && self.unit.module_damage { self.unit.talent1_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        let mut targets = if ((self.unit.skill_index as f64) as f64) == 3.0 { (self.unit.targets as f64) } else { 1.0 };
        if (self.unit.skill_index as f64) == 3.0 { atkbuff += self.unit.skill_parameters.get(0).copied().unwrap_or(0.0); }
        final_atk = self.unit.atk * (1.0 +atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        dps = hitdmg/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 * targets;
        ...
        let mut reflectdmg = ((final_atk * skill_scale - defense) as f64).max((final_atk * skill_scale * 0.05) as f64);
