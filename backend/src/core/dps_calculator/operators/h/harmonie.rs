...
impl Harmonie {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "vsBlocked", false, &[2], &[], 1, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atk_scale = if ((self.unit.elite as f64) as f64) > 0.0 && self.unit.talent_damage || ((self.unit.skill_index as f64) as f64) == 2.0 { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) } else { 1.0 };
        if (self.unit.skill_index as f64) < 2.0 {
        atk_interval = if ((self.unit.skill_index as f64) as f64) == 1.0 { (self.unit.attack_interval as f64)/ 5.0 } else { (self.unit.attack_interval as f64) };
        ...
        hitdmg = ((final_atk * skill_scale * atk_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * skill_scale * atk_scale * 0.05) as f64);
        ...
        let mut extra_dps = ((self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) * (1.0 -res/ 100.0)) as f64).max((self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) * 0.05) as f64) * (self.unit.targets as f64);
