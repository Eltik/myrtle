...
impl Indra {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("module", ">50% HP", false, &[], &[1], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        aspd = if self.unit.module_damage && ((self.unit.module_index as f64) as f64) == 1.0 { 10.0 } else { 0.0 };
        if (self.unit.skill_index as f64) < 2.0 {
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        let mut newdef = defense * (1.0 - self.unit.skill_parameters.get(1).copied().unwrap_or(0.0));
        if (self.unit.skill_index as f64) == 1.0 { final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + self.unit.skill_parameters.get(0).copied().unwrap_or(0.0)) + self.unit.buff_atk_flat; }
        skilldmg = ((final_atk - newdef) as f64).max((final_atk * 0.05) as f64);
        dps = 0.2*(4.0 *hitdmg + skilldmg)/(self.unit.attack_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0;
        }
        if (self.unit.skill_index as f64) == 2.0 {
