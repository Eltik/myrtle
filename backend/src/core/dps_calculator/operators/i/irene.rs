...
impl Irene {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "vsLevitateImmune", true, &[], &[], 0, 0), ("talent2", "vsSeaborn", false, &[], &[2], 0, 2), ("skill", "vsHeavy", true, &[3], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atkbuff = if ((self.unit.module_index as f64) as f64) == 2.0 && ((self.unit.module_level as f64) as f64) > 1.0 { self.unit.talent2_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        if self.unit.talent2_damage {
        atkbuff *= 2.0;
        aspd *= 2.0;
        }
        let mut skill_dmg = if ((self.unit.module_index as f64) as f64) == 1.0 { 1.1 } else { 1.0 };
        let mut newdef1 = if ((self.unit.module_index as f64) as f64) != 2.0 { defense } else { ((0) as f64).max((defense -70.0) as f64) };
        defshred = 0.0;
        if (self.unit.elite as f64) > 0.0 {
        defshred = self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0);
        }
        ...
        skill_dmg = ((final_atk * skill_scale - newdef2) as f64).max((final_atk * skill_scale * 0.05) as f64) * skill_dmg;
        if (self.unit.skill_index as f64) == 0.0 { skill_dmg = (hitdmg1+hitdmg2)/ 2.0; }
        ...
        let mut skill_scale1 = self.unit.skill_parameters.get(0).copied().unwrap_or(0.0);
        ...
        final_atk = self.unit.atk * (1.0 +atkbuff+ self.unit.buff_atk) + self.unit.buff_atk_flat;
        let mut initialhit1 = ((final_atk * skill_scale1 - newdef1) as f64).max((final_atk *skill_scale1 * 0.05) as f64)*skill_dmg;
        let mut initialhit2 = ((final_atk * skill_scale1 - newdef2) as f64).max((final_atk * skill_scale1 * 0.05) as f64)*skill_dmg;
        hitdmg1 = ((final_atk * skill_scale - newdef1) as f64).max((final_atk *skill_scale * 0.05) as f64)*skill_dmg;
        hitdmg2 = ((final_atk * skill_scale - newdef2) as f64).max((final_atk *skill_scale * 0.05) as f64)*skill_dmg;
        dps = 0.5*initialhit1 + 0.5* initialhit2;
        levduration = self.unit.skill_parameters.get(1).copied().unwrap_or(0.0);
        if !self.unit.talent_damage { return (dps + hits * (0.5*hitdmg1+0.5*hitdmg2)); }
