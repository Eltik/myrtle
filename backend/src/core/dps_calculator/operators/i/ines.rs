...
impl Ines {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("skill", "maxSteal", false, &[], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut stolen_atk = if ((self.unit.elite as f64) as f64) < 1.0 { 0.0 } else { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) };
        if (self.unit.skill_index as f64) == 1.0 {
        skill_scale = self.unit.skill_parameters.get(1).copied().unwrap_or(0.0);
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat + stolen_atk;
        ...
        dps = hitdmg/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 + skillhitdmg * ((1) as f64).min((3.0 / ((sp_cost+1.0) * (self.unit.attack_interval as f64)/self.unit.attack_speed*100.0)) as f64);
        }
        if (self.unit.skill_index as f64) == 2.0 {
        atkbuff = self.unit.skill_parameters.get(1).copied().unwrap_or(0.0);
        aspd = if self.unit.skill_damage { self.unit.skill_parameters.get(3).copied().unwrap_or(0.0) } else { self.unit.skill_parameters.get(2).copied().unwrap_or(0.0) };
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + atkbuff) + self.unit.buff_atk_flat + stolen_atk;
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        ...
        atkbuff = if ((self.unit.skill_index as f64) as f64) == 3.0 { self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + atkbuff) + self.unit.buff_atk_flat + stolen_atk;
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
