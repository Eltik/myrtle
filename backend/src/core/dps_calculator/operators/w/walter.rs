...
impl Walter {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("skill", "overdrive", false, &[2], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut bonushits = if ((self.unit.module_index as f64) as f64) == 1.0 { 2.0 } else { 1.0 };
        let mut maintargetscale = if ((self.unit.elite as f64) as f64) == 0.0 { 1.0 } else { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) };
        let mut explosionscale = if ((self.unit.elite as f64) as f64) == 0.0 { 0.0 } else { self.unit.talent1_parameters.get(2).copied().unwrap_or(0.0) };
        let mut prob = 1.0 - (0.85 as f64).powf(bonushits as f64);
        if (self.unit.skill_index as f64) == 0.0 {
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg_main = ((final_atk * maintargetscale - defense) as f64).max((final_atk * maintargetscale * 0.05) as f64);
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        explosiondmg = ((final_atk * explosionscale - defense) as f64).max((final_atk * explosionscale * 0.05) as f64);
        avghit = hitdmg_main + hitdmg + explosiondmg * prob;
        ...
        hitdmg_main = ((final_atk * maintargetscale - defense) as f64).max((final_atk * maintargetscale * 0.05) as f64);
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        bonushitdmg_main = ((final_atk * maintargetscale * 0.5 - defense) as f64).max((final_atk * maintargetscale * 0.5 * 0.05) as f64);
        bonushitdmg = ((final_atk * 0.5 - defense) as f64).max((final_atk  * 0.5 * 0.05) as f64);
        let mut skillhitdmg_main = ((final_atk * maintargetscale * skill_scale - defense) as f64).max((final_atk * maintargetscale * skill_scale * 0.05) as f64);
        ...
        let mut avghit_main = (sp_cost * (hitdmg_main + bonushitdmg_main * bonushits) + hitdmg_main + (bonushits+2.0)*skillhitdmg_main) / (sp_cost + 1.0);
        avghit = (sp_cost * (hitdmg + bonushitdmg * bonushits) + hitdmg + (bonushits+2.0)*skillhitdmg) / (sp_cost + 1.0);
        let mut avg_explosion = (sp_cost * explosiondmg * prob + explosiondmg * prob2) / (sp_cost + 1.0);
        dps = (avghit_main+avg_explosion)/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0;
        if (self.unit.targets as f64) > 1.0 {
        dps += (avghit+avg_explosion)/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 * ((self.unit.targets as f64) - 1.0);
        }
        }
        ...
        atk_scale = if self.unit.skill_damage { self.unit.skill_parameters.get(2).copied().unwrap_or(0.0) } else { 1.0 };
        hitdmg_main = ((final_atk * maintargetscale * atk_scale - defense) as f64).max((final_atk * maintargetscale * atk_scale * 0.05) as f64);
        // hitdmg = np.fmax(final_atk * atk_scale - defense, final_atk * atk_scale * 0.05)
        bonushitdmg_main = ((final_atk * maintargetscale * atk_scale * 0.5 - defense) as f64).max((final_atk * maintargetscale * atk_scale * 0.05) as f64);
        // bonushitdmg = np.fmax(final_atk * atk_scale * 0.5 - defense, final_atk * atk_scale * 0.05)
        explosiondmg = ((final_atk * explosionscale - defense) as f64).max((final_atk * explosionscale * 0.05) as f64);
        dps = (hitdmg_main + bonushitdmg_main * bonushits + prob * explosiondmg)/atk_interval * self.unit.attack_speed/ 100.0;
        if self.unit.skill_damage { dps *= 4.0; }
        else if (self.unit.targets as f64) > 1.0 { dps *= ((3) as f64).min(((self.unit.targets as f64)) as f64); }
        }
        ...
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + atkbuff) + self.unit.buff_atk_flat;
        hitdmg_main = ((final_atk * maintargetscale * skill_scale - defense) as f64).max((final_atk * maintargetscale * skill_scale * 0.05) as f64);
        hitdmg = ((final_atk * skill_scale - defense) as f64).max((final_atk * skill_scale * 0.05) as f64);
        bonushitdmg_main = ((final_atk * maintargetscale * skill_scale * 0.5 - defense) as f64).max((final_atk * skill_scale * maintargetscale * 0.5 * 0.05) as f64);
        bonushitdmg = ((final_atk * 0.5 - defense) as f64).max((final_atk * 0.5 * 0.05) as f64);
        explosiondmg = ((final_atk * explosionscale - defense) as f64).max((final_atk * explosionscale * 0.05) as f64);
        dps = (hitdmg_main + bonushitdmg_main * bonushits + explosiondmg)/ 5.0 * self.unit.attack_speed/ 100.0;
        if (self.unit.targets as f64) > 1.0 {
        dps += (hitdmg + bonushitdmg * bonushits + explosiondmg)/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 * ((self.unit.targets as f64)-1.0);
        }
        }
        let mut shadowhit = ((self.unit.drone_atk * (1.0 -res/ 100.0)) as f64).max((self.unit.drone_atk * 0.05) as f64) * shadows;
        dps += shadowhit/4.25;
        return dps;
    }
