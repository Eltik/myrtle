...
impl Warmy {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("trait", "no Burn", true, &[], &[], 0, 0), ("skill", "avgBurn", false, &[], &[], 0, 0), ("skill", "vsBurn", false, &[2], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        burst_scale = if ((((self.unit.skill_index as f64) as f64) == 2.0 && self.unit.skill_damage) || (((self.unit.skill_index as f64) as f64) == 1.0 && self.unit.trait_damage)) && ((self.unit.module_index as f64) as f64) == 1.0 { 1.1 } else { 1.0 };
        ...
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat;
        if (self.unit.elite as f64) > 0.0 { falloutdmg += self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) * final_atk; }
        newres = ((0) as f64).max((res-20.0) as f64);
        let mut elegauge = if self.unit.skill_damage { 1000.0 } else { 2000.0 };
        let mut hitdmg1 = ((final_atk * (1.0 -res/ 100.0)) as f64).max((final_atk * 0.05) as f64);
        let mut hitdmg2 = ((final_atk * (1.0 -newres/ 100.0)) as f64).max((final_atk * 0.05) as f64);
        let mut dps_norm = hitdmg1/(self.unit.attack_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0;
        let mut dps_fallout = hitdmg2/(self.unit.attack_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0;
        ...
        hitdmg = if self.unit.skill_damage { hitdmgarts + hitdmgele } else { hitdmgarts };
        dps = hitdmg* burst_scale/2.5 * self.unit.attack_speed/ 100.0 * (((self.unit.targets as f64)) as f64).min((3) as f64);
        }
        return dps;
    }
}
