...
impl Weedy {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[
        ("talent", "+cannon", false, &[], &[1], 1, 2),
        ("module", "nextToCannon", false, &[], &[1], 0, 2),
    ];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        if (self.unit.skill_index as f64) == 2.0 {
            ...
            final_atk =
                self.unit.atk * (1.0 + self.unit.buff_atk + atkbuff) + self.unit.buff_atk_flat;
            hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
            dps = hitdmg / (3.84 / (self.unit.attack_speed / 100.0))
                * ((self.unit.targets as f64) as f64).min((2) as f64);
        }
        if self.unit.talent_damage && (self.unit.elite as f64) > 0.0 {
            let mut summonhit =
                ((self.unit.drone_atk - defense) as f64).max((self.unit.drone_atk * 0.05) as f64);
