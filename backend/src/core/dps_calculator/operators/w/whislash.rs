...
impl Whislash {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut talent_buff = self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0);
        ...
        let mut aspd = if ((self.unit.skill_index as f64) as f64) == 2.0 { talent_buff * self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) } else { 0.5 * talent_buff * self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) * ((self.unit.skill_index as f64) as f64) };
        ...
        let mut targets = if ((self.unit.skill_index as f64) as f64) == 2.0 { 3.0 } else { 1.0 };
        let mut dps = hitdmg/(self.unit.attack_interval as f64) * (self.unit.attack_speed + aspd)/ 100.0 * ((targets) as f64).min(((self.unit.targets as f64)) as f64);
        return dps;
    }
}
...
impl DpsCalculator for Whislash {
