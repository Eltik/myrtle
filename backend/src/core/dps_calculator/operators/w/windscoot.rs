...
impl Windscoot {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("trait", "halfStacks", true, &[], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut extrahit = if ((self.unit.elite as f64) as f64) > 0.0 && self.unit.trait_damage && self.unit.talent_damage { ((final_atk * self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) - defense) as f64).max((final_atk * 0.05) as f64) } else { 0.0 };
        ...
        dps = (hitdmg+extrahit)/(self.unit.attack_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0;
        }
        ...
        dps = (hitdmg+extrahit)/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 * (((self.unit.targets as f64)) as f64).min((2) as f64);
        }
        return dps;
    }
}
...
impl DpsCalculator for Windscoot {
