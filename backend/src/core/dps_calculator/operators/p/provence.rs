...
impl Provence {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "directFront", false, &[], &[], 1, 0), ("skill", "vs<1%Hp", false, &[], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        crit_rate = if self.unit.talent_damage { self.unit.talent1_parameters.get(1).copied().unwrap_or(0.0) } else { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) };
        }
        if (self.unit.skill_index as f64) < 2.0 {
        skill_scale = if self.unit.skill_damage && ((self.unit.skill_index as f64) as f64) == 1.0 { 1.0 + self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) * 5.0 } else { 1.0 };
...
impl DpsCalculator for Provence {
