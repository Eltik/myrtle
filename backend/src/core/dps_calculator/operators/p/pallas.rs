...
impl Pallas {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("trait", "w/o trait", true, &[], &[], 0, 0), ("talent", "w/o vigor", true, &[], &[], 1, 0), ("skill", "selfbuffS3", false, &[3], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atk_scale = 1.0;
        atkbuff = if self.unit.talent_damage && ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent1_parameters.iter().cloned().fold(f64::INFINITY, f64::min) } else { 0.0 };
        ...
        avgphys = (sp_cost * hitdmg + (1.0 + (self.unit.skill_index as f64)) * skillhitdmg) / (sp_cost + 1.0);
        ...
        atkbuff = ((atkbuff) as f64).max((self.unit.skill_parameters.get(2).copied().unwrap_or(0.0)) as f64);
        }
        atkbuff += self.unit.skill_parameters.get(0).copied().unwrap_or(0.0);
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * atk_scale - defense) as f64).max((final_atk * atk_scale * 0.05) as f64);
        dps = hitdmg/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0	 * (((self.unit.targets as f64)) as f64).min((3) as f64);
        }
        return dps;
    }
}
