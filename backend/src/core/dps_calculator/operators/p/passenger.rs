...
impl Passenger {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut dmg_scale = if ((self.unit.elite as f64) as f64) > 0.0 && self.unit.talent_damage { self.unit.talent1_parameters[1] } else { 1.0 };
        let mut sp_boost = 0.0;
        atkbuff = if self.unit.talent2_damage && ((self.unit.elite as f64) as f64) == 2.0 { self.unit.talent2_parameters[0] } else { 0.0 };
        ...
        atk_interval = (self.unit.attack_interval as f64) * (1.0 + self.unit.skill_parameters[0] * (self.unit.skill_index as f64)/ 2.0);
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * (1.0 -res/ 100.0)) as f64).max((final_atk * 0.05) as f64);
