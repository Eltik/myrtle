...
impl Pudding {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        dps = hitdmg/(self.unit.attack_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0 * targetscaling[(targets) as usize];
        }
        ...
        if (self.unit.targets as f64) > 1.0 { dps = hitdmg/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0  * targetscaling[4]; }
        }
        return dps;
    }
}
