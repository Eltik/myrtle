...
impl PramanixAlter {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("module", "manyTargets", false, &[], &[2], 0, 0), ("skill", "iceOnly", true, &[2], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        dps = hitdmg * (self.unit.targets as f64) / ((self.unit.skill_cost as f64)+1.0) * (1.0 + (self.unit.sp_boost as f64));
        }
        if (self.unit.skill_index as f64) == 2.0 {
        skill_scale = self.unit.skill_parameters.get(0).copied().unwrap_or(0.0);
        let mut ice_scale = self.unit.skill_parameters.get(1).copied().unwrap_or(0.0);
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * atk_scale * skill_scale * (1.0 -res/ 100.0)) as f64).max((final_atk  * skill_scale * atk_scale * 0.05) as f64);
        let mut icedmg = ((final_atk * atk_scale * ice_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * atk_scale * ice_scale * 0.05) as f64);
        dps = hitdmg/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 * (self.unit.targets as f64) + icedmg * (self.unit.targets as f64);
        ...
        hitdmg = ((final_atk * atk_scale * skill_scale * (1.0 -newres/ 100.0)) as f64).max((final_atk  * skill_scale * atk_scale * 0.05) as f64);
        dps = hitdmg/(self.unit.attack_interval as f64) * (self.unit.attack_speed+30.0)/ 100.0 * (self.unit.targets as f64);
        }
        return dps;
    }
}
