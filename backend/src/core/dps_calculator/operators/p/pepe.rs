...
impl Pepe {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("module", "3inRange", false, &[], &[1], 0, 0), ("skill", "maxStacks", false, &[1], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        hitdmg = ((final_atk * atk_scale - defense) as f64).max((final_atk * atk_scale * 0.05) as f64) + ((0.5 * final_atk * atk_scale - defense) as f64).max((0.5 * final_atk * atk_scale * 0.05) as f64) * ((self.unit.targets as f64)-1.0);
        skilldmg = ((final_atk * atk_scale * skill_scale - defense) as f64).max((final_atk * atk_scale * skill_scale * 0.05) as f64) + ((0.5 * skill_scale * final_atk * atk_scale - defense) as f64).max((0.5 * skill_scale * final_atk * atk_scale * 0.05) as f64) * ((self.unit.targets as f64)-1.0);
        ...
        if self.unit.skill_parameters[0] > 2.4 { // a bit of a redneck way, but the json data doesnt seem to include the skill charge count...
        ...
        hitdmgaoe = ((0.5 * final_atk * atk_scale - defense) as f64).max((0.5 * final_atk * atk_scale * 0.05) as f64);
        dps = hitdmg/(atk_interval/((self.unit.attack_speed+aspd)/ 100.0)) + hitdmgaoe/(atk_interval/((self.unit.attack_speed+aspd)/ 100.0))*((self.unit.targets as f64) - 1.0);
        }
        ...
        hitdmg = ((final_atk * atk_scale - defense) as f64).max((final_atk * atk_scale * 0.05) as f64);
        hitdmgaoe = ((0.5 * final_atk * atk_scale - defense) as f64).max((0.5 * final_atk * atk_scale * 0.05) as f64);
        dps = hitdmg/(atk_interval/(self.unit.attack_speed/ 100.0)) + hitdmgaoe/(atk_interval/(self.unit.attack_speed/ 100.0))*((self.unit.targets as f64) - 1.0);
        }
        return dps;
    }
}
