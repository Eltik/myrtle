...
impl Platinum {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("module", "aerial target", false, &[], &[1], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        aspd = if ((self.unit.skill_index as f64) as f64) == 2.0 { -20.0 } else { 0.0 };
        atk_scale = if ((self.unit.module_index as f64) as f64) == 1.0 && self.unit.module_damage { 1.1 } else { 1.0 };
        final_atk = self.unit.atk * (1.0 + self.unit.skill_parameters.iter().cloned().fold(f64::NEG_INFINITY, f64::max) * (((self.unit.skill_index as f64)) as f64).min((1) as f64) + self.unit.buff_atk) + self.unit.buff_atk_flat;
