...
impl Vigna {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("module", "vsLowHp", false, &[], &[2], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut crit_rate = if ((self.unit.elite as f64) as f64) == 0.0 { 0.0 } else { self.unit.talent1_parameters.get(2).copied().unwrap_or(0.0) };
        if (self.unit.skill_index as f64) == 0.0 && (self.unit.elite as f64) > 0.0 { crit_rate = self.unit.talent1_parameters.get(1).copied().unwrap_or(0.0); }
        let mut cdmg = self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0);
        let mut atkbuff = self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) * (((self.unit.skill_index as f64)) as f64).min((1) as f64);
        atk_interval = if ((self.unit.skill_index as f64) as f64) == 2.0 { 1.5 } else { (self.unit.attack_interval as f64) };
        let mut atk_scale = if ((self.unit.module_index as f64) as f64) == 2.0 && self.unit.module_damage { 1.1 } else { 1.0 };
        let mut final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        let mut final_atk_crit = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk + cdmg) + self.unit.buff_atk_flat;
        let mut hitdmg = ((final_atk * atk_scale - defense) as f64).max((final_atk * atk_scale * 0.05) as f64);
        let mut critdmg = ((final_atk_crit * atk_scale - defense) as f64).max((final_atk_crit * atk_scale * 0.05) as f64);
