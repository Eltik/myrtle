...
impl Vigil {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("trait", "vsBlocked", false, &[], &[], 0, 0), ("skill", "1wolf", true, &[], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        defignore = if ((self.unit.elite as f64) as f64) == 2.0 { self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        }
        }
        let mut newdef = ((0) as f64).max((defense - defignore) as f64);
        let mut wolfdef = if ((self.unit.elite as f64) as f64) == 2.0 { ((0) as f64).max((defense - self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0)) as f64) } else { defense };
        // ###the actual skills
        ...
        if self.unit.talent_damage { dps += hitdmgwolf/(self.unit.drone_atk_interval as f64) * self.unit.attack_speed/ 100.0 * wolves; }
        }
        if (self.unit.skill_index as f64) == 2.0 {
        ...
        dps = hitdmg/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0;
        if self.unit.talent_damage { dps += avghit/(self.unit.drone_atk_interval as f64) * self.unit.attack_speed/ 100.0 * wolves; }
        }
