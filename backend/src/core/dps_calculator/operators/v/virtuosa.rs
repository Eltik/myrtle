...
impl Virtuosa {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        if (self.unit.skill_index as f64) == 2.0 {
            ...
            let mut artsdps = hitdmgarts
                / ((self.unit.attack_interval as f64) / ((self.unit.attack_speed + aspd) / 100.0));
            let mut target_eledps =
                falloutdmg * ele_fragile / (15.0 + ele_gauge / ele_application_target);
            let mut ambient_eledps =
                falloutdmg * ele_fragile / (15.0 + ele_gauge / ele_application_base);
            dps = ((self.unit.targets as f64) as f64).min((2) as f64)
                * (artsdps + target_eledps / (1.0 + self.unit.buff_fragile));
            if (self.unit.targets as f64) > 2.0 {
                dps += ambient_eledps * ((self.unit.targets as f64) - 2.0)
                    / (1.0 + self.unit.buff_fragile);
            }
        }
        if [0.0, 3.0].contains(&((self.unit.skill_index as f64) as f64)) {
            ...
            necro_dps = final_atk * necro_scale * necro_fragile;
            time_to_fallout = ele_gauge / necro_dps;
            dps = (self.unit.targets as f64) * 12000.0 * ele_fragile / (15.0 + time_to_fallout)
