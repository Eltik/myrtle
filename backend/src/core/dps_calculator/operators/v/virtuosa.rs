...
impl Virtuosa {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("trait", "vsBoss", true, &[], &[], 0, 0), ("skill", "selfBuff", false, &[3], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut necro_scale = self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0);
        ...
        let mut ele_fragile = if ((self.unit.module_index as f64) as f64) == 1.0 && ((self.unit.module_level as f64) as f64) > 1.0 { self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0) } else { 1.0 };
        ...
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat;
        let mut extra_ele = final_atk * self.unit.skill_parameters.get(1).copied().unwrap_or(0.0);
        ele_gauge = ele_gauge / necro_fragile;
        ...
        if (self.unit.skill_index as f64) == 3.0 { necro_fragile = self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) * (necro_fragile - 1.0) + 1.0; }
        atkbuff = self.unit.skill_parameters.get(0).copied().unwrap_or(0.0);
        atkbuff += if self.unit.skill_damage { self.unit.skill_parameters.get(3).copied().unwrap_or(0.0) } else { 0.0 };
...
impl DpsCalculator for Virtuosa {
