...
impl Viviana {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut dmg_scale = if self.unit.talent_damage { 1.0 + self.unit.talent1_parameters[1] * 2.0 } else { 1.0 + self.unit.talent1_parameters[1] };
        if (self.unit.elite as f64) == 0.0 { dmg_scale = 1.0; }
        let mut burn_res = ((0) as f64).max((res-20.0) as f64);
        let mut fallout_dmg = 7000.0;
        ele_scale = 0.15;
        let mut ele_appli = if ((self.unit.module_index as f64) as f64) == 3.0 && ((self.unit.module_level as f64) as f64) > 1.0 { self.unit.talent1_parameters[0] } else { 0.0 };
        ...
        aspd = if self.unit.skill_damage { self.unit.skill_parameters[6] } else { 0.0 };
