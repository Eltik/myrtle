...
impl Viviana {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "vsElite", false, &[], &[], 1, 0), ("module", "(boss)", false, &[], &[3], 0, 2), ("skill", "afterSteal", false, &[2], &[], 0, 0), ("skill", "1stActivation", true, &[3], &[], 0, 0), ("module", "avgBurn", false, &[], &[3], 0, 2)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut dmg_scale = if self.unit.talent_damage { 1.0 + self.unit.talent1_parameters.get(1).copied().unwrap_or(0.0) * 2.0 } else { 1.0 + self.unit.talent1_parameters.get(1).copied().unwrap_or(0.0) };
        if (self.unit.elite as f64) == 0.0 { dmg_scale = 1.0; }
        let mut burn_res = ((0) as f64).max((res-20.0) as f64);
        let mut fallout_dmg = 7000.0;
        ele_scale = 0.15;
        let mut ele_appli = if ((self.unit.module_index as f64) as f64) == 3.0 && ((self.unit.module_level as f64) as f64) > 1.0 { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        if self.unit.talent_damage { ele_appli *= 2.0; }
        let mut ele_gauge = 1000.0;
        if !self.unit.talent2_damage { ele_gauge = 2000.0; }
        ...
        hitdmgarts = ((final_atk * (1.0 -res/ 100.0)) as f64).max((final_atk * 0.05) as f64) * dmg_scale;
        skilldmg = ((final_atk * skill_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * skill_scale * 0.05) as f64) * dmg_scale * 2.0;
        hitdmgarts2 = ((final_atk * (1.0 -burn_res/ 100.0)) as f64).max((final_atk * 0.05) as f64) * dmg_scale;
        skilldmg2 = ((final_atk * skill_scale * (1.0 -burn_res/ 100.0)) as f64).max((final_atk * skill_scale * 0.05) as f64) * dmg_scale * 2.0;
        ...
        avghit = (skilldmg + ((atks_per_skillactivation) as f64).trunc() * hitdmgarts) / (((atks_per_skillactivation) as f64).trunc()+1.0);
        avghit2 = (skilldmg2 + ((atks_per_skillactivation) as f64).trunc() * hitdmgarts2) / (((atks_per_skillactivation) as f64).trunc()+1.0);
        }
        dps = avghit/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0;
        if (self.unit.module_index as f64) == 3.0 && self.unit.module_damage && (self.unit.module_level as f64) > 1.0 {
        time_to_trigger = ele_gauge / (dps*ele_appli);
        fallout_dps = (avghit2 + ele_scale * final_atk)/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0;
        dps = (dps * time_to_trigger + fallout_dps * 10.0 + fallout_dmg) / (time_to_trigger + 10.0);
        }
        }
        if (self.unit.skill_index as f64) == 2.0 {
        atkbuff = self.unit.skill_parameters.get(0).copied().unwrap_or(0.0);
        aspd = if self.unit.skill_damage { self.unit.skill_parameters.get(6).copied().unwrap_or(0.0) } else { 0.0 };
        ...
        hitdmgarts = ((final_atk * (1.0 -res/ 100.0)) as f64).max((final_atk * 0.05) as f64) * dmg_scale;
        skilldmg = 2.0 * ((final_atk * cdmg * (1.0 -res/ 100.0)) as f64).max((final_atk * cdmg * 0.05) as f64) * dmg_scale;
        avgdmg = crit_rate * skilldmg + (1.0 -crit_rate) * hitdmgarts;
        hitdmgarts2 = ((final_atk * (1.0 -burn_res/ 100.0)) as f64).max((final_atk * 0.05) as f64) * dmg_scale;
        skilldmg2 = 2.0 * ((final_atk * cdmg * (1.0 -burn_res/ 100.0)) as f64).max((final_atk * cdmg * 0.05) as f64) * dmg_scale;
        let mut avgdmg2 = crit_rate * skilldmg2 + (1.0 -crit_rate) * hitdmgarts2;
        dps = avgdmg/(self.unit.attack_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0 * (((self.unit.targets as f64)) as f64).min((2) as f64);
        if (self.unit.module_index as f64) == 3.0 && self.unit.module_damage && (self.unit.module_level as f64) > 1.0 {
        time_to_trigger = ele_gauge / (dps*ele_appli/(((self.unit.targets as f64)) as f64).min((2) as f64));
        fallout_dps = (avgdmg2 + ele_scale * final_atk)/(self.unit.attack_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0;
        dps = (dps * time_to_trigger + fallout_dps * 10.0 + fallout_dmg) / (time_to_trigger + 10.0);
        }
        }
        ...
        if (self.unit.module_index as f64) == 3.0 && self.unit.module_damage && (self.unit.module_level as f64) > 1.0 {
        time_to_trigger = ele_gauge / (dps*ele_appli);
        fallout_dps = hits * (hitdmgarts2 + ele_scale * final_atk)/1.75 * self.unit.attack_speed/ 100.0;
        dps = (dps * time_to_trigger + fallout_dps * 10.0 + fallout_dmg) / (time_to_trigger + 10.0);
        }
        }
        return dps;
...
impl DpsCalculator for Viviana {
