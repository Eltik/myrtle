...
impl Vulpisfoglia {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        // 6, 3, 7, 5, 5, 2 are skill 2 params
        let mut arts_scale = if ((self.unit.elite as f64) as f64) > 0.0 && self.unit.talent_damage { self.unit.talent1_parameters[0] } else { 0.0 };
        ...
        aspd = self.unit.skill_parameters[2] / 2.0;
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        hitdmgarts = ((final_atk * arts_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * arts_scale * 0.05) as f64);
