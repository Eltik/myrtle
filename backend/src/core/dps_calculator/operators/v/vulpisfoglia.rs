...
impl Vulpisfoglia {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "w/o Talent1", true, &[], &[], 1, 0), ("module", "blocking", false, &[], &[1], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        // 6, 3, 7, 5, 5, 2 are skill 2 params
        let mut arts_scale = if ((self.unit.elite as f64) as f64) > 0.0 && self.unit.talent_damage { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        ...
        hitdmgarts = ((final_atk * arts_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * arts_scale * 0.05) as f64);
        ...
        dps = (avghit+hitdmgarts)/(self.unit.attack_interval as f64)*(self.unit.attack_speed)/ 100.0 * (self.unit.targets as f64);
        }
        ...
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        hitdmgarts = ((final_atk * arts_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * arts_scale * 0.05) as f64);
        dps = (hitdmg+hitdmgarts) / (self.unit.attack_interval as f64) * (self.unit.attack_speed + aspd) / 100.0 * (((self.unit.targets as f64)) as f64).min((2) as f64);
        }
        return dps;
    }
}
