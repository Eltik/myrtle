...
impl Eyjafjalla {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("skill", "minResshred", true, &[2], &[], 0, 0), ("talent", "minAspd+", true, &[], &[2], 0, 3), ("module", "vsElite", false, &[2], &[2], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        if self.unit.skill_damage && (self.unit.skill_index as f64) == 1.0 { atkbuff += self.unit.skill_parameters.get(2).copied().unwrap_or(0.0); }
        ...
        final_atk = self.unit.atk * (1.0 +atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        let mut newres2 = ((0) as f64).max((res*(1.0 +self.unit.skill_parameters.get(5).copied().unwrap_or(0.0))-resignore) as f64);
        ...
        let mut extra_boost = if ((self.unit.module_index as f64) as f64) == 2.0 && self.unit.module_damage { 1.0 /(atk_interval)*(self.unit.attack_speed+aspd)/ 100.0 } else { 0.0 };
        ...
        dps = hitdmgarts/atk_interval * (self.unit.attack_speed+aspd)/ 100.0 * (((self.unit.targets as f64)) as f64).min((maxtargets) as f64);
        }
        return dps;
    }
}
...
impl DpsCalculator for Eyjafjalla {
