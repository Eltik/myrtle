...
impl Eyjafjalla {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[
        ("skill", "minResshred", true, &[2], &[], 0, 0),
        ("talent", "minAspd+", true, &[], &[2], 0, 3),
        ("module", "vsElite", false, &[2], &[2], 0, 0),
    ];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        if (self.unit.skill_index as f64) == 2.0 {
            ...
            final_atk =
                self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
            let mut newres2 = ((0) as f64).max(
                (res * (1.0 + self.unit.skill_parameters.get(5).copied().unwrap_or(0.0))
                    - resignore) as f64,
            );
            ...
            skilldmg = ((final_atk * atk_scale * (1.0 - newres2 / 100.0)) as f64)
            ...
            if atks_per_skillactivation > 1.0 {
                if self.unit.skill_parameters[3] > 1.0 {
                    avghit = (skilldmg
                        + ((self.unit.targets as f64) - 1.0) * aoeskilldmg
