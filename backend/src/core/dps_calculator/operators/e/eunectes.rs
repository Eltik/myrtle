...
impl Eunectes {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut atk_scale = if self.unit.talent_damage && ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent1_parameters[2] } else { 1.0 };
        let mut atkbuff = if self.unit.module_damage && ((self.unit.module_index as f64) as f64) == 2.0 { 0.15 } else { 0.0 };
        let mut final_atk = self.unit.atk *(1.0 + self.unit.buff_atk + atkbuff + self.unit.skill_parameters[0] * (((self.unit.skill_index as f64)) as f64).min((1) as f64)) + self.unit.buff_atk_flat;
