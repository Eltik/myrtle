...
impl Exusiai {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("module", "aerialTarget", false, &[], &[1], 0, 0), ("module", "groundEnemies", false, &[], &[2], 0, 0), ("talent", "w/o Defignore", true, &[], &[2], 0, 2)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atkbuff = self.unit.talent2_parameters.iter().cloned().fold(f64::INFINITY, f64::min);
        aspd = self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0);
        if (self.unit.module_index as f64) == 2.0 && self.unit.module_damage { aspd += 8.0; }
        let mut newdef = if ((self.unit.module_index as f64) as f64) == 2.0 && ((self.unit.module_level as f64) as f64) > 1.0 && self.unit.talent_damage { ((defense - self.unit.talent1_parameters.get(1).copied().unwrap_or(0.0)*self.unit.talent1_parameters.get(2).copied().unwrap_or(0.0)) as f64).max((0) as f64) } else { defense };
        ...
        skill_scale = if ((self.unit.skill_index as f64) as f64) > 0.0 { self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) } else { 1.0 };
        if (self.unit.skill_index as f64) < 2.0 {
        hitdmg = ((final_atk * atk_scale - newdef) as f64).max((final_atk * atk_scale * 0.05) as f64);
        let mut skillhitdmg = ((final_atk * atk_scale * skill_scale - newdef) as f64).max((final_atk* atk_scale * skill_scale * 0.05) as f64) * 3.0;
        avgphys = ((self.unit.skill_cost as f64) * hitdmg + skillhitdmg) / ((self.unit.skill_cost as f64) + 1.0);
        ...
        dps = 4.0 *hitdmg/((self.unit.attack_interval as f64)/((self.unit.attack_speed+aspd)/ 100.0));
        } else if (self.unit.skill_index as f64) == 3.0 {
        atk_interval = (self.unit.attack_interval as f64) + 2.0 * self.unit.skill_parameters.get(2).copied().unwrap_or(0.0);
        hitdmg = ((final_atk * atk_scale * skill_scale - newdef) as f64).max((final_atk* atk_scale* skill_scale * 0.05) as f64);
