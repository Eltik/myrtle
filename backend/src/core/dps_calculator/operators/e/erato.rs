...
impl Erato {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "vsSleep", false, &[], &[], 1, 0), ("talent", "selfAppliedSleep", true, &[1], &[], 1, 0), ("module", "vsHeavy", false, &[], &[1], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut newdef = if self.unit.talent_damage || ((self.unit.skill_index as f64) as f64) == 1.0 { defense * (1.0 - self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0)) } else { defense };
        ...
        let mut hitdmg_base = ((final_atk * atk_scale - defense) as f64).max((final_atk * atk_scale * 0.05) as f64);
        if self.unit.talent_damage { hitdmg_base = hitdmg; }
        skilldmg = ((final_atk * skill_scale * atk_scale - newdef) as f64).max((final_atk * skill_scale * atk_scale * 0.05) as f64);
        ...
        let mut hits_on_sleep = ((((5.0 / atkcycle) as f64).trunc()) as f64).min((atks_per_skillactivation) as f64);
        avghit = skilldmg;
        if atks_per_skillactivation > 1.0 {
        avghit = (skilldmg + hits_on_sleep * hitdmg + (atks_per_skillactivation-hits_on_sleep) * hitdmg_base) / (atks_per_skillactivation +1.0);
        }
