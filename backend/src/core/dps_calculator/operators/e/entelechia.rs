...
impl Entelechia {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("skill", "overlapp", false, &[2], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut arts_dps = if ((self.unit.elite as f64) as f64) > 0.0 { ((self.unit.talent1_parameters.get(3).copied().unwrap_or(0.0) * (1.0 -res/ 100.0)) as f64).max((self.unit.talent1_parameters.get(3).copied().unwrap_or(0.0) * 0.05) as f64) * (self.unit.targets as f64) } else { 0.0 };
        ...
        avgphys = if ((self.unit.skill_index as f64) as f64) == 1.0 { (sp_cost * hitdmg + 2.0 * skillhitdmg) / (sp_cost + 1.0) } else { hitdmg };
        dps = avgphys/(self.unit.attack_interval as f64) * (self.unit.attack_speed)/ 100.0 * (self.unit.targets as f64);
        }
        if (self.unit.skill_index as f64) == 2.0 {
        skill_scale = self.unit.skill_parameters.get(0).copied().unwrap_or(0.0);
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat;
        ...
        dps = 2.0 * hitdmg;
        if self.unit.skill_damage { dps *= 2.0; }
        }
        ...
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64) * (self.unit.targets as f64);
        let mut hitdmg_candle = ((final_atk - defense) as f64).max((final_atk * 0.35) as f64) * (((self.unit.targets as f64)) as f64).min((3) as f64);
        dps = (hitdmg+hitdmg_candle)/(self.unit.attack_interval as f64) * (self.unit.attack_speed + aspd)/ 100.0;
        }
        return dps + arts_dps;
    }
}
