...
impl Ela {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent2", "MineDebuff", false, &[], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        cdmg = self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0);
        ...
        crit_rate = self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0);
        }
        ...
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + self.unit.skill_parameters.get(5).copied().unwrap_or(0.0)) + self.unit.buff_atk_flat;
