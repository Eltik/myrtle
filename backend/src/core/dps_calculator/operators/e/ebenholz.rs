...
impl Ebenholz {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "+Talent1Dmg", false, &[], &[], 1, 0), ("talent2", "withAvgNecrosis", false, &[], &[3], 0, 0), ("module", "+30aspd(mod,somehow)", false, &[], &[2], 0, 0), ("module", "vsBoss", true, &[], &[3], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        aspd = if self.unit.module_damage && ((self.unit.module_index as f64) as f64) == 2.0 { 30.0 } else { 0.0 };
        atk_scale = if self.unit.talent_damage && ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) } else { 1.0 };
        eledmg = 0.0;
        let mut bonus_scale = if (self.unit.targets as f64) == 1.0 && ((self.unit.elite as f64) as f64) == 2.0 { self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        eledmg = if ((self.unit.module_index as f64) as f64) == 3.0 && ((self.unit.module_level as f64) as f64) > 1.0 && self.unit.talent2_damage { ((self.unit.module_level as f64) as f64) * 0.1 /(1.0 +self.unit.buff_fragile) } else { 0.0 };
        let mut extra_scale = if ((self.unit.module_index as f64) as f64) == 2.0 && ((self.unit.module_level as f64) as f64) > 1.0 { self.unit.talent2_parameters.get(3).copied().unwrap_or(0.0) } else { 0.0 };
        if (self.unit.skill_index as f64) < 2.0 {
        skill_scale = if ((self.unit.skill_index as f64) as f64) == 1.0 { self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) } else { 1.0 };
        atk_interval = if ((self.unit.skill_index as f64) as f64) == 1.0 { (self.unit.attack_interval as f64) * self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) } else { (self.unit.attack_interval as f64) };
        ...
        bonusdmg = ((final_atk * bonus_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * bonus_scale * 0.05) as f64);
        extradmg = ((final_atk * extra_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * extra_scale * 0.05) as f64);
        ...
        dps += eledmg * final_atk /((self.unit.attack_interval as f64)/((self.unit.attack_speed + aspd)/ 100.0)) * 15.0/(fallouttime + 15.0);
        }
        ...
        dps += extradmg/(atk_interval/((self.unit.attack_speed + aspd)/ 100.0)) * ((self.unit.targets as f64) -1.0);
        }
        }
        ...
        if (self.unit.targets as f64) == 1.0 {
        dps += bonusdmg/((self.unit.attack_interval as f64)/((self.unit.attack_speed + aspd)/ 100.0));
        }
        if (self.unit.targets as f64) > 1.0 && (self.unit.module_index as f64) == 2.0 {
        dps += extradmg/((self.unit.attack_interval as f64)/((self.unit.attack_speed + aspd)/ 100.0)) * ((self.unit.targets as f64) -1.0);
        }
        }
        return dps;
    }
