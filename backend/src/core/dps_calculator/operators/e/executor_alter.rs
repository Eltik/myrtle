...
impl ExecutorAlter {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "NoAmmoUsed", true, &[], &[], 1, 0), ("module", "vs2+", false, &[], &[2], 0, 0)];
    ...
    pub fn new(operator_data: OperatorData, params: OperatorParams) -> Self {
        ...
        if unit.elite > 0 && unit.skill_index != 1 && unit.talent2_damage && unit.elite == 2 { unit.ammo += 4.0; }
        if unit.elite > 0 && unit.skill_index != 1 && !unit.talent_damage { unit.ammo = 1.0; }
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        if self.unit.elite > 0 && self.unit.skill_index != 1 && self.unit.talent2_damage && self.unit.elite == 2 { ammo += 4.0; }
        if self.unit.elite > 0 && self.unit.skill_index != 1 && !self.unit.talent_damage { ammo = 1.0; }
        ...
        let mut crit_rate = if ((self.unit.elite as f64) as f64) > 0.0 && ((self.unit.skill_index as f64) as f64) != 0.0 { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) + self.unit.talent1_parameters.get(1).copied().unwrap_or(0.0) * ammo } else { 0.0 };
        ...
        modatkbuff = if ((self.unit.module_index as f64) as f64) == 2.0 && ((self.unit.module_level as f64) as f64) > 1.0 { 0.04 * ((self.unit.module_level as f64) as f64) } else { 0.0 };
        atkbuff = if ((self.unit.skill_index as f64) as f64) > 0.0 { self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        if (self.unit.skill_index as f64) < 2.0 {
        let mut defignore = if ((self.unit.skill_index as f64) as f64) == 1.0 { self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        let mut newdef = ((0) as f64).max((defense - defignore) as f64);
        critdef = ((0) as f64).max((defense - defignore - critdefignore) as f64);
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk + (self.unit.skill_index as f64) * modatkbuff) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk - newdef) as f64).max((final_atk * 0.05) as f64);
        critdmg = ((final_atk - newdef) as f64).max((final_atk * 0.05) as f64) + ((final_atk - critdef) as f64).max((final_atk * 0.05) as f64);
        avgdmg = crit_rate * critdmg + (1.0 -crit_rate) * hitdmg;
        dps = avgdmg/(self.unit.attack_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0 * (self.unit.targets as f64);
        }
        if (self.unit.skill_index as f64) == 2.0 {
        critdef = ((0) as f64).max((defense - critdefignore) as f64);
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk + modatkbuff) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        critdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64) + ((final_atk - critdef) as f64).max((final_atk * 0.05) as f64);
        avgdmg = crit_rate * critdmg + (1.0 -crit_rate) * hitdmg;
        dps = avgdmg/(self.unit.attack_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0 * (self.unit.targets as f64);
        }
        ...
        critdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64) + ((final_atk - critdef) as f64).max((final_atk * 0.05) as f64);
        avgdmg = crit_rate * critdmg + (1.0 -crit_rate) * hitdmg;
        dps = avgdmg/1.8 * (self.unit.attack_speed+aspd)/ 100.0 * (self.unit.targets as f64);
        }
        return dps;
    }
}
