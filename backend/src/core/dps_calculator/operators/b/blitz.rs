...
impl Blitz {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "vsStun", false, &[1], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atk_scale = if ((self.unit.skill_index as f64) as f64) < 2.0 && !self.unit.talent_damage { 1.0 } else { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) };
        ...
        atk_scale += 1.0;
        hitdmg = ((final_atk * atk_scale - defense) as f64).max((final_atk * atk_scale * 0.05) as f64);
        dps = hitdmg / (self.unit.attack_interval as f64) * (self.unit.attack_speed + 200.0) / 100.0;
        }
        return dps;
    }
}
