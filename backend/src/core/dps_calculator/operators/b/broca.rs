...
impl Broca {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "blocking2+", false, &[], &[], 1, 0), ("module", "vsBlocked", false, &[], &[1], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut atkbuff = if self.unit.talent_damage { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        ...
        atk_interval = if ((self.unit.skill_index as f64) as f64) == 2.0 { 1.98 } else { (self.unit.attack_interval as f64) };
        let mut hitdmg = if ((self.unit.skill_index as f64) as f64) > 0.0 { ((final_atk * atk_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * atk_scale * 0.05) as f64) } else { ((final_atk * atk_scale - defense) as f64).max((final_atk * atk_scale * 0.05) as f64) };
        let mut dps = hitdmg/atk_interval * self.unit.attack_speed/ 100.0 * ((3) as f64).min(((self.unit.targets as f64)) as f64);
        return dps;
    }
}
