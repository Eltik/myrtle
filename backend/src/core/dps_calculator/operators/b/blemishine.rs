//! DPS calculations for Blemishine
//!
...
/// Blemishine operator implementation
pub struct Blemishine {
...
impl Blemishine {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent2", "vsSleep", false, &[], &[], 0, 0)];

    /// Creates a new Blemishine operator
    #[allow(unused_parens)]
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atkbuff = 0.0;
        atk_scale = if self.unit.talent2_damage { self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0) } else { 1.0 };
        ...
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + atkbuff) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * atk_scale - defense) as f64).max((final_atk * atk_scale * 0.05) as f64);
        let mut artsdmg = ((final_atk * atk_scale * self.unit.skill_parameters.get(2).copied().unwrap_or(0.0) * (1.0 -res/ 100.0)) as f64).max((final_atk * atk_scale * self.unit.skill_parameters.get(2).copied().unwrap_or(0.0) * 0.05) as f64);
        dps = (hitdmg + artsdmg) / (self.unit.attack_interval as f64) * self.unit.attack_speed / 100.0;
        }
        return dps;
    }
}

impl std::ops::Deref for Blemishine {
...
impl std::ops::DerefMut for Blemishine {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.unit
    }
}

impl DpsCalculator for Blemishine {
