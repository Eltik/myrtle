...
impl BluePoison {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("module", "GroundTargets", false, &[], &[2], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        aspd = if self.unit.module_damage && ((self.unit.module_index as f64) as f64) == 2.0 { 8.0 } else { 0.0 };
        let mut artsdmg = self.unit.talent1_parameters.get(1).copied().unwrap_or(0.0);
        let mut artsdps = if ((self.unit.elite as f64) as f64) > 0.0 { ((artsdmg * (1.0 - res/ 100.0)) as f64).max((artsdmg * 0.05) as f64) } else { 0.0 };
        if (self.unit.skill_index as f64) < 2.0 {
        skill_scale = self.unit.skill_parameters.get(0).copied().unwrap_or(0.0);
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        let mut skillhitdmg = ((final_atk * skill_scale - defense) as f64).max((final_atk * skill_scale * 0.05) as f64) * ((2) as f64).min(((self.unit.targets as f64)) as f64);
        ...
        dps = avgphys/((self.unit.attack_interval as f64)/((self.unit.attack_speed + aspd)/ 100.0)) + artsdps * ((1.0 + (self.unit.skill_index as f64)) as f64).min(((self.unit.targets as f64)) as f64);
        }
        if (self.unit.skill_index as f64) == 2.0 {
        ...
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        dps = self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) * hitdmg/((self.unit.attack_interval as f64)/((self.unit.attack_speed+ aspd)/ 100.0)) + hitdmg/((self.unit.attack_interval as f64)/((self.unit.attack_speed+ aspd)/ 100.0)) * ((2) as f64).min(((self.unit.targets as f64)-1.0) as f64) + artsdps * ((3) as f64).min(((self.unit.targets as f64)) as f64);
        }
        return dps;
    }
}
