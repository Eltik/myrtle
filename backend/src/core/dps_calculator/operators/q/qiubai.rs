...
impl Qiubai {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("trait", "rangedAtk", true, &[], &[], 0, 0), ("talent", "vsBindANDslow", false, &[], &[1], 0, 2), ("talent", "vsBind/Slow", false, &[], &[], 0, 0), ("talent", "w/o talent1", true, &[], &[], 0, 0), ("module", "vsBindAndSlow", false, &[], &[], 0, 0), ("talent", "vsBindOrSlow", false, &[], &[], 0, 0), ("skill", "maxStacks", false, &[], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut bonus = if ((self.unit.module_index as f64) as f64) == 1.0 { 0.1 } else { 0.0 };
        extrascale = if ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        dmg = if ((self.unit.module_index as f64) as f64) == 1.0 && self.unit.module_damage { 1.0 + 0.1 * (((self.unit.module_level as f64) as f64)-1.0) } else { 1.0 };
        ...
        hitdmg = ((final_atk * atk_scale - defense) as f64).max((final_atk * atk_scale * 0.05) as f64) * dmg;
        hitdmgarts = ((final_atk * extrascale * (1.0 -res/ 100.0)) as f64).max((final_atk * extrascale * 0.05) as f64) * dmg;
        skilldmg = ((final_atk * (skill_scale+extrascale) * (1.0 -res/ 100.0)) as f64).max((final_atk * (skill_scale+extrascale) * 0.05) as f64) * dmg * (self.unit.targets as f64);
        bonusdmg = ((final_atk * bonus *(1.0 -res/ 100.0)) as f64).max((final_atk * bonus * 0.05) as f64);
        avghit = (hitdmg + hitdmgarts + bonusdmg) * (self.unit.skill_cost as f64) + skilldmg + bonusdmg * (self.unit.targets as f64);
        avghit = if ((self.unit.skill_index as f64) as f64) == 1.0 { avghit/((self.unit.skill_cost as f64)+1.0) } else { hitdmg + hitdmgarts + bonusdmg };
        ...
        aspd = if self.unit.skill_damage { self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) * self.unit.skill_parameters.get(2).copied().unwrap_or(0.0) } else { 0.0 };
        ...
        let mut atk_cycle = (self.unit.attack_interval as f64) / (self.unit.attack_speed + aspd) * 100.0;
        let mut bind_chance = self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0);
        let mut counting_hits = ((1.5/atk_cycle) as f64).trunc() + 1.0;
        let mut chance_to_attack_bind = 1.0 - ((1.0 -bind_chance) as f64).powf(counting_hits as f64);
        if !self.unit.talent_damage && !self.unit.talent2_damage { // talent not active
        extrascale = 0.0;
        dmg = 1.0;
        ...
        hitdmgarts = ((final_atk * (1.0 +extrascale) * (1.0 -res/ 100.0)) as f64).max((final_atk * (1.0 +extrascale) * 0.05) as f64) * dmg;
        bonusdmg = ((final_atk * bonus *(1.0 -res/ 100.0)) as f64).max((final_atk * bonus * 0.05) as f64);
        dps = (hitdmgarts+bonusdmg)/(self.unit.attack_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0 * ((3) as f64).min(((self.unit.targets as f64)) as f64);
        }
        return dps;
    }
}
