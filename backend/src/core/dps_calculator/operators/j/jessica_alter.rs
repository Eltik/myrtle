//! DPS calculations for JessicaAlter
//!
...
/// JessicaAlter operator implementation
pub struct JessicaAlter {
...
impl JessicaAlter {
    ...
    /// Creates a new JessicaAlter operator
    #[allow(unused_parens)]
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) * (self.unit.skill_index as f64)) + self.unit.buff_atk_flat;
        ...
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + self.unit.skill_parameters.get(0).copied().unwrap_or(0.0)) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        dps = hitdmg/ 1.8 * self.unit.attack_speed/ 100.0;
        }
        return dps;
    }
}

impl std::ops::Deref for JessicaAlter {
...
impl std::ops::DerefMut for JessicaAlter {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.unit
    }
}

impl DpsCalculator for JessicaAlter {
