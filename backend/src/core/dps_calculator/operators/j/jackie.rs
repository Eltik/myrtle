...
impl Jackie {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "afterDodge", false, &[], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut aspd = if self.unit.talent_damage { self.unit.talent1_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        ...
        let mut skilldmg = ((final_atk * self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) - defense) as f64).max((final_atk * self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) * 0.05) as f64);
        let mut avgdmg = if ((self.unit.skill_index as f64) as f64) == 1.0 { (hitdmg * (self.unit.skill_cost as f64) + skilldmg) / ((self.unit.skill_cost as f64)+1.0) } else { hitdmg };
        let mut dps = avgdmg/(self.unit.attack_interval as f64)*(self.unit.attack_speed+aspd)/ 100.0;
        return dps;
    }
}
