...
impl Muelsyse {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "no Clones", true, &[], &[], 0, 0), ("trait", "blocked", false, &[], &[], 0, 0), ("talent2", "maxSteal", false, &[], &[], 0, 0), ("trait", "blocked", false, &[], &[], 0, 0), ("talent", "noClones", true, &[], &[], 0, 0), ("trait", "blocked", false, &[], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut copy_factor = if ((self.unit.module_index as f64) as f64) == 1.0 && ((effective_module_level as f64) as f64) == 3.0 { 1.0 } else { 0.5 + 0.2 * ((self.unit.elite as f64) as f64) };
        atkbuff = if ((self.unit.skill_index as f64) as f64) == 1.0 { self.unit.skill_parameters.get(2).copied().unwrap_or(0.0) } else { self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) * ((((self.unit.skill_index as f64) as f64)) as f64).min((1) as f64) };
        aspd = if ((self.unit.skill_index as f64) as f64) == 1.0 { self.unit.skill_parameters.get(3).copied().unwrap_or(0.0) } else { 0.0 };
        ...
        let mut clone_atk = cloned_op_atk * copy_factor * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        if !cloned_op_ranged && self.unit.talent2_damage { clone_atk += 250.0; }
        let mut summondamage = if !cloned_op_physical { ((clone_atk * (1.0 -res/ 100.0)) as f64).max((clone_atk * 0.05) as f64) } else { ((clone_atk - defense) as f64).max((clone_atk * 0.05) as f64) };
        extra_summons = 0.0;
        let mut extra_summons_skill = 0.0;
        if cloned_op_ranged && self.unit.talent_damage {
        extra_summons += ((4) as f64).min((2.5/(cloned_op_atk_interval/((self.unit.attack_speed + aspd)/ 100.0))) as f64);
        extra_summons_skill = if ((self.unit.skill_index as f64) as f64) == 2.0 { ((4) as f64).min((2.5/(cloned_op_atk_interval/((self.unit.attack_speed + aspd)/ 100.0)) * 2.0) as f64) } else { ((4) as f64).min((2.5/(cloned_op_atk_interval/((self.unit.attack_speed + aspd)/ 100.0))) as f64) };
        ...
        dps += (main+extra_summons) * summondamage/(cloned_op_atk_interval/((self.unit.attack_speed + aspd)/ 100.0));
        }
        if (self.unit.skill_index as f64) == 2.0 && cloned_op_ranged {
        dps += (main+extra_summons) * summondamage/(cloned_op_atk_interval/((self.unit.attack_speed + aspd)/ 100.0)) * 2.0;
        } else if (self.unit.skill_index as f64) != 3.0 || ((self.unit.skill_index as f64) == 3.0 && !cloned_op_ranged) {
        dps += (main+extra_summons) * summondamage/(cloned_op_atk_interval/((self.unit.attack_speed + aspd)/ 100.0));
        }
        return dps;
    }
