...
impl MissChristine {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("skill", "avgNerv", false, &[1], &[], 0, 0), ("skill", "vsFallout", false, &[2], &[], 0, 0), ("module", "vsPara", false, &[], &[1], 0, 2)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        if (self.unit.module_index as f64) == 1.0 && (self.unit.module_level as f64) > 1.0 && self.unit.module_damage {
        ...
        final_atk = self.unit.atk * (1.0 + self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * atk_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * atk_scale * 0.05) as f64);
        dps = hitdmg / (self.unit.attack_interval as f64) * self.unit.attack_speed / 100.0;
        ...
        ele_scale = if self.unit.skill_damage && self.unit.trait_damage { self.unit.skill_parameters.get(2).copied().unwrap_or(0.0) } else { 0.0 };
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat;
