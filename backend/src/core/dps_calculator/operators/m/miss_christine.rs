...
impl MissChristine {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        final_atk = self.unit.atk * (1.0 + self.unit.skill_parameters[0] + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * atk_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * atk_scale * 0.05) as f64);
        dps = hitdmg / (self.unit.attack_interval as f64) * self.unit.attack_speed / 100.0;
        ...
        ele_scale = if self.unit.skill_damage && self.unit.trait_damage { self.unit.skill_parameters[2] } else { 0.0 };
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat;
