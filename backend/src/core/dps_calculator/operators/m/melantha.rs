...
/// Melantha operator implementation
pub struct Melantha {
...
impl Melantha {
    ...
    /// Creates a new Melantha operator
    #[allow(unused_parens)]
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) + self.unit.skill_parameters.get(0).copied().unwrap_or(0.0)*(self.unit.skill_index as f64)) + self.unit.buff_atk_flat;
...
impl std::ops::Deref for Melantha {
...
impl std::ops::DerefMut for Melantha {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.unit
    }
}

impl DpsCalculator for Melantha {
