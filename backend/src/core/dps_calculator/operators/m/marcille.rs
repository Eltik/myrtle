...
impl Marcille {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "noMana", true, &[], &[], 0, 0), ("talent2", "FullSquad", false, &[], &[], 2, 0), ("skill", "2ndActivation", false, &[2], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        dps = 0.0;
        atkbuff = if self.unit.talent_damage { self.unit.talent1_parameters.get(5).copied().unwrap_or(0.0) } else { 0.0 };
        aspd = if self.unit.talent2_damage { self.unit.talent2_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        if (self.unit.skill_index as f64) < 2.0 {
        atkbuff += if ((self.unit.skill_index as f64) as f64) == 1.0 { self.unit.skill_parameters.get(3).copied().unwrap_or(0.0) } else { 0.0 };
        ...
        aspd += if self.unit.skill_damage { self.unit.skill_parameters.get(5).copied().unwrap_or(0.0) } else { 0.0 };
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * (1.0 -res/ 100.0)) as f64).max((final_atk * 0.05) as f64);
        dps = hitdmg / (self.unit.attack_interval as f64) * (self.unit.attack_speed + aspd) / 100.0 * (self.unit.targets as f64);
        }
    ...
    pub fn total_dmg(&self, enemy: &EnemyStats) -> f64 {
        ...
        if (self.unit.skill_index as f64) == 3.0 { skill_duration = 10.0; }
