...
impl Magallan {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("trait", "noDrones", true, &[1], &[], 0, 0)];
    ...
    pub fn new(operator_data: OperatorData, params: OperatorParams) -> Self {
        ...
        if unit.module_index == 2 && unit.module_level == 3 && unit.skill_index == 2 { unit.drone_atk += 40.0; }
        if unit.module_index == 2 && unit.module_level == 3 && (unit.skill_index == 0 || unit.skill_index == 3) { unit.drone_atk += 50.0; }
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        final_drone = self.unit.drone_atk * (1.0 + self.unit.buff_atk + self.unit.skill_parameters.get(0).copied().unwrap_or(0.0)*(self.unit.skill_index as f64)/ 3.0) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * (1.0 -res/ 100.0)) as f64).max((final_atk * 0.05) as f64);
