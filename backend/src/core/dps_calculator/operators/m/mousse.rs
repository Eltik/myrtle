...
impl Mousse {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("module", "NotBlocking", false, &[], &[1], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut final_atk2 = self.unit.atk * (1.0 + self.unit.buff_atk + atkbuff) + self.unit.buff_atk_flat;
        let mut hitdmg2 = ((final_atk2 * (1.0 -res/ 100.0)) as f64).max((final_atk2 * 0.05) as f64);
        avgdmg = (hitdmg * sp_cost + hitdmg2) / (sp_cost + 1.0);
        dps = avgdmg/((self.unit.attack_interval as f64)/((self.unit.attack_speed+aspd)/ 100.0)) * (1.0 +crit_rate);
        }
        if (self.unit.skill_index as f64) == 2.0 {
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + atkbuff) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * (1.0 -res/ 100.0)) as f64).max((final_atk * 0.05) as f64);
        dps = hitdmg/((self.unit.attack_interval as f64)/((self.unit.attack_speed+aspd)/ 100.0)) * (1.0 +crit_rate);
        }
        return dps;
    }
}
