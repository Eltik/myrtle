...
impl Mizuki {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent2", "vsLowHp", false, &[], &[], 0, 0), ("module", "inIS", false, &[], &[3], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        bonusdmg = if ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        let mut bonustargets = if ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent1_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        atkbuff = if self.unit.talent2_damage { self.unit.talent2_parameters.get(1).copied().unwrap_or(0.0) } else { 0.0 };
        aspd = if ((self.unit.module_index as f64) as f64) == 3.0 && self.unit.module_damage { 50.0 } else { 0.0 };
        if (self.unit.module_index as f64) == 3.0 && self.unit.module_damage && (self.unit.module_level as f64) > 1.0 { bonustargets += 1.0; }
        if (self.unit.module_index as f64) == 3.0 && self.unit.module_damage && (self.unit.module_level as f64) == 3.0 && (self.unit.skill_index as f64) > 0.0 { bonustargets += 1.0; }
        ...
        let mut talent_scale = if ((self.unit.skill_index as f64) as f64) == 1.0 { self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) } else { 1.0 };
        ...
        let mut hitbonus = ((final_atk * bonusdmg * (1.0 -res/ 100.0)) as f64).max((final_atk * bonusdmg * 0.05) as f64);
        skilldmg = ((final_atk * skill_scale - defense) as f64).max((final_atk * skill_scale * 0.05) as f64);
        let mut skillbonus = ((final_atk * bonusdmg * talent_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * bonusdmg * talent_scale * 0.05) as f64);
        ...
        avgarts = (skillbonus + (atks_per_skillactivation -1.0) * hitbonus) / atks_per_skillactivation;
        } else {
        avghit = (skilldmg + ((atks_per_skillactivation) as f64).trunc() * hitdmg) / (((atks_per_skillactivation) as f64).trunc()+1.0);
        avgarts = (skillbonus + ((atks_per_skillactivation) as f64).trunc() * hitbonus) / (((atks_per_skillactivation) as f64).trunc()+1.0);
        }
        }
        dps = avghit/((self.unit.attack_interval as f64)/(self.unit.attack_speed/ 100.0)) * (self.unit.targets as f64) + avgarts/((self.unit.attack_interval as f64)/((self.unit.attack_speed+aspd)/ 100.0)) * (((self.unit.targets as f64)) as f64).min((bonustargets) as f64);
        }
        if (self.unit.skill_index as f64) == 2.0 {
        ...
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        hitdmgarts = ((final_atk * bonusdmg * (1.0 -res/ 100.0)) as f64).max((final_atk * bonusdmg * 0.05) as f64);
        dps = hitdmg/(atk_interval/(self.unit.attack_speed/ 100.0)) * (self.unit.targets as f64) + hitdmgarts/(atk_interval/((self.unit.attack_speed+aspd)/ 100.0)) * (((self.unit.targets as f64)) as f64).min((bonustargets) as f64);
        }
        ...
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        hitdmgarts = ((final_atk * bonusdmg * (1.0 -res/ 100.0)) as f64).max((final_atk * bonusdmg * 0.05) as f64);
        dps = hitdmg/((self.unit.attack_interval as f64)/(self.unit.attack_speed/ 100.0)) * (self.unit.targets as f64) + hitdmgarts/((self.unit.attack_interval as f64)/((self.unit.attack_speed+aspd)/ 100.0)) * (((self.unit.targets as f64)) as f64).min((bonustargets) as f64);
        }
        return dps;
    }
}
