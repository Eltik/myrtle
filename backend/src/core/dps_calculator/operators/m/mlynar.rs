...
impl Mlynar {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("trait", "-10stacks", true, &[], &[], 0, 0), ("talent", "3+Nearby", false, &[], &[], 1, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atkbuff = 0.0;
        atk_scale = 1.0;
        if (self.unit.elite as f64) > 0.0 { atk_scale = if self.unit.talent_damage || (self.unit.targets as f64) > 2.0 { self.unit.talent1_parameters.get(2).copied().unwrap_or(0.0) } else { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) }; }
        let mut stacks = 40.0;
        if !self.unit.trait_damage { stacks -= 10.0; }
        atkbuff += stacks * 0.05;
        if (self.unit.skill_index as f64) == 0.0 { dps = res * 0.0; }
        ...
        finaldmg = ((final_atk * atk_scale - defense) as f64).max((final_atk * atk_scale * 0.05) as f64);
        ...
        atk_scale *= self.unit.skill_parameters.get(0).copied().unwrap_or(0.0);
        final_atk = self.unit.atk * (1.0 +atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        finaldmg = ((final_atk * atk_scale - defense) as f64).max((final_atk * atk_scale * 0.05) as f64) * 2.0;
        dps = finaldmg/atk_interval * self.unit.attack_speed/ 100.0;
        }
        ...
        atk_scale *= self.unit.skill_parameters.get(0).copied().unwrap_or(0.0);
        final_atk = self.unit.atk * (1.0 +atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        let mut truedmg = final_atk * self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) * ((1) as f64).max((-defense) as f64);
        finaldmg = ((final_atk * atk_scale - defense) as f64).max((final_atk * atk_scale * 0.05) as f64);
