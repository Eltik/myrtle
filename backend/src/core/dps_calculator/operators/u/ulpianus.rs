...
impl Ulpianus {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut bonus_base = if self.unit.talent2_damage && ((self.unit.elite as f64) as f64) == 2.0 { self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0) * self.unit.talent2_parameters.get(2).copied().unwrap_or(0.0) } else { 0.0 };
        ...
        final_atk = (self.unit.atk + bonus_base) * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat;
        ...
        dps = avghit/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 * ((2) as f64).min(((self.unit.targets as f64)) as f64);
        }
        if (self.unit.skill_index as f64) == 2.0 {
        atkbuff = self.unit.skill_parameters.get(1).copied().unwrap_or(0.0);
        final_atk = (self.unit.atk + bonus_base) * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        dps = hitdmg/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 * ((3) as f64).min(((self.unit.targets as f64)) as f64);
        }
        if (self.unit.skill_index as f64) == 3.0 {
        atkbuff = self.unit.skill_parameters.get(1).copied().unwrap_or(0.0);
        final_atk = (self.unit.atk + bonus_base) * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        dps = hitdmg/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 * ((2) as f64).min(((self.unit.targets as f64)) as f64);
        }
        return dps;
    }
}
