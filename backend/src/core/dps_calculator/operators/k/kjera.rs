...
impl Kjera {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "noGroundTiles", true, &[], &[], 1, 0), ("trait", "minDroneDmg", true, &[], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atkbuff = 0.0;
        if (self.unit.elite as f64) > 0.0 { atkbuff += if self.unit.talent_damage { self.unit.talent1_parameters.get(2).copied().unwrap_or(0.0) } else { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) }; }
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk + self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) * (((self.unit.skill_index as f64)) as f64).min((1) as f64)) + self.unit.buff_atk_flat;
        let mut drone_atk = drone_dmg * final_atk;
        let mut dmgperinterval = final_atk + drone_atk * (self.unit.skill_index as f64);
        if (self.unit.skill_index as f64) < 2.0 {
        hitdmgarts = ((dmgperinterval *(1.0 -res/ 100.0)) as f64).max((dmgperinterval * 0.05) as f64);
        dps = hitdmgarts/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0;
        }
        if (self.unit.skill_index as f64) == 2.0 {
        let mut res2 = ((0) as f64).max((res-15.0) as f64);
        hitdmgarts = ((dmgperinterval *(1.0 -res/ 100.0)) as f64).max((dmgperinterval * 0.05) as f64);
        let mut hitdmgfreeze = ((dmgperinterval *(1.0 -res2/ 100.0)) as f64).max((dmgperinterval * 0.05) as f64);
