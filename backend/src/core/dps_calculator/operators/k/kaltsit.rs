...
impl Kaltsit {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "NotInHealRange", true, &[], &[2], 0, 2)];
    ...
    pub fn new(operator_data: OperatorData, params: OperatorParams) -> Self {
        ...
        if (unit.module_index == 2 || unit.module_index == 3) { unit.attack_speed -= 4.0 + (unit.module_level as f64); }
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        if (self.unit.module_level as f64) == 2.0 { aspd = 12.0; }
        if (self.unit.module_level as f64) == 3.0 { aspd = 20.0; }
        }
        atkbuff = if ((self.unit.module_index as f64) as f64) == 3.0 { 0.25 * (((self.unit.module_level as f64) as f64) - 1.0) } else { 0.0 };
        if (self.unit.skill_index as f64) < 2.0 {
        final_atk = self.unit.drone_atk * (1.0 + self.unit.buff_atk + atkbuff) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        dps = hitdmg/(self.unit.drone_atk_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0;
        }
        if (self.unit.skill_index as f64) == 2.0 {
        final_atk = self.unit.drone_atk * (1.0 + self.unit.buff_atk + self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) + atkbuff) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        dps = hitdmg/(self.unit.drone_atk_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0 * (((self.unit.targets as f64)) as f64).min((3) as f64);
        }
        if (self.unit.skill_index as f64) == 3.0 {
        final_atk = self.unit.drone_atk * (1.0 + self.unit.buff_atk + self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) * 0.5 + atkbuff) + self.unit.buff_atk_flat;
        dps = final_atk/(self.unit.drone_atk_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0 * ((-defense) as f64).max((1) as f64);
        }
        return dps;
    }
}
