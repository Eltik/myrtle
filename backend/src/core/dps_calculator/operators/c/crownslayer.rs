...
impl Crownslayer {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent2", "dmgTaken", true, &[], &[], 2, 0), ("skill", "singleTargetOnly", true, &[3], &[], 0, 0), ("module", "alone", false, &[], &[2], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atkbuff = if self.unit.module_damage && ((self.unit.module_index as f64) as f64) == 2.0 { 0.1 } else { 0.0 };
        atk_scale = if self.unit.talent2_damage && ((self.unit.elite as f64) as f64) == 2.0 { self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0) } else { 1.0 };
        if (self.unit.skill_index as f64) < 2.0 {
        final_atk = self.unit.atk * (1.0 + self.unit.skill_parameters.get(0).copied().unwrap_or(0.0)*(self.unit.skill_index as f64) + self.unit.buff_atk + atkbuff) + self.unit.buff_atk_flat;
        ...
        dps = hitdmg;
        if !self.unit.skill_damage { dps *= 1.0 / 3.0; }
        }
        return dps;
    }
}
