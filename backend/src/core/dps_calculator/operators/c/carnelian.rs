...
impl Carnelian {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("skill", "charged", false, &[], &[], 0, 0), ("module", "manyTargets", false, &[], &[2], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        if (self.unit.skill_index as f64) == 0.0 { return (defense * 0.0); }
        ...
        atkbuff = if self.unit.skill_damage { self.unit.skill_parameters.get(2).copied().unwrap_or(0.0) } else { 0.0 };
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + atkbuff) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * atk_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * atk_scale * 0.05) as f64);
        dps = hitdmg/atk_interval * self.unit.attack_speed/ 100.0 * (self.unit.targets as f64);
        }
        if (self.unit.skill_index as f64) == 3.0 {
        let mut maxatkbuff = self.unit.skill_parameters.get(0).copied().unwrap_or(0.0);
        let mut duration = 21.0;
        let mut totalatks = 1.0 + ((duration / ((self.unit.attack_interval as f64)/(self.unit.attack_speed/ 100.0))) as f64).trunc();
        let mut totalduration = totalatks * ((self.unit.attack_interval as f64)/(self.unit.attack_speed/ 100.0));
        let mut damage = 0.0;
        ...
        for _i in 0..(totalatks as i32) {
            let i = _i as f64;
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk + i * ((self.unit.attack_interval as f64)/(self.unit.attack_speed/ 100.0)) / 21.0 * maxatkbuff) + self.unit.buff_atk_flat;
        damage += ((final_atk * atk_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * atk_scale * 0.05) as f64) * (1.0 + ((bonusscaling) as f64).min((i) as f64) * 0.2);
        }
...
impl DpsCalculator for Carnelian {
