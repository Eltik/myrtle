...
impl ChenAlter {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("trait", "maxRange", true, &[0], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        aspd = if ((self.unit.elite as f64) as f64) == 2.0 { 8.0 } else { 0.0 };
        if (self.unit.module_index as f64) == 2.0 {
        if (self.unit.module_level as f64) == 2.0 { atkbuff += 0.1; }
        if (self.unit.module_level as f64) == 3.0 {
        atkbuff += 0.28;
        aspd = 20.0;
        }
        }
        atk_scale = if ((self.unit.module_index as f64) as f64) == 1.0 { 1.6 } else { 1.5 };
        if (self.unit.skill_index as f64) == 0.0 && !self.unit.trait_damage { atk_scale = 1.0; }
        ...
        if self.unit.shreds[0] < 1.0 && self.unit.shreds[0] > 0.0 {
        newdefense *= self.unit.shreds.get(0).copied().unwrap_or(0.0);
        }
        hitdmg = ((final_atk * atk_scale - newdefense) as f64).max((final_atk* atk_scale * 0.05) as f64);
        dps = 2.0 * hitdmg/(self.unit.attack_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0 * (self.unit.targets as f64);
        }
        return dps;
    }
...
impl DpsCalculator for ChenAlter {
