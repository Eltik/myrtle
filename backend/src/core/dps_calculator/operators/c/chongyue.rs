...
impl Chongyue {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("module", ">50%Hp", false, &[], &[2], 0, 0), ("talent", "NoSkillCrit", true, &[1], &[], 1, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        aspd = if ((self.unit.module_index as f64) as f64) == 2.0 && self.unit.module_damage { 10.0 } else { 0.0 };
        let mut crit_rate = if ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        let mut dmg = if ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent1_parameters.get(1).copied().unwrap_or(0.0) } else { 1.0 };
        let mut duration = if ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent1_parameters.get(2).copied().unwrap_or(0.0) } else { 0.0 };
        ...
        if self.unit.talent_damage && (self.unit.elite as f64) > 0.0 { skilldmg *= dmg; }
        relevant_hits = ((duration/((self.unit.attack_interval as f64) /(self.unit.attack_speed+aspd)*100.0)) as f64).trunc() + 1.0;
        crit_chance = 1.0 - ((1.0 -crit_rate) as f64).powf(relevant_hits as f64);
        hitdmg *= (1.0 -crit_chance) + dmg * crit_chance;
        dps = (hitdmg + skilldmg/(self.unit.skill_cost as f64) * (self.unit.skill_index as f64)) / (self.unit.attack_interval as f64) * (self.unit.attack_speed+aspd)/ 100.0;
        }
        if (self.unit.skill_index as f64) == 3.0 {
        let mut hits = (self.unit.skill_cost as f64)  / 2.0 + (self.unit.skill_cost as f64) % 2.0;
        relevant_hits = ((duration/((self.unit.attack_interval as f64) /(self.unit.attack_speed+aspd)*100.0)) as f64).trunc() * 2.0 + 2.0;
        relevant_hits *= hits/(hits+1.0);
        crit_chance = 1.0 - ((1.0 -crit_rate) as f64).powf(relevant_hits as f64);
        skilldmg *= (self.unit.targets as f64);
        avghit = 2.0 * (hits * hitdmg + skilldmg) /(hits + 1.0) * ((1.0 -crit_chance) + dmg * crit_chance);
