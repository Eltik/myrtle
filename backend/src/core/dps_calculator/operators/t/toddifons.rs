//! DPS calculations for Toddifons
//!
...
/// Toddifons operator implementation
pub struct Toddifons {
...
impl Toddifons {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "withRacism", false, &[], &[], 1, 0), ("module", "vsHeavy", false, &[], &[1], 0, 0)];

    /// Creates a new Toddifons operator
    #[allow(unused_parens)]
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        if (self.unit.module_index as f64) == 1.0 && self.unit.module_damage { atk_scale *= 1.15; }
        ...
        let mut newdef = defense * (1.0 + self.unit.skill_parameters.get(1).copied().unwrap_or(0.0));
        hitdmg = ((final_atk * skill_scale * atk_scale - newdef) as f64).max((final_atk * skill_scale * atk_scale * 0.05) as f64);
        dps = hitdmg/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0;
        }
        ...
        let mut hitdmg2 = ((final_atk * skill_scale2 * atk_scale - defense) as f64).max((final_atk * skill_scale2 * atk_scale * 0.05) as f64) * (self.unit.targets as f64);
        dps = (hitdmg+hitdmg2)/3.12 * self.unit.attack_speed/ 100.0;
        }
        return dps;
    }
}

impl std::ops::Deref for Toddifons {
...
impl std::ops::DerefMut for Toddifons {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.unit
    }
}

impl DpsCalculator for Toddifons {
