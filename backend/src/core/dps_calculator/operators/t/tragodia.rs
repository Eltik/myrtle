...
impl Tragodia {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("trait", "vsBoss", true, &[], &[], 0, 0), ("module", "vsElite", false, &[], &[], 0, 0), ("skill", "onlyCat", true, &[2], &[], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut mod_factor = if ((self.unit.module_index as f64) as f64) == 1.0 && (!self.unit.trait_damage || self.unit.module_damage) { 1.18 } else { 1.0 };
        let mut ele_gauge = if self.unit.trait_damage { 1000.0 } else { 2000.0 };
        atkbuff = if ((self.unit.skill_index as f64) as f64) == 3.0 { self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        ...
        ele_dps = 6000.0 /(10.0 + (ele_gauge / (final_atk * nerv_factor * mod_factor / (self.unit.attack_interval as f64) * self.unit.attack_speed / 100.0)));
        dps = hitdmg / (self.unit.attack_interval as f64) * self.unit.attack_speed / 100.0 + ele_dps;
        }
        if (self.unit.skill_index as f64) == 1.0 {
        skilldmg = ((final_atk * self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) * (1.0 -res/ 100.0)) as f64).max((final_atk * self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) * 0.05) as f64) * 2.0;
        let mut nerv_dps = (final_atk * nerv_factor * mod_factor * (self.unit.skill_cost as f64) + 2.0 * final_atk * nerv_factor * mod_factor * self.unit.skill_parameters.get(1).copied().unwrap_or(0.0))/((self.unit.skill_cost as f64)+1.0)/ (self.unit.attack_interval as f64) * self.unit.attack_speed  / 100.0;
        ele_dps = 6000.0 /(10.0 +ele_gauge/nerv_dps);
        dps = (skilldmg + hitdmg * (self.unit.skill_cost as f64))/((self.unit.skill_cost as f64)+1.0)/ (self.unit.attack_interval as f64) * self.unit.attack_speed / 100.0 + ele_dps;
        }
        if (self.unit.skill_index as f64) == 2.0 {
        let mut skill_factor = self.unit.skill_parameters.get(0).copied().unwrap_or(0.0);
        let mut artsdmg = ((final_atk * skill_factor * (1.0 -res/ 100.0)) as f64).max((final_atk * skill_factor * 0.05) as f64);
        dps = 12.0 * artsdmg / 25.0;
        if self.unit.skill_damage {
        ele_dps = 6000.0 /(10.0 + (ele_gauge / (final_atk * nerv_factor * mod_factor / (self.unit.attack_interval as f64) * (self.unit.attack_speed+self.unit.skill_parameters.get(7).copied().unwrap_or(0.0)) / 100.0)));
        dps += hitdmg / (self.unit.attack_interval as f64) * (self.unit.attack_speed+self.unit.skill_parameters.get(7).copied().unwrap_or(0.0)) / 100.0 + ele_dps;
        } else {
        ...
        ele_dps = 6000.0 /(6.666 + (ele_gauge / (final_atk * nerv_factor * mod_factor / (self.unit.attack_interval as f64) * self.unit.attack_speed / 100.0 + final_atk * 0.1 * mod_factor)));
        dps = hitdmg / (self.unit.attack_interval as f64) * self.unit.attack_speed / 100.0 + ele_dps;
        }
        return dps;
    }
