...
impl Tragodia {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        skilldmg = ((final_atk * self.unit.skill_parameters[0] * (1.0 -res/ 100.0)) as f64).max((final_atk * self.unit.skill_parameters[0] * 0.05) as f64) * 2.0;
        let mut nerv_dps = (final_atk * nerv_factor * mod_factor * (self.unit.skill_cost as f64) + 2.0 * final_atk * nerv_factor * mod_factor * self.unit.skill_parameters[1])/((self.unit.skill_cost as f64)+1.0)/ (self.unit.attack_interval as f64) * self.unit.attack_speed  / 100.0;
        ...
        ele_dps = 6000.0 /(10.0 + (ele_gauge / (final_atk * nerv_factor * mod_factor / (self.unit.attack_interval as f64) * (self.unit.attack_speed+self.unit.skill_parameters[7]) / 100.0)));
        dps += hitdmg / (self.unit.attack_interval as f64) * (self.unit.attack_speed+self.unit.skill_parameters[7]) / 100.0 + ele_dps;
        } else {
