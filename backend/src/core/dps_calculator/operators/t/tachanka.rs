...
impl Tachanka {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut dmg_scale = if ((self.unit.module_index as f64) as f64) == 1.0 && self.unit.module_damage { 1.1 } else { 1.0 };
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat;
        if (self.unit.skill_index as f64) == 0.0 {
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        dps = 2.0 * hitdmg / (self.unit.attack_interval as f64) * self.unit.attack_speed / 100.0;
        }
        if (self.unit.skill_index as f64) == 1.0 {
        skill_scale = self.unit.skill_parameters.get(1).copied().unwrap_or(0.0);
        let mut newdef = if self.unit.skill_damage { ((0) as f64).max((defense - self.unit.skill_parameters.get(2).copied().unwrap_or(0.0)) as f64) } else { defense };
        hitdmg = ((final_atk - newdef) as f64).max((final_atk * 0.05) as f64);
        hitdmgarts = ((final_atk * skill_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * skill_scale * 0.05) as f64) * dmg_scale;
        dps = 2.0 * hitdmg / (self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0 + hitdmgarts * (self.unit.targets as f64);
        }
        if (self.unit.skill_index as f64) == 2.0 {
        atk_interval = (self.unit.attack_interval as f64) * 0.15;
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        critdmg = ((final_atk * self.unit.skill_parameters.get(2).copied().unwrap_or(0.0) - defense) as f64).max((final_atk * self.unit.skill_parameters.get(2).copied().unwrap_or(0.0) * 0.05) as f64);
        avghit = critdmg * self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) + hitdmg * (1.0 -self.unit.skill_parameters.get(1).copied().unwrap_or(0.0));
...
impl DpsCalculator for Tachanka {
