...
/// Totter operator implementation
pub struct Totter {
...
impl Totter {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent", "vsInvis", false, &[], &[], 0, 0), ("module", "vsHeavy", false, &[], &[1], 0, 0)];

    /// Creates a new Totter operator
    #[allow(unused_parens)]
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut skillhitdmg = ((final_atk * atk_scale * skill_scale - defense) as f64).max((final_atk* atk_scale * skill_scale * 0.05) as f64) * (((self.unit.targets as f64)) as f64).min((2) as f64);
        ...
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * atk_scale - defense) as f64).max((final_atk * atk_scale * 0.05) as f64);
        if (self.unit.targets as f64) == 1.0 { hitdmg = ((final_atk * skill_scale *  atk_scale - defense) as f64).max((final_atk * skill_scale * atk_scale * 0.05) as f64); }
...
impl std::ops::DerefMut for Totter {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.unit
    }
}

impl DpsCalculator for Totter {
