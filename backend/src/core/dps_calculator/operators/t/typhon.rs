...
impl Typhon {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent2", "noCrits", true, &[], &[], 0, 0), ("module", "vsHeavy", false, &[], &[1], 0, 0), ("module", "maxRange", false, &[], &[2], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        if (self.unit.module_index as f64) == 2.0 && self.unit.module_damage { atk_scale = 1.12; }
        let mut crit_scale = if self.unit.talent2_damage && ((self.unit.elite as f64) as f64) == 2.0 { self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0) } else { 1.0 };
        let mut def_ignore = if ((self.unit.elite as f64) as f64) == 0.0 { 0.0 } else { 5.0 * self.unit.talent1_parameters.get(1).copied().unwrap_or(0.0) };
        if (self.unit.skill_index as f64) < 2.0 {
        atkbuff = self.unit.skill_parameters.get(0).copied().unwrap_or(0.0) * (self.unit.skill_index as f64);
        aspd = self.unit.skill_parameters.get(1).copied().unwrap_or(0.0) * (self.unit.skill_index as f64);
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * atk_scale * crit_scale - defense*(1.0 -def_ignore)) as f64).max((final_atk * atk_scale * crit_scale * 0.05) as f64);
        dps = hitdmg/atk_interval * (self.unit.attack_speed+aspd)/ 100.0;
        }
        if (self.unit.skill_index as f64) == 2.0 {
        atkbuff = self.unit.skill_parameters.get(0).copied().unwrap_or(0.0);
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        hitdmg = ((final_atk * atk_scale - defense*(1.0 -def_ignore)) as f64).max((final_atk * atk_scale * 0.05) as f64);
        critdmg = ((final_atk * atk_scale * crit_scale - defense*(1.0 -def_ignore)) as f64).max((final_atk * atk_scale * crit_scale * 0.05) as f64);
        if (self.unit.targets as f64) == 1.0 && (self.unit.module_index as f64) != 2.0 { dps = (hitdmg+critdmg)/atk_interval * self.unit.attack_speed/ 100.0; }
        else { dps = 2.0 * critdmg/atk_interval * self.unit.attack_speed/ 100.0; }
        }
        ...
        if self.unit.talent2_damage && (self.unit.module_index as f64) == 2.0 && (self.unit.module_level as f64) > 1.0 {
