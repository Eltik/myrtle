...
impl Typhon {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut crit_scale = if self.unit.talent2_damage && ((self.unit.elite as f64) as f64) == 2.0 { self.unit.talent2_parameters[0] } else { 1.0 };
        let mut def_ignore = if ((self.unit.elite as f64) as f64) == 0.0 { 0.0 } else { 5.0 * self.unit.talent1_parameters[1] };
        ...
        if (self.unit.targets as f64) == 1.0 && (self.unit.module_index as f64) != 2.0 { dps = (hitdmg+critdmg)/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0; }
        else { dps = 2.0 * critdmg/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0; }
        }
