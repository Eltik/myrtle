...
impl TexasAlter {
    ...
    pub const CONDITIONALS: &'static [ConditionalTuple] = &[("talent2", "preKill", false, &[], &[], 2, 0), ("module", "adjacentAlly", true, &[], &[2], 0, 0)];
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        aspd = if ((self.unit.elite as f64) as f64) == 2.0 && self.unit.talent2_damage { self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        atkbuff = if ((self.unit.elite as f64) as f64) > 0.0 { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        ...
        if (self.unit.module_index as f64) == 2.0 && !self.unit.module_damage { atkbuff += 0.1; }
        final_atk = self.unit.atk * (1.0 + atkbuff + self.unit.buff_atk) + self.unit.buff_atk_flat;
        if (self.unit.skill_index as f64) < 2.0 {
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        let mut artsdmg = if ((self.unit.skill_index as f64) as f64) == 1.0 { self.unit.skill_parameters.get(2).copied().unwrap_or(0.0) } else { 0.0 };
        ...
        let mut skillscale = self.unit.skill_parameters.get(0).copied().unwrap_or(0.0);
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        ...
        dps += hitdmgarts * (((self.unit.targets as f64)) as f64).min((self.unit.skill_parameters.get(2).copied().unwrap_or(0.0)) as f64);
        }
        return dps;
    }
}
