...
impl Tomimi {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        atkbuff = if ((self.unit.skill_index as f64) as f64) > 0.0 { self.unit.talent1_parameters.get(0).copied().unwrap_or(0.0) } else { 0.0 };
        ...
        dps = hitdmg/(self.unit.attack_interval as f64) * (self.unit.attack_speed + self.unit.skill_parameters.get(0).copied().unwrap_or(0.0)) / 100.0;
        }
        ...
        critdmg = ((final_atk * atk_scale - defense) as f64).max((final_atk * atk_scale * 0.05) as f64);
        ...
        dps = (avgnormal + avgstun + avgcrit + avgaoe)/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0;
        }
        return dps;
    }
}
