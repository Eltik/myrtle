...
impl TinMan {
    ...
    pub fn skill_dps(&self, enemy: &EnemyStats) -> f64 {
        ...
        let mut dmg_bonus = if ((self.unit.elite as f64) as f64) == 2.0 { self.unit.talent2_parameters.get(0).copied().unwrap_or(0.0) } else { 1.0 };
        let mut duration = self.unit.skill_parameters.get(0).copied().unwrap_or(0.0);
        skill_scale = self.unit.skill_parameters.get(2).copied().unwrap_or(0.0);
        final_atk = self.unit.atk * (1.0 + self.unit.buff_atk) + self.unit.buff_atk_flat;
        skilldmg = ((final_atk * skill_scale * (1.0 -res/ 100.0)) as f64).max((final_atk * skill_scale * 0.05) as f64) * dmg_bonus * (self.unit.targets as f64);
        hitdmg = ((final_atk - defense) as f64).max((final_atk * 0.05) as f64);
        if (self.unit.skill_index as f64) == 0.0 { return hitdmg/(self.unit.attack_interval as f64) * self.unit.attack_speed/ 100.0; }
        if (self.unit.skill_index as f64) == 1.0 {
        sp_cost = (self.unit.skill_cost as f64);
        dps = sp_cost/(sp_cost + 1.0) * hitdmg / (self.unit.attack_interval as f64) * (self.unit.attack_speed) / 100.0;
        dps += duration * skilldmg / ((sp_cost + 1.0) * (self.unit.attack_interval as f64)/self.unit.attack_speed*100.0);
        }
