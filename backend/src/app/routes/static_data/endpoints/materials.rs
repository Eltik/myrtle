...
pub async fn get_all_materials(
    ...
    Query(params): Query<MaterialQuery>,
    headers: HeaderMap,
    ...
    let cache_key = format!(
        "static:materials:all:limit:{}:cursor:{}:fields:{}",
        limit,
    ...
    cached_handler(
        ...
        &headers,
        || async {
            let all_materials = state.game_data.materials.items.values().collect::<Vec<_>>();
...
pub async fn get_material_by_id(
    ...
    cached_handler(
        ...
        &headers,
        || async {
            state.game_data.materials.items.get(&id).map(|mat| {
                if let Some(ref field_set) = field_set {
                    serde_json::json!({ "material": filter_fields(mat, field_set) })
                } else {
