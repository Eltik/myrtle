...
pub async fn get_all_ranges(
    ...
    let cache_key = format!(
        "static:ranges:all:limit:{}:cursor:{}:fields:{}",
        limit,
    ...
    cached_handler(
        ...
        &headers,
        || async {
            let all_ranges = state.game_data.ranges.values().collect::<Vec<_>>();
...
pub async fn get_range_by_id(
    ...
    cached_handler(
        ...
        &headers,
        || async {
            state.game_data.ranges.get(&id).map(|range| {
                if let Some(ref field_set) = field_set {
                    serde_json::json!({ "range": filter_fields(range, field_set) })
                } else {
