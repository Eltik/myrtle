...
impl Mode {
    const fn py_source_rel(&self) -> &'static str {
        match self {
    ...
    /// `Operator` or `Healer`).
    const fn class_base_alt(&self) -> &'static str {
        match self {
    ...
    const fn method_names(&self) -> &'static [&'static str] {
        match self {
    ...
    const fn formulas_json_path(&self) -> &'static str {
        match self {
    ...
    const fn generated_rs_path(&self) -> &'static str {
        match self {
    ...
    const fn dispatch_rs_path(&self) -> &'static str {
        match self {
    ...
    const fn generated_mod_name(&self) -> &'static str {
        match self {
    ...
    const fn expected_json_path(&self) -> &'static str {
        match self {
    ...
    const fn output_shape(&self) -> OutputShape {
        match self {
...
fn make_test_key(
    ...
    let base = format!("{name}_s{skill}_m{module}_{def:.0}_{res:.0}");
...
fn generate_expected_hps(repo_path: &str, formulas: &HashMap<String, OperatorFormula>) {
    ...
    // Python script: trace skill_hps() and read the (skill_hps, base_hps, avg_hps)
    // triple from its locals at return.
    let py_script = format!(
        r#"#!/usr/bin/env python3
...
fn transpile_all(mode: Mode, py_src: &str, formulas: &HashMap<String, OperatorFormula>) {
    ...
    // Generated code is transpiled, not hand-written - silence rustc style lints
    // and all clippy groups so the linter ignores this file entirely.
    generated.push_str("#![allow(unused_variables, unused_mut, unused_assignments, unused_parens, unreachable_code, path_statements, unused_must_use, non_snake_case, clippy::all, clippy::pedantic, clippy::nursery)]\n\n");
...
fn build_dispatch_mod(
    ...
    out.push_str("#![allow(clippy::all, clippy::pedantic, clippy::nursery)]\n\n");
    match shape {
        OutputShape::F64Dps => {
    ...
    for line in init_body.lines() {
        ...
        for &(py_field, rust_field, _field_type) in field_map {
            ...
            if let Some(rl) = rust_line {
                // untranspilable: compound bool conditions reference several shadow vars,
                // the rest are shapes the mini-transpiler cannot carry
...
fn transpile_skill_dps(
            ...
            && let Some(prev) = lines.last_mut()
        {
            ...
            if pt.ends_with('}') && pt.contains("if ") {
                let else_part = rust_line
                    .trim_start()
                    .strip_prefix("}")
                    .unwrap_or_else(|| rust_line.trim_start());
    ...
    // Close every brace still open before Some(dps): counted from the body text, not from indent_stack.
...
fn transpile_line(py: &str, declared: &mut std::collections::HashSet<String>) -> String {
    ...
    if let Some(cap) = RE.inline_if_re.captures(&line) {
        ...
        } else if let Some(tc) = RE.inner_ternary.captures(&val) {
...
fn transpile_expressions(line: &str, declared: &mut std::collections::HashSet<String>) -> String {
    ...
    // Standalone self.talentN_params (no index): must come AFTER the indexed regex replacements above
