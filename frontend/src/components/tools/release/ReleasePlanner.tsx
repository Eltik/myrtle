import { CalendarClock, CalendarDays, ChartGantt, ChevronRight, Shirt, Ticket } from "lucide-react";
...
type ReleaseView = "events" | "skins" | "banners" | "timeline" | "calendar";

export function ReleasePlanner(): React.ReactElement {
    const today = React.useMemo(() => new Date(), []);
    const [view, setView] = React.useState<ReleaseView>("events");
        ...
        <div className="relative z-1 mx-auto w-[min(1200px,calc(100%-2rem))] py-5 pb-20" translate="no">
            ...
            <div className="flex flex-wrap items-end justify-between gap-3">
                <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1.5 max-w-2xl font-sans text-[13.5px] text-muted-foreground leading-normal">When CN content lands on EN. Confirmed rows come from EN gamedata, announced rows from a manual override, and estimated rows from the trailing CN to EN lag with a band.</p>
                </div>
            </div>
