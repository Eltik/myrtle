    ...
    return (
        <div className="flex flex-wrap items-center gap-2">
            ...
            {selectedTags.map((tag) => {
                ...
                return (
                    <span className={cn("inline-flex items-center gap-1 rounded-md px-2 py-1 font-medium text-xs", isHighPriority ? "bg-amber-500/20 text-amber-400" : "bg-primary/20 text-foreground")} key={tag.id}>
                        {tag.name}
                        <button className="rounded-full p-0.5 transition-colors hover:bg-foreground/10" onClick={() => onRemoveTag(tag.id)} type="button">
                            <X className="h-3 w-3" />
                        </button>
                    </span>
                );
            ...
            {selectedTagIds.length > 1 && (
                <button className="text-muted-foreground text-xs transition-colors hover:text-foreground" onClick={onClearAll} type="button">
                    Clear all
                </button>
            )}
        </div>
