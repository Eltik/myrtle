...
import type { messages } from "./SquadSlab.messages";

type SquadT = TypedT<typeof messages>;
...
export function SquadSlab({ operators, squadSize, onReroll }: ISquadSlabProps): React.ReactElement {
    const t: SquadT = useT("tools");
    const filled = operators.length;

    return (
        <SlabFrame index="02" kicker={t("randomizer.squad.kicker")} accent="lagoon">
            <div className="flex items-start justify-between gap-3">
                <div className="flex min-w-0 flex-1 items-center gap-2 text-[11px] text-muted-foreground uppercase tracking-[0.18em]">
                    ...
                    <span>
                        {t("randomizer.squad.deployment")} <span className="font-mono text-foreground/80 normal-case tracking-normal">{t("randomizer.squad.count", { filled, size: squadSize })}</span>
                    </span>
                </div>
                <Button onClick={onReroll} size="xs" variant="outline" aria-label={t("randomizer.squad.rerollAria")}>
                    <RotateCcw aria-hidden="true" />
                    {t("randomizer.squad.reroll")}
                </Button>
            </div>
            ...
            <div className="mt-4 grid grid-cols-4 gap-1.5 sm:grid-cols-6 sm:gap-2 md:grid-cols-8 xl:grid-cols-12">
                {operators.map((op, idx) => (
                    ...
                    <OperatorTile key={`${op.id}-${idx}`} op={op} index={idx} t={t} />
                ))}
                {filled === 0 && <p className="col-span-full font-mono text-muted-foreground text-sm">{t("randomizer.squad.empty")}</p>}
            </div>
        </SlabFrame>
    );
}

function OperatorTile({ op, index, t }: { op: IRandomizerOperator; index: number; t: SquadT }) {
    ...
    return (
        <Link
            to="/operators/$id"
            params={{ id: op.id }}
            aria-label={t("randomizer.squad.viewOperator", { name: op.name })}
