...
export function EnemyPanel({ enemy, hydrationToken, onChangeEnemy, onChangeShred }: IEnemyPanelProps): React.ReactElement {
    ...
    return (
        <Card>
            <CardHeader>
                <CardTitle className="text-[15px]">{t("dps.enemy.title")}</CardTitle>
                <p className="text-[12px] text-muted-foreground">{t("dps.enemy.desc")}</p>
            </CardHeader>
            <CardPanel className="space-y-3 pt-0">
                <div>
                    <Label className="mb-1.5 block font-medium text-[11px] text-muted-foreground leading-none">{t("dps.enemy.presets")}</Label>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <NumField key={`def-${hydrationToken}-${presetVersion}`} label={t("dps.enemy.snapshotDef")} value={enemy.defense} min={0} max={5000} step={100} onChange={(v) => onChangeEnemy({ defense: v })} />
                    <NumField key={`res-${hydrationToken}-${presetVersion}`} label={t("dps.enemy.snapshotRes")} value={enemy.res} min={0} max={100} step={5} onChange={(v) => onChangeEnemy({ res: v })} />
                    <NumField key={`targets-${hydrationToken}-${presetVersion}`} label={t("dps.enemy.targets")} value={enemy.targets} min={1} max={20} step={1} onChange={(v) => onChangeEnemy({ targets: Math.max(1, v) })} />
                    <NumField key={`sp-${hydrationToken}`} label={t("dps.enemy.spBoost")} value={enemy.spBoost} min={0} max={5} step={0.1} onChange={(v) => onChangeEnemy({ spBoost: v })} />
                </div>
                ...
                <Collapsible open={shredOpen} onOpenChange={setShredOpen}>
                    <button type="button" onClick={() => setShredOpen((v) => !v)} className="flex w-full cursor-pointer items-center justify-between rounded py-1 text-left font-medium text-[11.5px] text-muted-foreground hover:text-foreground">
                        <span className="flex items-center gap-1.5">
                            {t("dps.enemy.shred")}
                            {hasShred && <span className="rounded bg-primary/15 px-1.5 py-0.5 font-mono text-[9.5px] text-primary">{t("dps.enemy.shred.on")}</span>}
                        </span>
                        {shredOpen ? <ChevronUp className="size-3.5" /> : <ChevronDown className="size-3.5" />}
                    </button>
                    <CollapsibleContent>
                        <div className="mt-2 grid grid-cols-2 gap-3">
                            <NumField key={`shred-def-${hydrationToken}`} label={t("dps.enemy.shred.defPct")} value={enemy.shred.def ?? 0} min={0} max={100} step={5} onChange={(v) => onChangeShred({ def: v })} compact />
                            <NumField key={`shred-defFlat-${hydrationToken}`} label={t("dps.enemy.shred.defFlat")} value={enemy.shred.defFlat ?? 0} min={0} max={3000} step={50} onChange={(v) => onChangeShred({ defFlat: v })} compact />
                            <NumField key={`shred-res-${hydrationToken}`} label={t("dps.enemy.shred.resPct")} value={enemy.shred.res ?? 0} min={0} max={100} step={5} onChange={(v) => onChangeShred({ res: v })} compact />
                            <NumField key={`shred-resFlat-${hydrationToken}`} label={t("dps.enemy.shred.resFlat")} value={enemy.shred.resFlat ?? 0} min={0} max={100} step={5} onChange={(v) => onChangeShred({ resFlat: v })} compact />
                        </div>
                    </CollapsibleContent>
                </Collapsible>
            </CardPanel>
