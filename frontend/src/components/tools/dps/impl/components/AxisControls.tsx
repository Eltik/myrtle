        ...
        <div className="flex flex-wrap items-end gap-3">
            ...
            <div className="ml-auto flex flex-col items-stretch gap-1">
                <div className="flex items-end gap-2">
                    <NumberField key={`${xAxis}-min-${hydrationToken}`} defaultValue={sweep.min} min={0} max={sweep.max - 1} step={sweepStep} onValueChange={(v) => onChangeSweep({ min: v ?? 0 })} size="sm" className="w-24">
                    ...
                    <NumberField key={`${xAxis}-max-${hydrationToken}`} defaultValue={sweep.max} min={sweep.min + 1} max={sweepMaxBound} step={sweepStep} onValueChange={(v) => onChangeSweep({ max: v ?? 0 })} size="sm" className="w-24">
                ...
                <p className="text-right font-mono text-[10.5px] text-muted-foreground/80">
