...
function KpiRow({ inst, snap, index, isLeader, leaderValue, yMetric, sameOpCount }: KpiRowProps): React.ReactElement {
    ...
    return (
        <div className={`flex items-center gap-3 rounded-lg border border-border bg-card px-3 py-2.5 transition-opacity ${dim ? "opacity-50" : ""}`}>
...
function Stat({ label, value, highlight, loading }: { label: string; value: number | undefined; highlight: boolean; loading: boolean }): React.ReactElement {
    return (
        <div className="min-w-14">
            ...
            <div className={`flex items-center justify-end gap-1 font-mono text-[14px] tabular-nums ${highlight ? "text-primary" : "text-foreground"}`}>{loading && value === undefined ? <Spinner className="size-3 text-muted-foreground" /> : formatLargeNumber(value)}</div>
        </div>
    );
}
