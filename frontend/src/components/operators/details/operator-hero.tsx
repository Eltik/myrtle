    ...
    return (
        <div className="relative h-[450px] w-full overflow-hidden md:h-[550px]">
            ...
            <motion.div animate={{ scale: 1, opacity: 1 }} className="absolute inset-0" initial={{ scale: 1.1, opacity: 0 }} transition={{ duration: 0.8, ease: "easeOut" }}>
                <Image alt={operator.name} className="object-cover object-center" fill priority src={imageUrl || "/placeholder.svg"} />
            </motion.div>
            ...
            <div
                ...
                style={{
                    background: "linear-gradient(180deg, rgba(0,0,0,0) 0%, rgba(0,0,0,0.4) 60%, hsl(var(--background)) 100%)",
                }}
            />
            <div
                className="absolute inset-0 z-10"
                style={{
                    background: "linear-gradient(90deg, hsl(var(--background) / 0.6) 0%, transparent 50%)",
                }}
            />

            {/* Content - adjusted positioning */}
            <div className="container relative z-20 mx-auto flex h-full flex-col justify-end px-4 pb-24">
                <motion.div animate={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 20 }} transition={{ delay: 0.3, duration: 0.5 }}>
                    <Breadcrumb className="mb-4">
                        <BreadcrumbList>
                            <BreadcrumbItem>
                                <BreadcrumbLink className="text-muted-foreground hover:text-foreground" href="/operators/list">
                                    Operators
                                </BreadcrumbLink>
                            </BreadcrumbItem>
                            <BreadcrumbSeparator />
                ...
                <motion.div animate={{ opacity: 1, scale: 1 }} className="absolute right-4 bottom-20 hidden md:right-8 lg:block" initial={{ opacity: 0, scale: 0.9 }} transition={{ delay: 0.5, duration: 0.5 }}>
