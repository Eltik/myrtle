...
export default function TierLists({ onSelect }: { onSelect: (op: Operator) => void }) {
    ...
    return (
        <section className="page-wrap my-20">
            <div className="mb-5.5">
                <div className="flex flex-wrap items-end justify-between gap-4">
                    <div>
                        ...
                        <p className="mt-1.5 max-w-[520px] font-sans text-sm leading-normal text-muted-foreground">Previews from the 24 most-watched lists. Click any card to open the full ranking.</p>
                    </div>
                    <div role="tablist" className="inline-flex gap-0.5 rounded-[10px] border border-border bg-muted p-[3px]">
                        {filters.map((f) => {
            ...
            <a href="#" className="mt-4.5 inline-flex w-max cursor-pointer items-center gap-2 rounded-lg border border-dashed border-border bg-transparent px-3.5 py-2.5 font-sans text-[12.5px] font-medium leading-none text-muted-foreground no-underline transition-colors hover:border-primary hover:bg-[color-mix(in_srgb,var(--primary)_5%,transparent)] hover:text-foreground">
