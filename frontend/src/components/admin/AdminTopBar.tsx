...
import type { messages } from "./AdminTopBar.messages";
    ...
    return (
        <div className="sticky top-0 z-30 border-border border-b bg-background/80 backdrop-blur-lg backdrop-saturate-150 supports-backdrop-filter:bg-background/60">
            <div className="flex h-14 items-center gap-2 px-3 sm:gap-3.5 sm:px-4 lg:px-6">
                <button type="button" aria-label={t("topbar.openMenu")} onClick={onOpenSidebar} className="-ml-1 inline-flex size-8 shrink-0 cursor-pointer items-center justify-center rounded-lg border border-transparent text-foreground hover:bg-accent lg:hidden">
                    <MenuIcon className="size-4.5 opacity-85" strokeWidth={1.9} />
                </button>

                <nav aria-label={t("topbar.breadcrumb")} className="hidden min-w-0 flex-1 items-center gap-1.5 font-medium text-[13px] sm:flex">
                    {crumbs.map((c, i) => (
                ...
                <button
                    ...
                    className="inline-flex h-8 shrink-0 cursor-pointer items-center justify-center rounded-lg border border-input bg-card px-2 font-normal text-[13px] text-muted-foreground hover:bg-accent sm:h-7.5 sm:gap-2 sm:px-2.5 md:min-w-60 lg:min-w-70"
                >
                    <SearchIcon className="size-3.5" strokeWidth={1.9} />
                    <span className="hidden flex-1 text-left sm:inline">{t("topbar.searchPlaceholder")}</span>
                ...
                <div className="flex items-center gap-1 sm:gap-2">
                    <button type="button" className="inline-flex size-8 cursor-pointer items-center justify-center rounded-lg border border-transparent text-foreground hover:bg-accent sm:size-7.5" aria-label={t("topbar.notifications")}>
                        <BellIcon className="size-4 opacity-85" strokeWidth={1.9} />
                    </button>
                    <a href={REPO_URL} target="_blank" rel="noreferrer" className="inline-flex size-8 items-center justify-center rounded-lg border border-transparent text-foreground opacity-85 hover:bg-accent sm:size-7.5" aria-label={t("topbar.github")}>
