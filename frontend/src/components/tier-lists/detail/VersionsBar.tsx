        ...
        <section aria-label="Tier list versions" className="mx-auto mt-4 w-[min(1200px,calc(100%-1.5rem))] sm:mt-6 sm:w-[min(1200px,calc(100%-2rem))]">
            <div className="flex flex-wrap items-center gap-2 rounded-xl border border-border bg-card/60 px-3 py-2 backdrop-blur-sm">
                ...
                <Menu>
                    ...
                    <MenuPopup align="start" sideOffset={6} className="min-w-72">
                        <MenuItem render={<Link to="/tier-lists/$id" params={{ id: slug }} search={{}} replace resetScroll={false} />} className={cn("flex-col items-start gap-0.5 py-2", isLatestView && "bg-accent/60 text-accent-foreground")}>
                        ...
                        {versions.map((v) => {
                            const selected = !isLatestView && selectedVersion?.id === v.id;
                            return (
                                <MenuItem key={v.id} render={<Link to="/tier-lists/$id" params={{ id: slug }} search={{ v: v.version }} replace resetScroll={false} />} className={cn("flex-col items-start gap-0.5 py-2", selected && "bg-accent/60 text-accent-foreground")}>
                                    <span className="flex w-full items-center gap-2">
                                        ...
                                        <span className="font-mono text-[10.5px] text-muted-foreground">{formatRelative(v.publishedAt)}</span>
                                        {selected && <CheckIcon className="ml-auto h-3.5 w-3.5" aria-hidden="true" />}
                                    </span>
                                    {v.changelog && <span className="line-clamp-2 pl-3.5 font-sans text-[11.5px] text-muted-foreground leading-snug">{v.changelog}</span>}
                                </MenuItem>
                            );
                        })}
                    </MenuPopup>
            ...
            {!isLatestView && selectedVersion && (
                <output aria-live="polite" className="mt-2 block rounded-xl border border-[color-mix(in_srgb,var(--primary)_30%,transparent)] bg-[color-mix(in_srgb,var(--primary)_8%,transparent)] px-3 py-2.5">
                    ...
                    {selectedVersion.changelog && <p className="m-0 mt-1.5 font-sans text-[13px] text-foreground leading-relaxed">{selectedVersion.changelog}</p>}
                </output>
            )}
        </section>
    );
