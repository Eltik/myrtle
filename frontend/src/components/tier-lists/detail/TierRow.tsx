    ...
    return (
        ...
        >
            <HoverCard>
                <HoverCardTrigger
                    render={
                        <button type="button" onClick={openDialog} className={styles.tierLabel} id={labelledById} data-size={tierLabelSize(tier.name)} aria-label={hasDescription ? t("detail.tier.labelWithDescription", { name: tier.name, description: tier.description }) : t("detail.tier.label", { name: tier.name })}>
...
function TierHoverCard({ tier, color, totalOps, hasDescription, onOpen }: IHoverProps) {
    ...
    return (
        <button type="button" onClick={onOpen} className="group flex w-full cursor-pointer flex-col bg-popover text-left no-underline transition-colors" aria-label={t("detail.tier.openDetails", { name: tier.name })}>
            <div className="relative flex items-center gap-3.5 border-border border-b px-4 pt-4 pb-3.5" style={{ background: `linear-gradient(135deg, color-mix(in srgb, ${color} 14%, transparent), transparent 70%)` }}>
                ...
                <div className="min-w-0 flex-1">
                    <p className="m-0 font-bold font-mono text-[10px] text-muted-foreground/80 uppercase leading-none tracking-[0.18em]">{t("detail.tier.kicker")}</p>
            ...
            <div className="flex flex-col gap-3.5 px-4 py-3.5">
                {hasDescription ? (
                    ...
                    <p className="m-0 font-sans text-[12.5px] text-muted-foreground italic leading-[1.55]">{t("detail.tier.noDescription")}</p>
                )}
            ...
            <div className="flex items-center justify-between gap-2 border-border border-t bg-muted/60 px-4 py-2.5 font-bold font-sans text-[10.5px] text-muted-foreground uppercase leading-none tracking-[0.14em] transition-colors group-hover:bg-[color-mix(in_srgb,var(--ring)_10%,transparent)] group-hover:text-foreground">
                <span>{t("detail.tier.viewDetails")}</span>
