        ...
        <div className={`${PANEL_PADDING} flex flex-col gap-5`}>
            {base.layout.length > 0 && (
                <div className="flex flex-col gap-2">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                        {base.layout.map((l) => (
                            <span key={l.room_type} className="flex items-center gap-1.5 rounded-md border border-border/40 bg-muted/15 px-2 py-1 text-[11px]">
                                <span className="font-medium">{roomLabel(l.room_type)}</span>
                                <span className="font-mono text-[10px] tabular-nums text-muted-foreground">×{l.count}</span>
                                <span className="font-mono text-[10px] tabular-nums text-muted-foreground">L{l.levels.join(", L")}</span>
                            </span>
                        ))}
                    </div>
                </div>
            ...
            {base.rotation && (
                <div className="flex flex-col gap-3">
                    ...
                    <p className="text-[11px] text-muted-foreground">Average of shift A and shift B. This is what your base score is computed against — run operators on these rotations to hit it.</p>
            ...
            {base.optimal && (
                <div className="flex flex-col gap-3">
                    ...
                    <p className="text-[11px] text-muted-foreground">Best possible arrangement at one snapshot in time — useful for set-and-forget AFK runs.</p>
                    <div className="flex flex-col gap-2">
    ...
    return (
        <div className="flex flex-col gap-2 rounded-lg border border-border/40 bg-muted/10 p-3">
            <div className="flex items-center justify-between">
                <span className="font-mono text-[10.5px] font-semibold uppercase tracking-wider" style={{ color: `color-mix(in oklch, ${accent} 60%, var(--foreground))` }}>
                    {title}
                </span>
...
function RoomRow({ room, accent, compact }: { room: IRoomAssignment; accent: string; compact?: boolean }) {
    ...
    return (
        <div className={`flex items-center gap-2 rounded-md border border-border/35 bg-muted/15 ${compact ? "px-2 py-1.5" : "px-2.5 py-2"}`}>
            <div className="flex min-w-[7rem] flex-col">
                <div className="flex items-center gap-1.5">
                    <span className="font-mono text-[11px] font-semibold">{label}</span>
                    <span className="rounded-sm border border-border/40 bg-background px-1 py-0.5 font-mono text-[9px] tabular-nums text-muted-foreground">L{room.level}</span>
                </div>
            ...
            <div className="flex flex-1 flex-wrap gap-1">
                {room.operators.length === 0 && <span className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground/60">No assignment</span>}
                        ...
                        <TooltipTrigger
                            render={
                                <span className="flex items-center gap-1.5 rounded-md border border-border/40 bg-background/65 px-1.5 py-0.5">
                                    ...
                                    <span className="max-w-[11ch] truncate text-[11px] font-medium">{op.name}</span>
                                </span>
                            }
                        />
                        <TooltipContent sideOffset={4}>
            ...
            <span className="shrink-0 font-mono text-[11px] font-semibold tabular-nums" style={{ color: `color-mix(in oklch, ${accent} 65%, var(--foreground))` }}>
