...
import type { messages } from "./ImprovementsPanel.messages";
    ...
    if (!improvements) {
        return (
            <div className={PANEL_PADDING}>
                <p className="rounded-md border border-border/40 border-dashed bg-muted/15 px-3 py-2 text-center text-[11px] text-muted-foreground">{t("score.improvements.unavailable")}</p>
            </div>
        );
    }
    switch (sub.key) {
