            ...
            <DialogTrigger
                render={
                    <Card
                        ...
                        className="fade-in slide-in-from-bottom-4 group relative rounded-xl! animate-in cursor-pointer overflow-hidden text-left transition-all duration-200 hover:-translate-y-0.5 hover:border-foreground/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            ...
            >
                ...
                <div className="relative flex items-start justify-between gap-2 px-4 pt-3.5 pb-1">
                    <div className="flex min-w-0 items-center gap-2">
                        ...
                        <span className="truncate font-mono font-semibold text-[10.5px] uppercase tracking-[0.12em] text-muted-foreground">{CATEGORY_ITEM_LABELS[item.category]}</span>
                    </div>
                ...
                <h3 className="relative px-4 pb-3 font-semibold text-[15.5px] leading-tight tracking-tight text-foreground">{item.name}</h3>

                <div className="relative flex items-center gap-3.5 px-4 pb-3">
                    <ItemIcon item={item} size={64} />
                    <div className="flex min-w-0 flex-1 flex-col gap-0.5">
                        <span className="font-mono font-semibold text-[10.5px] uppercase tracking-[0.12em] text-muted-foreground">Quantity</span>
                        <span className="font-bold tabular-nums leading-none" style={{ fontSize: "1.875rem", letterSpacing: "-0.02em" }}>
                            {item.quantity.toLocaleString()}
                        </span>
                        {item.meta?.usage && <span className="mt-1 line-clamp-2 text-[11.5px] leading-tight text-muted-foreground">{item.meta.usage}</span>}
                    </div>
                </div>

                {obtain.length > 0 && (
                    <div className="relative flex items-center gap-2 border-t border-border px-4 py-2.5">
                        <span className="shrink-0 font-mono font-semibold text-[10.5px] uppercase tracking-[0.12em] text-muted-foreground">Obtain</span>
                        <div className="flex min-w-0 flex-1 items-center gap-1.5 overflow-hidden">
                            ...
                            {obtain.length > 2 && <span className="shrink-0 font-mono text-[10.5px] tabular-nums text-muted-foreground">+{obtain.length - 2}</span>}
                        </div>
                    </div>
                )}
