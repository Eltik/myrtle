...
export function ServerTag({ server, className }: { server: string; className?: string }) {
    ...
    return (
        <span className={`inline-flex h-5 items-center rounded font-mono font-semibold text-[10px] uppercase leading-none tracking-[0.08em] ${tint ? "" : "bg-muted text-muted-foreground"} ${className ?? ""}`.trim()} style={{ ...style, padding: "0 6px" }}>
            {code}
        </span>
    );
}
