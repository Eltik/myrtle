    ...
    return (
        <section className="relative overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
            ...
            <div
                ...
                style={{
                    background: "radial-gradient(ellipse 80% 60% at 60% 40%, color-mix(in oklab, var(--primary) 7%, transparent), transparent 70%), radial-gradient(ellipse 60% 80% at 80% 20%, color-mix(in oklab, var(--chart-2) 6%, transparent), transparent 70%)",
                }}
            />
            ...
            <div className="relative grid gap-x-7 gap-y-4 px-5 py-5 sm:grid-cols-[1fr_auto] sm:gap-y-5 sm:px-6 sm:py-6">
                <div className="flex min-w-0 flex-col gap-1.5">
                    ...
                    <p className="m-0 max-w-[60ch] font-sans text-[13.5px] text-muted-foreground leading-snug">{t("leaderboard.hero.blurb")}</p>
                </div>

                <div className="flex items-center gap-4 self-center sm:gap-5">
                    <HeroStat label={t("leaderboard.hero.stat.ranked")} value={rankedDoctors == null ? "-" : f.compact(rankedDoctors)} loading={isLoading} />
                    <span aria-hidden className="h-10 w-px bg-border" />
                    <HeroStat label={t("leaderboard.hero.stat.topScore")} value={topScore == null ? "-" : f.compact(topScore)} loading={isLoading} />
                    <span aria-hidden className="h-10 w-px bg-border" />
                    <HeroStat label={t("leaderboard.hero.stat.updated")} value={formatRelative(updatedAt, t)} small loading={isLoading} />
                </div>
