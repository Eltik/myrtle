// forked from design-sync lib/dts.mjs — app repo ships no .d.ts tree, so props must be resolved from the .tsx sources.
//
// The bundled adapter builds its ts-morph project from `<typesRoot>/**/*.d.ts`
// and looks up a `<Name>Props` interface. This repo publishes nothing: there is
// no types field, no .d.ts anywhere, and its components declare props inline in
// shadcn style (`React.ComponentProps<"button"> & VariantProps<typeof
// buttonVariants>`). Both lookups miss, so every one of the 692 components
// emitted `{ [key: string]: unknown }` — an empty API contract for the design
// agent, which is the single most valuable artifact of the sync.
//
// Two changes, both additive — every original code path still runs first:
//   1. projectFor() teaches the project about TS/TSX sources: `#/*` path
//      mapping, `allowJs`, and the repo's tsconfig-equivalent compiler options.
//   2. propsBodyFor() gains a source-file resolution step. It reads the
//      name→source map that overrides/source-kit.mjs writes (needed because 39
//      exports are renamed to break collisions, so the emitted name is not the
//      name declared in the file), loads just that file, and reads the props
//      type off the component's first call signature.
//
// Files are added to the project on demand, never globbed: a bulk add of 330
// components plus their transitive imports makes the checker unusably slow.

import { existsSync, readFileSync, readdirSync } from 'node:fs';
import { dirname, join } from 'node:path';
import { Project, Node, ts } from 'ts-morph';

// Written by .design-sync/overrides/source-kit.mjs: { <emitted name>: { file, local } }.
const SRC_MAP_REL = '.design-sync/.cache/component-src.json';
let srcMapCache;
function componentSrcMap(pkgDir) {
  if (srcMapCache !== undefined) return srcMapCache;
  const p = join(pkgDir, SRC_MAP_REL);
  try {
    srcMapCache = JSON.parse(readFileSync(p, 'utf8'));
    console.error(`  [DTS] source map: ${Object.keys(srcMapCache).length} components → ${SRC_MAP_REL}`);
  } catch {
    srcMapCache = null;
    console.error(`  [DTS] no ${SRC_MAP_REL} — falling back to .d.ts-only prop extraction`);
  }
  return srcMapCache;
}
...
function projectFor(pkgDir, typesRoot) {
  ...
  const project = new Project({
    ...
    compilerOptions: {
      ...
      skipLibCheck: true,
      strict: false,
      // FORK: the sources this project now loads are .tsx that import through
      // the repo's `#/*` alias and pull in .css/.module.css side-effect imports.
      allowJs: true,
      baseUrl: pkgDir,
      paths: { '#/*': ['./src/*'], '@/*': ['./src/*'] },
    },
...
export function propsBodyFor(name, ctx) {
  ...
  // FORK: resolve from the component's own source file first. This is the only
  // path that produces real props in this repo — see the header.
  const fromSrc = propsFromSource(name, project, pkgDir);
...
// FORK: resolve a component's props from its .tsx source.
//
// Order matters: an explicitly declared `<Name>Props` in the file is the
// author's own contract and beats anything inferred, so it is tried first. The
// call-signature path is what actually fires for most of this repo, where props
// are written inline on the parameter.
function propsFromSource(name, project, pkgDir) {
  const map = componentSrcMap(pkgDir);
  const hit = map?.[name];
  if (!hit) return null;

  let sf;
  try {
    sf = project.addSourceFileAtPathIfExists(hit.file);
  } catch {
    return null;
  }
  if (!sf) return null;

  const declared = sf.getInterface(`${hit.local}Props`) ?? sf.getTypeAlias(`${hit.local}Props`);
  if (declared) return emitBody(declared.getType(), declared, '', pkgDir);

  const decls = sf.getExportedDeclarations().get(hit.local) ?? [];
  const decl =
    decls.find((d) => Node.isVariableDeclaration(d) || Node.isFunctionDeclaration(d) || Node.isClassDeclaration(d)) ?? decls[0];
  if (!decl || Node.isSourceFile(decl)) return null;

  // forwardRef/memo wrappers and plain function components alike expose the
  // props object as the first parameter of the first call signature.
  const sig = decl.getType().getCallSignatures()[0];
  const p0 = sig?.getParameters()[0];
  if (!p0) return null;
  const body = emitBody(p0.getTypeAtLocation(decl), decl, '', pkgDir);
  // An empty body is no better than the fallback — let the original paths try.
