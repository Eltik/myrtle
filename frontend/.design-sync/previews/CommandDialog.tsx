import { Command, CommandDialog, CommandDialogPopup, CommandDialogTrigger, CommandFooter, CommandGroup, CommandGroupLabel, CommandInput, CommandItem, CommandList, CommandPanel, CommandSeparator, Kbd, OperatorAvatar } from "frontend";
import { MapIcon, SearchIcon, TrophyIcon } from "lucide-react";
...
const PageBehind = ({ children }: { children?: React.ReactNode }) => (
    <div className="mx-auto w-full max-w-2xl">
        <div className="flex items-center justify-between border-b pb-3">
            <span className="font-semibold text-lg">Operators</span>
            {children ?? <span className="text-muted-foreground text-sm">312 operators · EN server</span>}
        </div>
...
/** Uncontrolled and open — the site's ⌘K palette. */
export const Open = () => (
    <div>
        <PageBehind />
...
/** Closed, driven by the header's search affordance. */
export const ClosedWithTrigger = () => (
    <div>
        <PageBehind>
            <CommandDialog>
                <CommandDialogTrigger className="flex items-center gap-2 rounded-lg border bg-card px-3 py-2 text-muted-foreground text-sm shadow-xs">
