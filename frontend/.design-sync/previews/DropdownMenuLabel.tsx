...
import { ChevronDownIcon, ListFilterIcon, FlaskConicalIcon } from "lucide-react";
    ...
    <div className="min-h-[60vh] w-full max-w-2xl">
        <div className="flex items-center justify-between gap-3 rounded-xl border border-border bg-card/60 px-3 py-2">
            ...
            <DropdownMenu modal={false} open>
                <DropdownMenuTrigger className="inline-flex h-8 cursor-default items-center justify-between gap-2 rounded-lg border border-border bg-popover px-2.5 font-medium font-sans text-foreground text-xs leading-none">
                    <span className="flex min-w-0 items-center gap-2">
                        <FlaskConicalIcon className="h-3.5 w-3.5 opacity-70" />
                        <span className="truncate">Tier 5 materials</span>
                    </span>
                    <ChevronDownIcon className="h-3 w-3 shrink-0 opacity-70" />
                </DropdownMenuTrigger>
                <DropdownMenuContent align="start" className="w-64" sideOffset={6}>
                    <DropdownMenuGroup>
                        <DropdownMenuLabel inset>Tier 5 — Elite</DropdownMenuLabel>
                        <DropdownMenuCheckboxItem defaultChecked>D32 Steel</DropdownMenuCheckboxItem>
                        <DropdownMenuCheckboxItem>Bipolar Nanoflake</DropdownMenuCheckboxItem>
                        <DropdownMenuCheckboxItem>Crystalline Electronic Unit</DropdownMenuCheckboxItem>
                    </DropdownMenuGroup>
                    <DropdownMenuSeparator />
                    <DropdownMenuGroup>
                        <DropdownMenuLabel inset>Tier 4 — Advanced</DropdownMenuLabel>
                        <DropdownMenuCheckboxItem defaultChecked>Grindstone Pentahydrate</DropdownMenuCheckboxItem>
                        <DropdownMenuCheckboxItem>Polymerization Preparation</DropdownMenuCheckboxItem>
                    </DropdownMenuGroup>
                </DropdownMenuContent>
            </DropdownMenu>
        </div>
