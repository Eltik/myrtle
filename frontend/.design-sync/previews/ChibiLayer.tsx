...
/** Route `d` strings straight out of `buildRouteEntries` for 4-10 — Extinguished Flames. */
...
const SOLO = [WALKERS[0]];
/** A later group: the same three lanes, different ranks on them. */
const LATER_GROUP = [
    { position: 11, enemyKey: "enemy_1032_katar", d: LANE_MID, speed: 50, waits: [], ...spine("enemy_1032_katar") },
    { position: 12, enemyKey: "enemy_1029_shdsbr", d: LANE_BOTTOM, speed: 50, waits: [], ...spine("enemy_1029_shdsbr") },
];
...
/** MapView's stage: dark gridded surface, perspective rig, board, routes, chibis on top. */
const Stage = ({ focus, walkers }: { focus: number; walkers: unknown[] }) => (
...
/** One boss walking her patrol: FrostNova on the focused route. */
export const SingleWalker = () => <Stage focus={0} walkers={SOLO} />;

/** The focused spawn group — every enemy that entered within 10s of it. */
export const SpawnGroup = () => <Stage focus={0} walkers={WALKERS} />;

/** A later spawn group — a Bladed Fighter and a Shielded Soldier on the two mob lanes. */
export const LaterGroup = () => <Stage focus={11} walkers={LATER_GROUP} />;

/** With walking chibis switched off the layer draws nothing; the route overlay carries the board. */
