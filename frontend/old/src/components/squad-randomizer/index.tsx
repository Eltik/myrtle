                ...
                <ScrollArea className="h-full min-h-[300px] flex-grow rounded-md border">
                    <div className="p-2">
                        {isLoading ? (
                            ...
                            viewMode === "list" ? (
                                <div className="space-y-0.5">{filteredOperators.slice(0, visibleOperatorCount).map((op, index) => renderOperatorListItem(op, excludedOperators, setExcludedOperators, index === visibleOperatorCount - 1 ? lastCharacterRef : null))}</div>
                            ) : (
                                <div className="grid grid-cols-3 gap-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-7 xl:grid-cols-8">
                                    {filteredOperators.slice(0, visibleOperatorCount).map((op, index) => renderOperatorGridItem(op, excludedOperators, setExcludedOperators, index === visibleOperatorCount - 1 ? lastCharacterRef : null))}
                                </div>
                            )
                        ) : (
                            <p className="text-center text-muted-foreground">No operators match filters.</p>
                        )}
                    </div>
                </ScrollArea>
            </div>
