...
class SafeSaver(WorkerCtrl):
    ...
    def save_object(
        ...
        if obj.object_reader is None or obj.object_reader.byte_size == 0:
            ...
            if name.lower().endswith('.json') or 'json' in name.lower():
                # Save as JSON file with .json extension
                byte = obj.m_Script.encode("utf-8", "surrogateescape")
                SafeSaver.save_bytes(
                    byte, destdir, name, ".json", on_queued, on_saved
                )
            else:
                # Save as raw file with no extension
                byte = obj.m_Script.encode("utf-8", "surrogateescape")
                SafeSaver.save_bytes(
                    byte, destdir, name, SafeSaver._EXT_RAW, on_queued, on_saved
                )
