...
class _ArkUnpackerArgParser(argparse.ArgumentParser):
    ...
    def instantiate():
        ...
        parser.add_argument(
            ...
            "--resume",
            action="store_true",
            help="in resolve ab mode: resume processing from a checkpoint file, skipping already processed files",
        )
        parser.add_argument(
            "--skip-problematic",
            action="store_true",
            help="in resolve ab mode: skip files that caused problems in previous runs",
        )
        parser.add_argument(
            "--timeout",
            type=int,
