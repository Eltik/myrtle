...
fn main() {
    ...
    for path in &args {
        ...
        let skin = PathBuf::from(path).file_stem().map(|s| s.to_string_lossy().to_string()).unwrap_or_default();
        for entry in &bundle.files {
            ...
            for (_p, (cid, v)) in &all {
                ...
                let Some(sp) = v.get("m_SavedProperties") else { continue };
                ...
                let bites = (d1 && a1.is_some_and(|x| x > 0.01)) || (d2 && a2.is_some_and(|x| x > 0.01));
                tot += 1;
                if bites {
                    active += 1;
                }
                println!(
                    ...
                    skin,
                    mname,
                    a1.map(|x| format!("{x:.3}")).unwrap_or_else(|| "-".into()),
                    a2.map(|x| format!("{x:.3}")).unwrap_or_else(|| "-".into()),
                    fl(sp, "_BorderWidth_01").map(|x| format!("{x:.2}")).unwrap_or_else(|| "-".into()),
                    fl(sp, "_BorderWidth_02").map(|x| format!("{x:.2}")).unwrap_or_else(|| "-".into()),
                    fl(sp, "_pow").map(|x| format!("{x:.2}")).unwrap_or_else(|| "-".into()),
                    if sp.get("m_Colors").and_then(|c| c.get("_Edgecolor")).is_some() { "yes" } else { "-" },
                    d1,
                    d2,
