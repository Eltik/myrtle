...
fn main() {
    ...
    for entry in &bundle.files {
        ...
        println!("GameObjects with an animated m_IsActive window: {}", w.len());
        let mut rows: Vec<(String, Option<f32>, Option<f32>, i64)> = w.iter()
            .map(|(go, (f, u))| (gname.get(go).cloned().unwrap_or_default(), *f, *u, *go)).collect();
        ...
        for (n, f, u, go) in &rows {
            ...
            for _ in 0..6 {
                ...
                let Some(fa) = all.get(&tt).and_then(|(_, x)| x.get("m_Father").and_then(pid)).filter(|p| *p != 0) else { break };
                if let Some(g) = all.get(&fa).and_then(|(_, x)| x.get("m_GameObject").and_then(pid)) {
                    chain.push(gname.get(&g).cloned().unwrap_or_default());
                }
                t = Some(fa);
            }
            println!("  '{n}'  activeFrom={f:?} activeUntil={u:?}   under: {}", chain.join(" < "));
        }
    }
}
