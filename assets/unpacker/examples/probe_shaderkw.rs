...
fn main() {
    ...
    let want: i64 = std::env::args().nth(2).expect("pathID").parse().expect("pathID must be an integer");
    ...
    for entry in &bundle.files {
        ...
        for obj in &sf.objects {
            if obj.class_id != 48 || obj.path_id != want { continue }
            ...
            if !table.is_empty() { println!("keyword table ({}): {table:?}", table.len()); }
            let pf = v.get("m_ParsedForm");
            let subs = pf.and_then(|p| p.get("m_SubShaders")).and_then(Value::as_array).cloned().unwrap_or_default();
            for (si, sub) in subs.iter().enumerate() {
                ...
                for (pi, pass) in passes.iter().enumerate() {
                    for stage in ["progVertex", "progFragment"] {
                        let Some(prog) = pass.get(stage) else { continue };
                        let sp = prog.get("m_SubPrograms").and_then(Value::as_array).cloned().unwrap_or_default();
                        for (k, s) in sp.iter().enumerate() {
                            let idx: Vec<i64> = s.get("m_KeywordIndices").and_then(Value::as_array)
                                .map(|a| a.iter().filter_map(Value::as_i64).collect()).unwrap_or_default();
                            let named: Vec<&str> = idx.iter()
                                .filter_map(|i| table.get(usize::try_from(*i).ok()?).map(String::as_str))
                                .collect();
                            let inline = strs(s.get("m_GlobalKeywords"));
                            let local = strs(s.get("m_LocalKeywords"));
                            println!("  sub[{si}] pass[{pi}] {stage}[{k}] indices={idx:?} named={named:?} global={inline:?} local={local:?}");
                        }
                    }
                }
            }
