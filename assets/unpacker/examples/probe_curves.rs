...
fn go_path(all: &HashMap<i64, (i32, Value)>, go_pid: i64) -> String {
    ...
    for _ in 0..64 {
        ...
        let tf = gv.get("m_Component").and_then(Value::as_array).and_then(|c| {
            c.iter().find_map(|x| {
...
fn decode_streamed(data: &[u32]) -> HashMap<u32, Vec<Seg>> {
    ...
    while i + 1 < data.len() {
        ...
        for _ in 0..n {
            ...
            let idx = data[i];
            let c = [bits(data[i + 1]), bits(data[i + 2]), bits(data[i + 3]), bits(data[i + 4])];
            i += 5;
...
fn main() {
    let path = std::env::args().nth(1).expect("usage: probe_curves <bundle.ab> <clip> [pathfilter]");
    let clipwant = std::env::args().nth(2).unwrap_or_default();
    let pathwant = std::env::args().nth(3).unwrap_or_default().to_ascii_lowercase();
    ...
    for entry in &bundle.files {
        ...
        for (_pid, (cid, v)) in &all {
            ...
            let clip = v.get("m_MuscleClip").and_then(|m| m.get("m_Clip")).and_then(|c| c.get("data"));
            let streamed = u32s(clip.and_then(|c| c.get("m_StreamedClip")).and_then(|s| s.get("data")));
            let sc = clip.and_then(|c| c.get("m_StreamedClip")).and_then(|s| s.get("curveCount")).and_then(Value::as_u64).unwrap_or(0) as u32;
            let dense = f32s(clip.and_then(|c| c.get("m_DenseClip")).and_then(|d| d.get("m_SampleArray")));
            let dc = clip.and_then(|c| c.get("m_DenseClip")).and_then(|d| d.get("m_CurveCount")).and_then(Value::as_u64).unwrap_or(0) as u32;
            let drate = clip.and_then(|c| c.get("m_DenseClip")).and_then(|d| d.get("m_SampleRate")).and_then(Value::as_f64).unwrap_or(30.0) as f32;
            let dbegin = clip.and_then(|c| c.get("m_DenseClip")).and_then(|d| d.get("m_BeginTime")).and_then(Value::as_f64).unwrap_or(0.0) as f32;
            let constant = f32s(clip.and_then(|c| c.get("m_ConstantClip")).and_then(|d| d.get("data")));

            let segs = decode_streamed(&streamed);
            println!("== [{name}]  bindings {}  streamed {sc}  dense {dc}  constant {}", binds.len(), constant.len());
            ...
            for b in binds {
                ...
                let dim = if ty == 4 { match a { 1 | 3 | 4 => 3, 2 => 4, _ => 1 } } else { 1 };
                let pname = hash_to_path.get(&p).cloned().unwrap_or_else(|| format!("<{p:#x}>"));
                let aname = if ty == 4 { match a { 1 => "position", 2 => "rotation", 3 => "scale", 4 => "euler", _ => "?" }.to_string() } else { format!("{a:#x}") };
                rows.push((pname, aname, off, dim));
                off += dim;
                total += dim;
            }
            println!("   expanded to {total} float curve(s)  [streamed {sc} + dense {dc} + constant {}]", constant.len());
            ...
            let val = |ci: u32, t: f32| -> Option<f32> {
                ...
                if ci < sc + dc {
                    ...
                    let fr = (((t - dbegin) * drate).max(0.0) as usize).min(if dc > 0 { dense.len() / dc as usize - 1 } else { 0 });
