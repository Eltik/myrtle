...
fn nf(v: &Value, k: &str) -> f64 { v.get(k).and_then(Value::as_f64).unwrap_or(f64::NAN) }
    ...
    for entry in &bundle.files {
        ...
        for (p, (cid, v)) in &all {
            if *cid == 1 {
                ...
                gactive.insert(*p, v.get("m_IsActive").and_then(Value::as_i64).unwrap_or(-1));
            }
        }
        for (cid, v) in all.values() {
            ...
            println!("\n##### Animator on GO '{name}'  m_IsActive={}", gactive.get(&go).copied().unwrap_or(-1));
            for k in ["m_Enabled","m_Speed","m_UpdateMode","m_ApplyRootMotion","m_HasTransformHierarchy"] {
                if let Some(x) = v.get(k) { println!("    {k} = {x}"); }
            }
            let Some(cp) = v.get("m_Controller").and_then(pid) else { continue };
            let Some((_, ctrl)) = all.get(&cp) else { println!("    controller {cp} EXTERNAL"); continue };
            let clips: Vec<String> = ctrl.get("m_AnimationClips").and_then(Value::as_array).map(|a| a.iter().filter_map(|c| pid(c)).map(|p|
                all.get(&p).and_then(|(_, cv)| cv.get("m_Name")).and_then(Value::as_str).unwrap_or("?").to_string()).collect()).unwrap_or_default();
            println!("    controller clips: {clips:?}");
            let Some(c) = ctrl.get("m_Controller") else { continue };
                ...
                for (si, sm) in sms.iter().enumerate() {
                    if let Some(states) = sm.get("m_StateConstantArray").and_then(Value::as_array) {
                        for (i, st) in states.iter().enumerate() {
                            println!("    sm{si} state{i}: speed={} cycleOffset={} mirror={:?} iKOnFeet={:?}",
                                nf(st, "m_Speed"), nf(st, "m_CycleOffset"),
                                st.get("m_Mirror").and_then(Value::as_i64), st.get("m_IKOnFeet").and_then(Value::as_i64));
                            if let Some(trs) = st.get("m_TransitionConstantArray").and_then(Value::as_array) {
                                for (ti, tr) in trs.iter().enumerate() {
                                    println!("        transition{ti}: duration={} offset={} exitTime={} hasExitTime={:?} atomic={:?}",
                                        nf(tr, "m_TransitionDuration"), nf(tr, "m_TransitionOffset"), nf(tr, "m_ExitTime"),
                                        tr.get("m_HasExitTime").and_then(Value::as_i64), tr.get("m_Atomic").and_then(Value::as_i64));
                                }
                            }
                        }
                    }
