...
fn main() {
    ...
    let want = std::env::args().nth(2).unwrap_or_else(|| "dyn_entrance".into()).to_ascii_lowercase();
    ...
    for entry in &bundle.files {
        ...
        let root_of = |go: i64| -> String {
            ...
            let mut last = cur;
            while cur != 0 { last = cur; cur = father.get(&cur).copied().unwrap_or(0); }
            name.get(&go_of_tf.get(&last).copied().unwrap_or(0)).cloned().unwrap_or_default()
        };
        ...
        for (n, en) in rows.iter().take(60) {
            println!("   {}{}", n, if *en { "" } else { "   (renderer DISABLED)" });
        }
        if rows.len() > 60 { println!("   … and {} more", rows.len() - 60); }
    }
}
