...
fn scan(path: &str) {
    {
        ...
        for e in &bundle.files {
            ...
            for o in &sf.objects {
                if let Ok(v) = read_object(&sf, o) { objs.insert(o.path_id, (o.class_id, v)); }
            }
        }
        let name_of = |go: i64| -> String {
            objs.get(&go).and_then(|(_, v)| v.get("m_Name")).and_then(Value::as_str).unwrap_or("?").to_string()
        };
        println!("=== {path}");
        let mut rows: Vec<String> = Vec::new();
        for (cid, v) in objs.values() {
            if *cid != 114 { continue }
            let Some(script) = v.get("m_Script").and_then(pid) else { continue };
            if script != UVSCROLL { continue }
            let go = v.get("m_GameObject").and_then(pid).unwrap_or(0);
            let (xs, ys) = (f(v, "xspeed"), f(v, "yspeed"));
            let use2 = v.get("useSecondMap").and_then(Value::as_bool).unwrap_or(false)
            ...
            let keep = v.get("keepInitOffset").and_then(Value::as_bool).unwrap_or(false);
            if xs == 0.0 && ys == 0.0 && sx == 0.0 && sy == 0.0 { continue }
            rows.push(format!(
