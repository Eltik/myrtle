...
fn dump_mm(tag: &str, mm: &Value) {
    ...
    println!(
        "    {tag}: minMaxState={state} scalar={:.4} minScalar={:.4}",
        f(mm, "scalar"),
        f(mm, "minScalar")
    );
    for which in ["maxCurve", "minCurve"] {
            ...
            .and_then(Value::as_array)
        {
            let vals: Vec<String> = c
                .iter()
                .take(8)
                .map(|k| format!("{:.3}@{:.2}", f(k, "value"), f(k, "time")))
                .collect();
            let hi = c.iter().map(|k| f(k, "value")).fold(f64::MIN, f64::max);
            println!(
                "      {which}: {} keys, max value {hi:.4}  first: {}",
                c.len(),
                vals.join(" ")
            );
        }
    ...
    for entry in &bundle.files {
        ...
        for (cid, v) in all.values() {
            ...
            println!(
                ...
                v.get("InitialModule")
                    .and_then(|i| i.get("maxNumParticles"))
            ...
            if let Some(im) = v.get("InitialModule") {
                if let Some(l) = im.get("startLifetime") {
                    dump_mm("startLifetime", l);
                }
                if let Some(s) = im.get("startSize") {
                    dump_mm("startSize", s);
                }
            }
            if let Some(em) = v.get("EmissionModule") {
                println!(
                    "    emission enabled={:?}",
                    em.get("enabled").and_then(Value::as_bool)
                );
                for k in ["rateOverTime", "rateOverDistance"] {
                    if let Some(r) = em.get(k) {
                        dump_mm(k, r);
                    }
                }
