...
fn main() {
    let path = std::env::args()
        .nth(1)
        .expect("usage: probe_separator <bundle.ab>");
        ...
        for (pid, (cid, v)) in &all {
            ...
            let sep: Vec<(&String, &Value)> = v
                ...
                .map(|m| {
                    m.iter()
                        .filter(|(k, _)| {
                            k.to_lowercase().contains("separator")
                                || k.to_lowercase().contains("partsrenderer")
                        })
                        .collect()
                })
                .unwrap_or_default();
            if !sep.is_empty() {
                println!(
                    "  pid {pid} skeletonDataAsset={has_skel} go={:?}",
                    v.get("m_GameObject")
                ...
                for (k, val) in sep {
                    ...
                    if k == "partsRenderers" {
                        for e in val.as_array().into_iter().flatten() {
                            let Some(rp) = e.get("m_PathID").and_then(Value::as_i64) else {
                                continue;
                            };
                            let Some((_, rv)) = all.get(&rp) else {
                                println!("        part pid {rp}: NOT FOUND");
                                continue;
                            };
                            let go = rv
                                .get("m_GameObject")
                            ...
                            println!(
                                "        part pid {rp} go={go:?} sortingOrder={order:?} sortingLayer={layer:?}"
                            );
                        }
                    }
                }
                ...
                let _ = keys;
                println!(
                    "  pid {pid} SKELETON MonoBehaviour (no separator field) go={:?}",
                    v.get("m_GameObject")
                        .and_then(|g| g.get("m_PathID"))
                        .and_then(Value::as_i64)
                );
