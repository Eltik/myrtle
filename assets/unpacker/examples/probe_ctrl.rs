...
fn walk(prefix: &str, v: &Value, out: &mut Vec<String>, depth: usize) {
    ...
    match v {
        Value::Object(o) => {
            ...
            let looks_color = keys.len() == 4 && ["r","g","b","a"].iter().all(|k| o.contains_key(*k));
            if looks_color {
                let f = |k: &str| o.get(k).and_then(Value::as_f64).unwrap_or(f64::NAN);
                let (r,g,b,a)=(f("r"),f("g"),f("b"),f("a"));
                if (r-1.0).abs()>1e-4 || (g-1.0).abs()>1e-4 || (b-1.0).abs()>1e-4 || (a-1.0).abs()>1e-4 {
                    out.push(format!("{prefix} = rgba({r:.4},{g:.4},{b:.4},{a:.4})  <-- NON-WHITE"));
                } else {
            ...
            for (k, vv) in o {
                if k.starts_with("m_") && k != "m_Name" { continue; }
                walk(&format!("{prefix}.{k}"), vv, out, depth + 1);
            }
        }
        Value::Array(a) => { for (i, vv) in a.iter().take(6).enumerate() { walk(&format!("{prefix}[{i}]"), vv, out, depth + 1); } }
        Value::Number(n) => {
            let lk = prefix.to_lowercase();
            if lk.contains("color") || lk.contains("alpha") || lk.contains("tint") || lk.contains("bright") || lk.contains("exposure") || lk.contains("gamma") {
...
fn main() {
    ...
    for entry in &bundle.files {
        ...
        for (p, (cid, v)) in &all { if *cid == 1 { names.insert(*p, v.get("m_Name").and_then(Value::as_str).unwrap_or("").into()); } }
        for (pid, (cid, v)) in &all {
            if *cid != 114 { continue; }
            let go = v.get("m_GameObject").and_then(|g| g.get("m_PathID")).and_then(Value::as_i64).unwrap_or(0);
            let mut out = Vec::new();
            walk("", v, &mut out, 0);
            let interesting: Vec<&String> = out.iter().filter(|l| l.contains("NON-WHITE") || l.contains('=') && !l.ends_with("rgba(1,1,1,1)")).collect();
            if interesting.is_empty() { continue; }
            println!("\nMB pid={pid} on GO '{}'", names.get(&go).cloned().unwrap_or_default());
            for l in interesting.iter().take(14) { println!("   {l}"); }
        }
    }
}
