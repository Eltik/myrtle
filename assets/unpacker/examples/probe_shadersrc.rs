...
fn as_bytes(v: &Value) -> Option<Vec<u8>> {
    match v {
        Value::Array(a) => Some(a.iter().filter_map(|x| x.as_u64().map(|n| n as u8)).collect()),
        Value::String(s) => {
            use base64::Engine;
...
fn nested_u32(v: Option<&Value>) -> Vec<Vec<u32>> {
    let Some(Value::Array(outer)) = v else { return Vec::new() };
    outer
        .iter()
        .map(|inner| match inner {
            Value::Array(a) => a.iter().filter_map(|x| x.as_u64().map(|n| n as u32)).collect(),
...
fn main() {
    ...
    let want: i64 = std::env::args().nth(2).expect("pathID").parse().expect("pathID");
    let min_run: usize = std::env::args().nth(3).and_then(|s| s.parse().ok()).unwrap_or(40);
    ...
    for entry in &bundle.files {
        ...
        for obj in &sf.objects {
            ...
            println!("SHADER '{name}'  blob {} bytes  platforms {platforms:?}", blob.len());

            for (i, p) in platforms.iter().enumerate() {
                let (Some(offs), Some(cl), Some(dl)) = (offsets.get(i), clens.get(i), dlens.get(i)) else { continue };
                for j in 0..offs.len().min(cl.len()).min(dl.len()) {
                    let (o, c, d) = (offs[j] as usize, cl[j] as usize, dl[j] as usize);
                    if o + c > blob.len() {
                        println!("  platform {p} ({}) seg {j}: slice out of range", platform_name(*p));
                        continue;
                    }
                    let out = match lz4::block::decompress(&blob[o..o + c], Some(d as i32)) {
                        Ok(x) => x,
                        Err(e) => {
                            println!("  platform {p} ({}) seg {j}: lz4 failed: {e}", platform_name(*p));
                            continue;
                        }
                    };
                    println!("\n  ===== platform {p} ({}) seg {j}: {} bytes decompressed", platform_name(*p), out.len());
