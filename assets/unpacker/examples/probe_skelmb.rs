    ...
    for path in std::env::args().skip(1) {
        ...
        for entry in &bundle.files {
            ...
            for (cid, v) in all.values() {
                if *cid != 114 || v.get("skeletonDataAsset").is_none() { continue; }
                let go = v.get("m_GameObject").and_then(|g| g.get("m_PathID")).and_then(Value::as_i64).unwrap_or(0);
                println!("  --- spine MonoBehaviour on GO '{}'", names.get(&go).cloned().unwrap_or_default());
                if let Some(o) = v.as_object() {
                    for (k, val) in o {
                        if k.starts_with("m_") && k != "m_Enabled" { continue; }
