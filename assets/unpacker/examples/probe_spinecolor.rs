    ...
    for path in std::env::args().skip(1) {
        ...
        for entry in &bundle.files {
            ...
            for (go, cs) in &chans {
                ...
                if !is_spine
                    && !nm.to_lowercase().contains("illust")
                    && !nm.to_lowercase().contains("entrance")
                {
                    continue;
                }
                println!(
                    "   {} GO '{}' (spine={})",
                    if is_spine { ">>> SPINE" } else { "        " },
                    nm,
                    is_spine
                );
                for c in cs {
                    ...
                    println!(
                        "       crc28=0x{:07X} ch={} n={} t[{:.2}..{:.2}] v[{:.3}..{:.3}] first={:.3} last={:.3}",
            ...
            for go in &spine_gos {
                if !chans.contains_key(go) {
                    println!(
                        "        spine GO '{}' has NO animated material colour",
