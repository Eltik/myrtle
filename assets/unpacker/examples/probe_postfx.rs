...
fn walk(v: &Value, depth: usize, out: &mut Vec<String>, path: &str) {
    ...
    match v {
        ...
        Value::Array(a) => {
            for (i, vv) in a.iter().enumerate().take(8) {
                walk(vv, depth + 1, out, &format!("{path}[{i}]"));
            }
        }
        _ => {}
    }
    ...
    for path in std::env::args().skip(1) {
        ...
        for entry in &bundle.files {
            ...
            for obj in &sf.objects {
                ...
                let raw = format!("{v}");
                let is_vol = raw.contains("sharedProfile")
                    || raw.contains("profile")
                    || raw.contains("isGlobal")
                    || raw.contains("blendDistance");
                if out.len() < 3 && !is_vol {
                    continue;
                }
                if is_vol {
                    ...
                    println!(
                        "   VOLUME-ish on GO '{}': {}",
                        names.get(&go2).cloned().unwrap_or_default(),
                        if raw.len() > 400 {
                            format!("{}…", &raw[..400])
                        } else {
                            raw
                ...
                println!(
                    "   MonoBehaviour on GO '{nm}' (m_Name '{own}') — {} post-fx-ish fields:",
                    out.len()
                );
                for l in out.iter().take(24) {
                    println!("{l}");
                }
            }
        }
