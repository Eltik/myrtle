...
fn kv<'a>(e: &'a Value) -> Option<(&'a str, &'a Value)> {
    let k = e
        .get("first")
...
fn dump_props(mv: &Value) {
    ...
    for (label, key) in [("float", "m_Floats"), ("color", "m_Colors"), ("tex", "m_TexEnvs")] {
        let Some(arr) = sp.get(key).and_then(Value::as_array) else { continue };
        if arr.is_empty() {
            continue;
        }
        for e in arr {
            ...
            match label {
                "float" => println!("      {label:<5} {k:<26} = {}", v.as_f64().unwrap_or(f64::NAN)),
                "color" => {
                    let c: Vec<String> = ["r", "g", "b", "a"]
                        .iter()
                        .map(|c| format!("{:.4}", v.get(c).and_then(Value::as_f64).unwrap_or(f64::NAN)))
                        .collect();
                ...
                _ => {
                    ...
                    println!(
                        ...
                        sc.map(|s| (s.get("x").and_then(Value::as_f64), s.get("y").and_then(Value::as_f64))),
                        of.map(|s| (s.get("x").and_then(Value::as_f64), s.get("y").and_then(Value::as_f64)))
                    );
                }
            }
        }
                    ...
                    match all.get(&mp) {
                        Some((21, mv)) => {
                            println!(
                                ...
                                mv.get("m_Shader"),
                                mv.get("m_ShaderKeywords").and_then(Value::as_str).unwrap_or("")
                            );
                            dump_props(mv);
                        }
