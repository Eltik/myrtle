...
fn main() {
    ...
    for entry in &bundle.files {
        ...
        let names: HashMap<i64, String> = all.iter()
            .filter(|(_, (c, _))| *c == 1)
            .map(|(k, (_, v))| (*k, v["m_Name"].as_str().unwrap_or("").to_string()))
            .collect();
        for (_, (cid, v)) in &all {
            if *cid != 23 { continue } // MeshRenderer
            let go = v.get("m_GameObject").and_then(pid).unwrap_or(0);
            let gname = names.get(&go).cloned().unwrap_or_default();
            let mat_pid = v.get("m_Materials").and_then(Value::as_array).and_then(|a| a.first()).and_then(pid).unwrap_or(0);
            let Some((_, mat)) = all.get(&mat_pid) else {
                println!("  {gname:<28} material UNRESOLVED");
                continue;
            };
            let shader = mat.get("m_Shader").and_then(|s| {
                let fid = s.get("m_FileID").and_then(Value::as_i64)?;
                resolve_shader(&sf.externals, fid, pid(s)?, &map)
            }).unwrap_or("?");
            let tex = mat.get("m_SavedProperties").and_then(|sp| sp.get("m_TexEnvs"))
                .and_then(|t| t.get("_MainTex")).and_then(|t| t.get("m_Texture")).and_then(pid)
                .and_then(|tp| all.get(&tp))
                .map_or_else(|| "none".to_string(), |(_, t)| format!("{}x{}",
