...
fn main() {
    let path = std::env::args().nth(1).expect("usage: probe_psmesh <bundle.ab>");
    ...
    for entry in &bundle.files {
        ...
        for (pid, (cid, v)) in &all {
            ...
            let g = |k: &str| ext.and_then(|e| e.get(k)).and_then(Value::as_f64).unwrap_or(f64::NAN);
        ...
        println!("{} mesh-mode ParticleSystemRenderer(s) in {}", rows.len(), entry.path);
        println!("  {:38} {:22} {:18} {}", "GameObject", "mesh", "size", "aspect");
        for (go, m, size, a) in &rows {
            let flag = if (a - 1.7778).abs() < 0.01 { "  <== 16:9" } else { "" };
