...
fn main() {
    for path in std::env::args().skip(1) {
        ...
        for e in &b.files {
            ...
            let local: HashSet<i64> = sf.objects.iter().filter(|o| o.class_id == 43).map(|o| o.path_id).collect();
            let mut ok = 0; let mut ext = 0; let mut null = 0; let mut missing = 0;
            ...
            for o in sf.objects.iter().filter(|o| o.class_id == 33) {
                ...
                let Some(mp) = v.get("m_Mesh") else { null += 1; continue };
                let fid = mp.get("m_FileID").and_then(Value::as_i64).unwrap_or(0);
                let pid = mp.get("m_PathID").and_then(Value::as_i64).unwrap_or(0);
                if pid == 0 { null += 1; }
                else if fid != 0 { ext += 1; }
                else if local.contains(&pid) { ok += 1; }
                else { missing += 1; }
            }
            let total = ok + ext + null + missing;
            println!("{:<34} MeshFilters {total:>3} | resolved {ok:>3}  EXTERNAL {ext:>3}  null {null:>3}  missing {missing:>3}  -> unit_quad for {}",
                path.rsplit('/').next().unwrap_or("?"), ext + null + missing);
            let mut vs: Vec<usize> = verts.values().copied().collect(); vs.sort();
