    ...
    for path in std::env::args().skip(1) {
        ...
        for entry in &bundle.files {
            ...
            for obj in &sf.objects {
                ...
                let fields = v.as_object().map(|m| {
                    let mut k: Vec<&str> = m.keys().map(String::as_str).filter(|s| !s.starts_with("m_")).collect();
                    k.sort_unstable();
                    k.join(",")
                }).unwrap_or_default();
                let e = by_script.entry(sp).or_insert((0, gname.clone(), fields.clone()));
                e.0 += 1;
                if e.1.is_empty() { e.1 = gname; }
                if e.2.is_empty() { e.2 = fields; }
            }
            if by_script.is_empty() { continue; }
            println!("== {short}");
            ...
            if let Some(want) = std::env::args().nth(2).and_then(|x| x.parse::<i64>().ok()) {
                for obj in &sf.objects {
                    if obj.class_id != 114 { continue; }
                    let Ok(v) = read_object(&sf, obj) else { continue };
                    if v.get("m_Script").and_then(pid) != Some(want) { continue; }
                    let go = v.get("m_GameObject").and_then(pid).unwrap_or(0);
                    println!("   --- on '{}':", names.get(&go).cloned().unwrap_or_default());
                    if let Some(m) = v.as_object() {
                        for (k, vv) in m {
                            if k.starts_with("m_") { continue; }
                            let t = format!("{vv}");
                            println!("        {k} = {}", if t.len() > 120 { format!("{}…", &t[..120]) } else { t });
                        }
                    }
                }
                continue;
            ...
            for (sp, (c, go, fields)) in rows.iter().take(14) {
                let f = if fields.len() > 110 { format!("{}…", &fields[..110]) } else { fields.clone() };
