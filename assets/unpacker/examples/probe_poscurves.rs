        ...
        for entry in &bundle.files {
            ...
            for g in go_tf.keys() {
                ...
                for i in 0..parts.len() {
                    ...
                    hash_name
                        .entry(crc32(&sub.join("/")))
                        .or_insert_with(|| parts[0].clone());
                }
            }
            ...
            for (cid, v) in all.values() {
                ...
                let cname = v
                    ...
                    .unwrap_or("")
                    .to_ascii_lowercase();
                if !(cname.contains("start")
                    || cname.contains("entrance")
                    || cname.contains("enter"))
                {
                    continue;
                }
                let Some(b) = v
                ...
                for (p, (has_pos, has_scale)) in per {
                    if has_pos && !has_scale {
                        ...
                        let owns = go_tf
                            .keys()
                            .any(|g| name(*g) == n && has_renderer.contains(g));
                        rows.push(format!(
                            "      {:38} pos-only{}",
                            n,
                            if owns { "   ** OWNS A RENDERER **" } else { "" }
                        ));
                    }
                }
            }
