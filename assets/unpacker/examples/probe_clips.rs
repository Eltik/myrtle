...
fn main() {
    ...
    for entry in &bundle.files {
        ...
        for (p, (cid, v)) in &all {
            ...
            println!(
                "  pid={p} '{n}'  sampleRate={sr}  start={start:.4} stop={stop:.4} (frames={:.1})  loopTime={loopt} legacy={legacy} wrapMode={wrap}",
                (stop - start) * sr
            );
        }
        ...
        for (cid, v) in all.values() {
            ...
            println!(
                "  class {cid} on GO '{n}'  speed={speed}  controller={ctrl}  playAuto={playauto} wrapMode={wrap} defaultClip={clip}"
            );
