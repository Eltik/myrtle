            ...
            if a.len() > 8 {
                out.push((
                    ...
                    format!(
                        "len={} first={}",
                        a.len(),
                        a[0].to_string().chars().take(60).collect::<String>()
                    ),
                ));
            } else {
...
fn main() {
    ...
    assert!(
        !paths.is_empty(),
        "usage: probe_entrancecfg <bundle.ab> [...]"
    );
        ...
        for entry in &bundle.files {
            ...
            for (pid, (cid, v)) in &all {
                ...
                let go = v
                    ...
                    .unwrap_or("");
                if own.to_ascii_lowercase().contains("entrance")
                    || go.to_ascii_lowercase().contains("entrance")
                {
                    hits.push((
                        *pid,
                        *cid,
