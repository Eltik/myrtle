...
fn main() {
    ...
    for entry in &bundle.files {
            ...
            if let Some(cs) = v.get("m_Component").and_then(Value::as_array) {
                for c in cs {
                    if let Some(cp) = c.get("component").and_then(pid)
                        && matches!(all.get(&cp), Some((4, _))) { go_tr.insert(*p, cp); }
                }
            }
        }
        ...
        let active_of = |go: i64| -> i64 {
            all.get(&go).and_then(|(_, v)| v.get("m_IsActive"))
                .map(|x| x.as_bool().map(i64::from).or_else(|| x.as_i64()).unwrap_or(-1)).unwrap_or(-1)
        };
        let name_of = |p: i64| -> String {
            all.get(&p).and_then(|(_, v)| v.get("m_Name").and_then(Value::as_str)).unwrap_or("?").to_string()
        };
        ...
        for (p, (cid, v)) in &all {
            ...
            let mut r = go; let mut any_inactive = 0i64;
            if active_of(go) == 0 { any_inactive = 1; }
            for _ in 0..64 {
                match parent_go(r) { Some(p2) => { r = p2; if active_of(r) == 0 { any_inactive = 1; } }, None => break }
            }
            rows.push((name_of(go), name_of(r), active_of(go), any_inactive));
        }
        rows.sort();
        let mut idle_active = 0; let mut idle_inactive = 0; let mut ent = 0;
        for (n, root, act, chain) in &rows {
            let is_ent = root.starts_with("dyn_entrance");
            if is_ent { ent += 1; } else if *chain == 0 { idle_active += 1; } else { idle_inactive += 1; }
            if n.contains("fish") || n.contains("xiaoyu") {
                println!("  {:<26} root={:<44} m_IsActive={} anyAncestorInactive={}", n, root, act, chain);
            }
        }
        if !rows.is_empty() {
            println!("\n  {} particle systems: {} entrance-root, IDLE-root {} fully ACTIVE / {} with an inactive ancestor",
                     rows.len(), ent, idle_active, idle_inactive);
        }
    }
}
