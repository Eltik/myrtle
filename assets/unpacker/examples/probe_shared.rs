...
fn main() {
    ...
    let target = std::env::args()
        .nth(2)
        .unwrap_or_else(|| "epoque#28_Start".into());
    ...
    for entry in &bundle.files {
        ...
        for obj in &sf.objects {
            if obj.class_id != 74 || skip.contains(&obj.class_id) {
                continue;
            }
        ...
        let Some(cam) = clips.iter().find(|c| c.0 == target) else {
            println!("clip '{target}' not found");
            return;
        };
        let cam_paths: HashSet<u32> = cam
            .2
            .iter()
            .filter(|(t, _)| *t == 4 || *t == 20)
            .map(|(_, p)| *p)
            .collect();
        println!(
            "camera clip '{}' (stop={:.4}) animates {} distinct transform/camera paths:",
            cam.0,
            cam.1,
            cam_paths.len()
        );
        ...
        for c in &clips {
            ...
            let shared: Vec<(i64, u32)> =
                c.2.iter()
                    .filter(|(_, p)| cam_paths.contains(p))
                    .copied()
                    .collect();
            if shared.is_empty() {
                continue;
        ...
        if !any {
            println!("  NONE — no other clip animates the camera's transform paths.");
        }
        return;
    }
}
