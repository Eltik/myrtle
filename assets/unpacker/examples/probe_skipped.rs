...
fn col(v: &Value, k: &str) -> String {
    v.get(k).and_then(|c| c.get("maxColor")).map_or("-".into(), |m| {
    ...
    for entry in &bundle.files {
        ...
        for (p, (cid, v)) in &all {
            match cid {
                ...
                199 => { if let Some(go) = v.get("m_GameObject").and_then(pid) { grend.insert(go, v.get("m_Enabled").and_then(Value::as_i64).unwrap_or(1)); } }
                _ => {}
            }
        }
        for (cid, v) in all.values() {
            ...
            let mut depth = 0;
            while let Some(tt) = t {
                let Some(f) = all.get(&tt).and_then(|(_, x)| x.get("m_Father").and_then(pid)).filter(|p| *p != 0) else { break };
                if let Some(g) = all.get(&f).and_then(|(_, x)| x.get("m_GameObject").and_then(pid)) {
                    let n = gname.get(&g).cloned().unwrap_or_default();
                    if gactive.get(&g).copied().unwrap_or(1) == 0 && inactive_anc.is_empty() { inactive_anc = n.clone(); }
                    chain.push(n);
                }
                t = Some(f); depth += 1;
                if depth > 8 { break; }
            }
            if !want.is_empty() && !chain.iter().any(|c| c.to_lowercase().contains(&want)) && !name.to_lowercase().contains(&want) { continue; }
            let init = v.get("InitialModule");
            let sc = init.map_or("-".into(), |i| col(i, "startColor"));
            let em_on = v.get("EmissionModule").and_then(|e| e.get("enabled")).and_then(Value::as_i64).unwrap_or(-1);
            let rend_on = grend.get(&go).copied().unwrap_or(-1);
            let go_active = gactive.get(&go).copied().unwrap_or(-1);
            let rm = v.get("ParticleSystemRenderer").and_then(|r| r.get("m_RenderMode")).and_then(Value::as_i64).unwrap_or(-1);
            println!("{name:<28} active={go_active} rendEnabled={rend_on} emission={em_on} rm={rm} startColorMax={sc} inactiveAncestor={}",
                if inactive_anc.is_empty() { "-".into() } else { inactive_anc });
        }
    }
}
