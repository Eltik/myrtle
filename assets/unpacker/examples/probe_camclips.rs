    ...
    for path in std::env::args().skip(1) {
        ...
        for entry in &bundle.files {
            ...
            for (p, (cid, v)) in &all {
                match cid {
                    4 | 224 => {
                        if let Some(g) = pid(v, "m_GameObject") { go_to_tf.insert(g, *p); tf_go.insert(*p, g); }
            ...
            for _ in 0..64 {
                ...
                names.push(all.get(&go).and_then(|(_, v)| v.get("m_Name")).and_then(Value::as_str).unwrap_or("?").to_string());
            ...
            for (_, (cid, v)) in &all {
                ...
                for bd in b {
                    ...
                    let tid = bd.get("typeID").or_else(|| bd.get("classID")).and_then(Value::as_i64).unwrap_or(0);
                    if !hashes.contains(&path) { continue; }
                ...
                rows.push(format!("      {:<52} [{}]  {}", name, hits.join(","), if gated { "admitted" } else { "** DROPPED by the name gate **" }));
            }
