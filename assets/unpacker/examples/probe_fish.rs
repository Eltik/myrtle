...
fn main() {
    ...
    for entry in &bundle.files {
        ...
        let tf_parent = |t: i64| -> Option<i64> {
            all.get(&t)
                .and_then(|(_, v)| v.get("m_Father").and_then(pid))
                .filter(|p| *p != 0)
        };
        let tf_go = |t: i64| -> Option<i64> {
            all.get(&t)
                .and_then(|(_, v)| v.get("m_GameObject").and_then(pid))
        };
        for (cid, v) in all.values() {
            ...
            let mut chain = vec![format!(
                "{name}[active={}]",
                go_active.get(&go).copied().unwrap_or(-1)
            )];
            ...
            while let Some(tt) = t.and_then(tf_parent) {
                ...
                if let Some(g) = tf_go(tt) {
                    chain.push(format!(
                        ...
                        go_active.get(&g).copied().unwrap_or(-1)
                    ));
                }
                t = Some(tt);
                depth += 1;
