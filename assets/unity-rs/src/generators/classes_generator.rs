...
//! 4. Handles inheritance hierarchies (Object → NamedObject → Texture → Texture2D)
...
fn get_base_type_map() -> HashMap<&'static str, &'static str> {
    ...
    // Floating point types → f32 or f64
...
struct NodeClass {
    ...
    /// Fields in this class (name → field)
...
fn generate_field_type(
    ...
    let result = if let Some(&py_typ) = base_type_map.get(typename.as_str()) {
        ...
        // PPtr<T> → PPtrData<T>, and map the inner type
...
fn main() {
    println!("🔧 UnityRs ClassesGenerator");
    println!("================================\n");

    // Step 1: Load TPK data from embedded resource
    println!("📦 Loading TPK database...");
    ...
    println!(
        "   ✓ Loaded {} nodes, {} strings",
        nodes.len(),
        strings.len()
    );

    // Step 2: Process all Unity classes
    println!("\n🔨 Processing Unity classes...");
    ...
    println!("   ✓ Processed {} Unity classes", main_classes.len());
    ...
    println!("   ✓ Sorted {} classes", sorted_classes.len());

    // Step 4: Generate output file
    println!("\n📝 Writing generated.rs...");
